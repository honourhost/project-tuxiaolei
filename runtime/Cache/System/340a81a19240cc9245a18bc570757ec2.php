					...
					<table width="100%" cellspacing="0">
						...
						<tbody>
							<?php if(is_array($group_list)): if(is_array($group_list)): $i = 0; $__LIST__ = $group_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
										...
										<td class="textcenter"><a href="<?php echo U('Group/order_list',array('group_id'=>$vo['group_id']));?>">订单列表</a> | <a href="<?php echo U('Group/reply_list',array('group_id'=>$vo['group_id']));?>">评论列表</a> | <a href="<?php echo U('Merchant/merchant_login',array('mer_id'=>$vo['mer_id'],'group_id'=>$vo['group_id']));?>">编辑</a> | <?php if($vo['group_lock'] == 0): ?><a href="<?php echo U('Group/lockGroup',array('group_id'=>$vo['group_id']));?>">锁定</a><?php else: ?>已锁定<?php endif; ?> | <a href="javascript:void(0);" class="cancle_row" parameter="group_id=<?php echo ($vo['group_id']); ?>" url="<?php echo U('Group/group_cancle');?>">删除</a></td>
