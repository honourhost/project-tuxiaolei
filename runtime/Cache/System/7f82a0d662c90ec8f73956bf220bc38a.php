					...
					<table width="100%" cellspacing="0">
						...
						<tbody>
							<?php if(is_array($merchant_list)): if(is_array($merchant_list)): $i = 0; $__LIST__ = $merchant_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
										...
										<?php if($vo['menus']): ?><td ><?php echo ($vo["account"]); ?></td>
										<td ><?php echo ($vo["name"]); ?></td>
										<?php else: ?>
										<td style="color:red;"><?php echo ($vo["account"]); ?></td>
										<td style="color:red;"><?php echo ($vo["name"]); ?>(未进行实名认证商户)</td><?php endif; ?>
										...
										<td class="textcenter"><a href="<?php echo U('Merchant/cash_info',array('mer_id'=>$vo['mer_id']));?>">查看提现记录</a></td>
										<td class="textcenter"><a href="<?php echo U('Merchant/order',array('mer_id'=>$vo['mer_id']));?>">查看账单</a>|<a href="<?php echo U('Merchant/fastorder',array('mer_id'=>$vo['mer_id']));?>">查看快捷付款账单</a></td>
