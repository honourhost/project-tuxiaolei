$(function(){
	$.getScript("http://api.map.baidu.com/getscript?v=2.0&ak=6116G5OMK3BEzR26gFvU1NYn",function(){
		var map = null;
	...
	// });
	// $('.J-piclist-wrapper li a').click(function(){
...
function get_reply_list(page){
	...
	$.post(get_reply_url,{tab:$('.rate-filter__item .on').attr('data-tab'),order:$('.J-filter-ordertype').val(),page:page},function(result){
		...
		if(result == '0'){
			...
			$.each(result.list,function(i,item){
				var item_html = '<dd class="cf"><div class="appraise_li-list_img"><div class="appraise_li-list_icon"><img width="48" height="48" src="'+(item.avatar ? item.avatar : site_url+'/static/images/portrait.jpg')+'" /></div><div class="appraise_li-list_tit"><p>'+item.nickname+'</p><div class="appraise_li-list_data">'+item.add_time+'</div><div class="appraise_li-list_txt">'+item.comment+'</div></div><div style="clear: both;"></div></div><div class="appraise_li-list_right cf"><div class="appraise_li-list_top cf"><div class="appraise_li-list_top_icon"><div><span style="width:'+(parseInt(item.score)/5*100)+'%"></span></div></div></div>';
				if(item.pics){
					...
					$.each(item.pics,function(j,jtem){
						item_html+= '<li m-src="'+jtem.m_image+'" big-src="'+jtem.image+'"><a class="pic-thumbnail" href="javascript:void(0);"  onclick="showPic(this);" hidefocus="true"><img src="'+jtem.s_image+'"></a></li>';
					});
					item_html+= '<div style="clear: both;"></div></ul></div></div>';
				}
				if(item.merchant_reply_content != ''){
					item_html+= '<p class="biz-reply"><span></span>商家回复：'+item.merchant_reply_content+'</p>';
				}
	...
	if(!first){
	window.location.href="#anchor-reviews";
	}
	first=false;
}

function showPic(obj){
		var m_src = obj.parentNode.getAttribute('m-src');
		var big_src = obj.parentNode.getAttribute('big-src');
		window.art.dialog({
			width:'60%',
			height:'60%',
			title: '查看图片',
