...
document.cookie=cookie;}};var MobileApp=function(){this.init();this.bindEvents();}
MobileApp.prototype={init:function(){this.querySetting();loadCss('http://xnd.winworlds.cn/tpl/Static/weizan/css/mask.css');},bindEvents:function(){$(window).on("mobileappdataload",$.proxy(this.onAppDataLoad,this));},querySetting:function(){$.ajax({url:"//www.tuniu.com/yii.php?r=api/actapi/activity",type:"GET",dataType:"jsonp",jsonp:"jsoncallback",data:{system:"WEB",pageType:"unhomepage"},success:function(ret){if(ret&&ret.success){var tgInfo=ret.data.GTInfo;for(var i=0,len=tgInfo.length;i<len;i++){if(tgInfo[i].activity_style==8){$(window).trigger("mobileappdataload",tgInfo[i]);break;}}}}});},onAppDataLoad:function(e,data){var newOpen=data.activity_frequence,newClose=data.activity_closelimit;var oldOpen=Cookie.get("MOBILE_APP_SETTING_OPEN"),oldState=Cookie.get("MOBILE_APP_SETTING_STATE")||"OPEN";var open,state;if(newOpen==0){Cookie.set("MOBILE_APP_SETTING_OPEN",newOpen,0,"/",".tuniu.com");state="OPEN";}else{if(oldOpen){state=oldState;}else{Cookie.set("MOBILE_APP_SETTING_OPEN",newOpen,newOpen*24*3600,"/",".tuniu.com");state="OPEN";}}
var self=this;if((newClose!=-1)&&(oldState=="OPEN")){this.closeTimer=setTimeout(function(){self.close();},newClose*1000);}
this.build(data);if(state=="OPEN"){this.openElt.css("left","0");this.closeElt.css("left","-100%");}else{this.openElt.css("left","-100%");this.closeElt.css("left","0");}
this.openElt.find(".btn-close").on("click",$.proxy(this.close,this));this.closeElt.on("click",$.proxy(this.open,this));},build:function(data){this.openElt=$(MobileApp.OpenHtml).appendTo(document.body);this.closeElt=$(MobileApp.CloseHtml).appendTo(document.body);this.openElt.find(".box-background").css("background-color","#"+data.color);this.openElt.find(".background-img img").attr("src",data.bkg.bkg_url);this.openElt.find(".background-img a").attr("href",data.bkg.bkg_linkurl);this.openElt.find(".btn-close img").attr("src",data.button_close.img_url);this.openElt.find(".btn-download a").each(function(index,item){$(this).attr("href",data.button_other[index].btn_url);$(this).find("img").attr("src",data.button_other[index].url);});this.closeElt.find(".background-img img").attr("src",data.button_other[3].url);},open:function(){var self=this;self.closeElt.animate({left:"-100%"},"normal","swing",function(){self.openElt.animate({left:"0"},"normal","swing",function(){Cookie.set("MOBILE_APP_SETTING_STATE","OPEN",365*24*3600,"/",".tuniu.com");})});},close:function(){var self=this;self.openElt.animate({left:"-100%"},"normal","swing",function(){self.closeElt.animate({left:"0"},"normal","swing",function(){Cookie.set("MOBILE_APP_SETTING_STATE","CLOSE",365*24*3600,"/",".tuniu.com");})});}}
MobileApp.OpenHtml=['<div class="m-app-open">','<div class="box-background">','</div>','<div class="box-inner">','   <div class="background-img">','       <a target="_blank" href=""></a>','       <img src="" alt="">','   </div>','   <div class="btn-download">','       <a href="" target="_blank">','           <img src="" alt="">','       </a>','       <a href="" target="_blank">','           <img src="" alt="">','       </a>','       <a href="" target="_blank">','           <img src="" alt="">','       </a>','   </div>','   <div class="btn-close">','       <img src="" alt="">','   </div>','</div>','</div>'].join("\n");MobileApp.CloseHtml=['<div class="m-app-close">','   <div class="background-img">','       <img src="" alt="">','   </div>','</div>'].join("\n");var init=true;document.onmousemove=function(){if(init){if(!window.XMLHttpRequest){init=false;return false;}
(new MobileApp());}
