...
<body>
	...
	<form id="buy-form" action="{pigcms{:U('NewGroup/buy',array('group_id'=>$now_group['group_id'],'sun_id'=>$_GET['sun_id']))}" method="POST" class="wrapper-list" autocomplete="off">
			...
			<dd>
				<dl>
					<dd class="dd-padding kv-line-r">
						...
						<if condition="$now_group['is_first_free'] eq 1">
						<dd class="dd-padding kv-line-r">
							<h6>团长价：</h6>
							<p>{pigcms{$finalprice}元</p>
					...
					<dd class="dd-padding kv-line-r quantity">
						<h6>数量：1</h6>
						<div class="kv-v" style="display: none;">
					...
					<if condition="$now_group['is_first_free'] eq 1 AND $is_tuanzhang eq 1">
						<dd class="dd-padding kv-line-r">
						<h6>团长总价：</h6>
						<span class="kv-v" id="amount">
					
						<span class="J_total-price">{pigcms{$finalprice}元</span>
		...
		<else/>
			<h4>选择收货地址</h4>
			<php>if(!empty($address)){</php>
				<dl class="list">
					<dd>
						<a id="change_address" class="react" href="javascript:void(0);">
							<div class="more more-weak">
								<input type="hidden" name="adress_id" value="{pigcms{$now_group['user_adress']['adress_id']}"/>
								<div class="kv-line">
									<h6>姓名：</h6><p><if condition="$contact_name">{pigcms{$contact_name}<else/>{pigcms{$now_group['user_adress']['name']}</if></p>
								</div>
								<div class="kv-line">
									<h6>手机：</h6><p><if condition="$phone">{pigcms{$phone}<else/>{pigcms{$now_group['user_adress']['phone']}</if></p>
								</div>
								<div class="kv-line">
									<h6>地址：</h6><p><if condition="$address">{pigcms{$address}<else/>{pigcms{$now_group['user_adress']['province_txt']} {pigcms{$now_group['user_adress']['city_txt']} {pigcms{$now_group['user_adress']['area_txt']} {pigcms{$now_group['user_adress']['adress']}</if></p>
								</div>
							</div>
						</a>
					</dd>
			...
			<if condition="$now_group['user_adress']['adress_id']">
				<dl class="list">
					<dd>
						<a id="change_address" class="react" href="{pigcms{:U('My/NewAddress',array('group_id'=>$now_group['group_id'],'current_id'=>$now_group['user_adress']['adress_id'],'sun_id'=>$_GET['sun_id']))}">
							<div class="more more-weak">
								<input type="hidden" name="adress_id" value="{pigcms{$now_group['user_adress']['adress_id']}"/>
								<div class="kv-line">
								...
								<div class="kv-line" style="display:none;">
				...
				<dl class="list">
					<dd>
						<a id="change_address" class="react" href="{pigcms{:U('My/NewAddress',array('group_id'=>$now_group['group_id'],'sun_id'=>$_GET['sun_id']))}">
			...
			<dl class="list">
				<dd class="dd-padding">
					<label class="select">
					<if condition="$delivery_type"><input type="hidden" name="delivery_type" value="{pigcms{$delivery_type}"/></if>
						<select name="delivery_type"  <if condition="$delivery_type">disabled="disabled"</if>>
							<if condition="$delivery_type eq 1"><option value="1" selected>工作日、双休日与假日均可送货</option><else/><option value="1">工作日、双休日与假日均可送货</option></if>
							<if condition="$delivery_type eq 2"><option value="2" selected>只工作日送货</option><else/><option value="2">只工作日送货</option></if>
							<if condition="$delivery_type eq 3"><option value="3" selected>只双休日、假日送货</option><else/><option value="3">只双休日、假日送货</option></if>
							<if condition="$delivery_type eq 4"><option value="4" selected>白天没人，其它时间送货</option><else/><option value="4">白天没人，其它时间送货</option></if>
						</select>
					</label>
				</dd>
			</dl>	
			...
			<dl class="list">
				<dd class="dd-padding">
					<input type="text" class="input-weak" value="{pigcms{$delivery_comment}" <if condition="$address">readonly</if> style="width:100%" placeholder="配送特殊说明，配送公司会尽量调节" name="delivery_comment"/>
				</dd>
			</dl>
		</if>
