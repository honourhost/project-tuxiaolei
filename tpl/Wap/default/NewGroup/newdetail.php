...
<html lang="zh-CN">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="{pigcms{$static_path}css/newcommon.css?210"/>
		...
		<div>
			<div id="scroller">
				...
				<section class="buyBox">
					<div class="priceDiv">
						<!-- <span class="price">￥<strong>{pigcms{$now_group['price']}</strong><span class="old">￥<del><php>echo getGroupPrice($now_group['related_id']);</php>t4p-ccc</del></span></span> -->
						<a class="btn buy-btn btn-large btn-strong" style="left:10px;width:80px;" href="{pigcms{:U('GroupBuy/lists')}">更多拼团</a>&nbsp;
						<if condition="$now_group['end_time'] gt $_SERVER['REQUEST_TIME']">
							<a class="btn buy-btn btn-large btn-strong" style="right:140px;" href="{pigcms{:U('Group/buy',array('group_id'=>$now_group['related_id']))}">直接购买&yen;<php>echo getGroupPrice($now_group['related_id']);</php></a>&nbsp;
							<a class="btn buy-btn btn-large btn-strong" href="{pigcms{:U('NewGroup/buy',array('group_id'=>$now_group['group_id']))}">立即开团&yen;{pigcms{$now_group['price']}</a>
					...
					<div class="saleDiv">
						...
						<span><i class="yesLightIcon"></i>24小时拼团失败自动退款</span>
