<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8" />
    <title>农丁头条</title>
    <meta name="viewport" id="viewport" content="width=device-width, initial-scale=1,user-scalable=no">
    <link rel="stylesheet" type="text/css" href="{pigcms{$static_path}css/public-toutiao.css"/>
    <link rel="stylesheet" type="text/css" href="{pigcms{$static_path}css/index-toutiao.css"/>
    <link rel="stylesheet" type="text/css" href="{pigcms{$static_path}css/swiper.min-toutiao.css"/>

</head>
<body>
<!-- top浮动 -->
<div class="header-fixed">
    <div class="header-top">
        <span class="ruzhu">企业入驻</span>
        <span class="logo"><img src="http://www.xiaonongding.com/tpl/Static/weizan/images/xnd_img/index_logo_small.png" /></span>
        <div class="top-tab">
            <img src="{pigcms{$static_path}images/toutiao/r-nav.png" />
        </div>
    </div>

    <div class="header-nav">
        <ul class="nav-show">
            <li class="on">
                <a href="#">头条</a>
            </li>
            <li>
                <a href="#">人物</a>
            </li>
            <li>
                <a href="#">干货</a>
            </li>
            <li>
                <a href="#">融资</a>
            </li>
            <li>
                <a href="#">新三板</a>
            </li>
            <li>
                <a href="#">VR</a>
            </li>
            <li>
                <a href="#">电商</a>
            </li>
            <li>
                <a href="#">O2O</a>
            </li>
            <li>
                <a href="#">智能硬件</a>
            </li>
            <span class="show-btn"><img src="{pigcms{$static_path}images/toutiao/more-img.jpg" /></span>
        </ul>
        <ul class="more-nav">
            <li class="on">
                <a href="#">头条</a>
            </li>
            <li>
                <a href="#">人物</a>
            </li>
            <li>
                <a href="#">干货</a>
            </li>
            <li>
                <a href="#">融资</a>
            </li>
            <li>
                <a href="#">新三板</a>
            </li>
            <li>
                <a href="#">VR</a>
            </li>
            <li>
                <a href="#">电商</a>
            </li>
            <li>
                <a href="#">O2O</a>
            </li>
            <li>
                <a href="#">智能硬件</a>
            </li>
            <li>
                <a href="#">人物</a>
            </li>
            <li>
                <a href="#">干货</a>
            </li>
            <li>
                <a href="#">融资</a>
            </li>
            <li>
                <a href="#">新三板</a>
            </li>
            <li>
                <a href="#">VR</a>
            </li>
            <li>
                <a href="#">电商</a>
            </li>
            <li>
                <a href="#">O2O</a>
            </li>
            <li>
                <a href="#">智能硬件</a>
            </li>
            <div class="clear-both"></div>
            <span class="hide-btn">
							<img src="{pigcms{$static_path}images/toutiao/header-top.jpg" />
						</span>
        </ul>
    </div>
</div>
<div style="width: 100%; height: 95px;"></div>
<div class="cen-main">
    <!-- 轮播图 -->
    <if condition="$nowcat eq 'all'">
        <div class="index-slide">
            <div class="swiper-container">
                <div class="swiper-wrapper">


                    <volist name="lunbo" id="lun">
                        <div class="swiper-slide">
                            <a href="{pigcms{$lun.url}">
                                <img src="{pigcms{$lun.pic}">
                                <div class="slide-bg">
                                    <h3>{pigcms{$lun.name}</h3>
                                </div>
                            </a>
                        </div>
                    </volist>

                </div>
                <!-- Add Pagination -->
                <div class="swiper-pagination"></div>
            </div>

        </div>
    </if>

    <!-- 轮播图 -->
    <!-- 新闻展示形式1 -->
    <!-- 不带新闻图片 -->
    <div class="news-list">
        <ul>
            <li>
                <div class="new-cen">
                    <div class="list-times">
                        <span>2017-02-13 09:45</span>
                        <label>亿欧网</label>
                        <i>转载</i>
                    </div>
                    <a href="#">
                        <div class="list-con">
                            <h3>获天力士千万美元融资的大象保险，要做智能化保险服务平台,获天力士千万美元融资的大象保险，要做智能化保险服务平台</h3>
                        </div>
                    </a>
                    <div class="clear-both"></div>
                </div>
            </li>
        </ul>
    </div>
    <!--灰色背景  独家、热门 -->
    <div class="dujia">
        <ul>
            <li>
                <a href="#">
                    <i>独家</i>
                    <h3>获天力士千万美元融资的大象保险</h3>
                    <div class="dujia-img" style="background-image: url(http://img2.ctoutiao.com/uploads/thumb/2017/02/10/14866872208419.jpg);"></div>
                    <div class="clear-both"></div>
                </a>
            </li>
            <li>
                <a href="#">
                    <i>热门</i>
                    <h3>获天力士千万美元融资的大象保险</h3>
                    <div class="dujia-img" style="background-image: url(http://img2.ctoutiao.com/uploads/thumb/2017/02/10/14866872208419.jpg);"></div>
                    <div class="clear-both"></div>
                </a>
            </li>
        </ul>
    </div>


    <!-- 话题 -->
    <div class="huati" style="display: none;">
        <ul>
            <li>
                <a href="#">
                    <h3><span>话题</span>获天力士千万美元融资的大象保险</h3>
                    <div class="huati-con">
                        <p>获天力士千万美元融资的大象保险，要做智能化保险服务平台,获天力士千万美元融资的大象保险，要做智能化保险服务平台</p>
                        <div class="huati-img" style="background-image: url(img/logo.jpg);"></div>
                    </div>
                </a>
                <div class="clear-both"></div>
                <div class="yuedu">
                    <a href="#"><span>点击订阅</span></a>
                    <b>899次阅读</b>
                    <b>6篇咨询</b>
                    <div class="clear-both"></div>
                </div>
                <div class="clear-both"></div>
            </li>
        </ul>
    </div>

    <!-- 带新闻图片 -->
    <div class="news-list">
        <ul>
            <li>
                <div class="new-cen">
                    <div class="list-times">
                        <span>2017-02-13 09:45</span>
                        <label>亿欧网</label>
                        <i>转载</i>
                    </div>
                    <a href="#">
                        <div class="list-con">
                            <h3 class="lf-title">获天力士千万美元融资的大象保险，要做智能化保险服务平台</h3>
                            <span style="background-image: url(http://www.xiaonongding.com/upload/merchant/000/000/629/589b22acc7cf6.png);"></span>
                        </div>
                    </a>
                    <div class="clear-both"></div>

                </div>
            </li>
        </ul>
    </div>
    <!-- 带新闻图片 -->
    <div class="news-list">
        <ul>
            <li>
                <div class="new-cen">
                    <div class="list-times">
                        <span>2017-02-13 09:45</span>
                        <label>亿欧网</label>
                        <i>转载</i>
                    </div>
                    <a href="#">
                        <div class="list-con">
                            <h3 class="lf-title">获天力士千万美元融资的大象保险，要做智能化保险服务平台</h3>
                            <span style="background-image: url(http://www.xiaonongding.com/upload/merchant/000/000/629/589b22acc7cf6.png);"></span>
                        </div>
                    </a>
                    <div class="clear-both"></div>

                </div>
            </li>
            <li>
                <div class="new-cen">
                    <div class="list-times">
                        <span>2017-02-13 09:45</span>
                        <label>亿欧网</label>
                        <i>转载</i>
                    </div>
                    <a href="#">
                        <div class="list-con">
                            <h3 class="lf-title">获天力士千万美元融资的大象保险，要做智能化保险服务平台</h3>
                            <span style="background-image: url(http://www.xiaonongding.com/upload/merchant/000/000/629/589b22acc7cf6.png);"></span>
                        </div>
                    </a>
                    <div class="clear-both"></div>

                </div>
            </li>
            <li>
                <div class="new-cen">
                    <div class="list-times">
                        <span>2017-02-13 09:45</span>
                        <label>亿欧网</label>
                        <i>转载</i>
                    </div>
                    <a href="#">
                        <div class="list-con">
                            <h3 class="lf-title">获天力士千万美元融资的大象保险，要做智能化保险服务平台</h3>
                            <span style="background-image: url(http://www.xiaonongding.com/upload/merchant/000/000/629/589b22acc7cf6.png);"></span>
                        </div>
                    </a>
                    <div class="clear-both"></div>

                </div>
            </li>
            <li>
                <div class="new-cen">
                    <div class="list-times">
                        <span>2017-02-13 09:45</span>
                        <label>亿欧网</label>
                        <i>转载</i>
                    </div>
                    <a href="#">
                        <div class="list-con">
                            <h3 class="lf-title">获天力士千万美元融资的大象保险，要做智能化保险服务平台</h3>
                            <span style="background-image: url(http://www.xiaonongding.com/upload/merchant/000/000/629/589b22acc7cf6.png);"></span>
                        </div>
                    </a>
                    <div class="clear-both"></div>

                </div>
            </li>
            <li>
                <div class="new-cen">
                    <div class="list-times">
                        <span>2017-02-13 09:45</span>
                        <label>亿欧网</label>
                        <i>转载</i>
                    </div>
                    <a href="#">
                        <div class="list-con">
                            <h3 class="lf-title">获天力士千万美元融资的大象保险，要做智能化保险服务平台</h3>
                            <span style="background-image: url(http://www.xiaonongding.com/upload/merchant/000/000/629/589b22acc7cf6.png);"></span>
                        </div>
                    </a>
                    <div class="clear-both"></div>

                </div>
            </li>
            <li>
                <div class="new-cen">
                    <div class="list-times">
                        <span>2017-02-13 09:45</span>
                        <label>亿欧网</label>
                        <i>转载</i>
                    </div>
                    <a href="#">
                        <div class="list-con">
                            <h3 class="lf-title">获天力士千万美元融资的大象保险，要做智能化保险服务平台</h3>
                            <span style="background-image: url(http://www.xiaonongding.com/upload/merchant/000/000/629/589b22acc7cf6.png);"></span>
                        </div>
                    </a>
                    <div class="clear-both"></div>

                </div>
            </li>
            <li>
                <div class="new-cen">
                    <div class="list-times">
                        <span>2017-02-13 09:45</span>
                        <label>亿欧网</label>
                        <i>转载</i>
                    </div>
                    <a href="#">
                        <div class="list-con">
                            <h3 class="lf-title">获天力士千万美元融资的大象保险，要做智能化保险服务平台</h3>
                            <span style="background-image: url(http://www.xiaonongding.com/upload/merchant/000/000/629/589b22acc7cf6.png);"></span>
                        </div>
                    </a>
                    <div class="clear-both"></div>

                </div>
            </li>
            <li>
                <div class="new-cen">
                    <div class="list-times">
                        <span>2017-02-13 09:45</span>
                        <label>亿欧网</label>
                        <i>转载</i>
                    </div>
                    <a href="#">
                        <div class="list-con">
                            <h3 class="lf-title">获天力士千万美元融资的大象保险，要做智能化保险服务平台</h3>
                            <span style="background-image: url(http://www.xiaonongding.com/upload/merchant/000/000/629/589b22acc7cf6.png);"></span>
                        </div>
                    </a>
                    <div class="clear-both"></div>

                </div>
            </li>
            <li>
                <div class="new-cen">
                    <div class="list-times">
                        <span>2017-02-13 09:45</span>
                        <label>亿欧网</label>
                        <i>转载</i>
                    </div>
                    <a href="#">
                        <div class="list-con">
                            <h3 class="lf-title">获天力士千万美元融资的大象保险，要做智能化保险服务平台</h3>
                            <span style="background-image: url(http://www.xiaonongding.com/upload/merchant/000/000/629/589b22acc7cf6.png);"></span>
                        </div>
                    </a>
                    <div class="clear-both"></div>

                </div>
            </li>
        </ul>
    </div>


    <div class="clear-both"></div>
    <a class="click-more">点击查看更多</a>
</div>
<!-- 点击左侧更多按钮弹出框 -->
<div class="bg-main">

</div>
<div class="fl-nav">
    <ul>
        <li>
            <a href="#"><i><img src="{pigcms{$static_path}images/toutiao/icon-zixun.png" /></i>资讯</a>
        </li>
        <li>
            <a href="#"><i><img src="{pigcms{$static_path}images/toutiao/icon-zixun.png" /></i>资讯</a>
        </li>
        <li>
            <a href="#"><i><img src="{pigcms{$static_path}images/toutiao/icon-zixun.png" /></i>资讯</a>
        </li>
        <li>
            <a href="#"><i><img src="{pigcms{$static_path}images/toutiao/icon-zixun.png" /></i>资讯</a>
        </li>
        <li>
            <a href="#"><i><img src="{pigcms{$static_path}images/toutiao/icon-zixun.png" /></i>资讯</a>
        </li>
        <li>
            <a href="#"><i><img src="{pigcms{$static_path}images/toutiao/icon-zixun.png" /></i>资讯</a>
        </li>
    </ul>
</div>
</body>
<script src="{pigcms{$static_path}js/jquery-1.9.1.min.js" type="text/javascript" charset="utf-8"></script>

<script>
    $(document).ready(function(){
        $(".show-btn").click(function(){
            $(".nav-show").hide();
            $(".more-nav").show();
        });
        $(".hide-btn").click(function(){
            $(".nav-show").show();
            $(".more-nav").hide();
        });
        $(".top-tab").click(function(){
            $(".bg-main").show();
            $(".fl-nav").show();
        });
        $(".bg-main").click(function(){
            $(".bg-main").hide();
            $(".fl-nav").hide();
        });
    });
</script>
<!-- 轮播图 -->
<script src="{pigcms{$static_path}js/swiper.min.js"></script>

<!-- Initialize Swiper -->
<script>
    var swiper = new Swiper('.swiper-container', {
        pagination: '.swiper-pagination',
        nextButton: '.swiper-button-next',
        prevButton: '.swiper-button-prev',
        slidesPerView: 1,
        paginationClickable: true,
        loop: true,
        centeredSlides: true,
        autoplay: 2500,
        autoplayDisableOnInteraction: false

    });
</script>
</html>
