...
<body id="index">
		...
		<dl class="list" style="margin-top:10px;">
		    <dd>
				<ul class="orderindex">
					<li><a href="{pigcms{:U('My/group_buy_list',array('status'=>-1))}" class="react <if condition="$_GET['status'] neq 1">hover</if>">
						<i class="text-icon">⌺</i>
						<span>已参加正在进行的拼团</span>
					</a>
					</li><li><a href="{pigcms{:U('My/group_buy_list',array('status'=>1))}" class="react <if condition="$_GET['status'] eq 1">hover</if>">
						<i class="text-icon">⌸</i>
						<span>已经结束的拼团</span>
					</a>
					</li>
				...
				<li>
					<a href="{pigcms{:U('GroupBuy/show',array('sun_id'=>$group_buy['sun_id']))}">
						<div class="list-wx-img" style="background-image: url({pigcms{$group_buy.list_pic});">
							<if condition="$group_buy['is_lottery_group_buy'] eq 1">
								<div class="postion">
									<span class="icon-color01">抽奖团</span>
						...
						<div class="list-right">
							...
							<a href="{pigcms{:U('NewGroup/detail',array('group_id'=>$group_buy['group_id']))}">
								<span class="btn-contikaituan" data-url="{pigcms{:U('NewGroup/detail',array('group_id'=>$group_buy['group_id']))}" >继续<if condition="$group_buy['is_lottery_group_buy'] eq 1">抽奖<else/>开团</if></span>
								</a>
							...
							<a href="{pigcms{:U('NewGroup/detail',array('group_id'=>$group_buy['group_id']))}">
								...
								<a href="{pigcms{:U('My/group_order',array('order_id'=>$group_buy['order_id']))}">
									<span class="btn-seeorder" data-url="{pigcms{:U('My/group_order',array('order_id'=>$group_buy['order_id']))}">查看订单</span>
								</a>
							</if>
							
						</div>
					</a>
					<if condition="$uid eq $group_buy['user_id'] ">
					<div class="tuanzhang">
						<img src="http://www.xiaonongding.com/tpl/Wap/default/static/img/tuanzhang.png" />
					</div>
					</if>
					...
					.tuanzhang2{
						...
						<div class="tuanzhang2">
						<img src="http://www.xiaonongding.com/tpl/Wap/default/static/img/pin-sb.png" />
					</div>
					</if>
						...
						<elseif condition="($group_buy['is_lottery_group_buy'] eq 1) AND ($group_buy['status'] eq 1)"/>
						<style>
							.tuanzhang3{
								position: absolute;
								right: 160px;
				 ...
				 <div class="s_empty" style="display:block;">您暂时没有拼团</div>
				 </if>
			</ul>
			<div style="clear: both;"></div>
		</div>
