				...
				<if condition="count($appoint_product) gt 0">
					...
					<div class="comm-service">
						...
						<div class="con-service">
							<div class="con-service-inner" data-role="packageDescription">{pigcms{$appoint_product[0]['name']}：{pigcms{$appoint_product[0]['content']}</div>
						</div>
					</div>
				</if>
							...
							<li>
								<switch name="vo['type']">
									...
									<case value="2">
										...
										<p class="cover">
											<input data-role="position" class="ipt-attr" type="text" name="custom_field[{pigcms{$i}][value]" placeholder="请选择{pigcms{$vo.name}<if condition="!$vo['iswrite']">（可选）</if>" readonly="readonly" <if condition="$vo['iswrite']">data-required="required"</if>/>
										</p>
									</case>
									<case value="3">
										<i class="icon-txt"></i>
									...
									<case value="5">
										...
										<input type="hidden" name="custom_field[{pigcms{$i}][name]" value="{pigcms{$vo.name}"/>
										<input type="hidden" name="custom_field[{pigcms{$i}][type]" value="{pigcms{$vo.type}"/>
										<p class="cover"><input class="ipt-attr" type="email" name="custom_field[{pigcms{$i}][value]" placeholder="请输入{pigcms{$vo.name}<if condition="!$vo['iswrite']">（可选）</if>" data-role="email" <if condition="$vo['iswrite']">data-required="required"</if>/></p>
									</case>
									<case value="6">
										<i class="icon-txt"></i>
										<input type="hidden" name="custom_field[{pigcms{$i}][name]" value="{pigcms{$vo.name}"/>
