...
<html>
	...
	<body>
		...
		<div class="public-cen">
			<!-- 顶部弹窗，前往购买 -->
			<div class="header-go">
				<span><a href="{pigcms{:U('NewGroup/detail',array('group_id'=>$newest_groupbuy[0]['group_id']))}"><if condition="$newest_groupbuy.is_lottery_group_buy eq 1">前往抽奖<else/>前往购买</if></a></span>
			...
			<div class="pin-header" onClick="window.location.href='http://www.xiaonongding.com/wap.php?g=Wap&c=NewGroup&a=detail&group_id={pigcms{$group_buy.group_id}'">
				...
				<p >{pigcms{$group_buy.need_num}人<if condition="$now_group.is_lottery_group_buy eq 1">抽奖团：<else/>团：</if><span>&yen;<i>{pigcms{$now_group['price']}</i></span>/件</p>
				<div style="clear: both;"></div>
				<if condition="$group_buy['status'] eq 1">
				<if condition="$now_group['is_lottery_group_buy'] eq 1">
				<div class="pin-zt">
					<img src="{pigcms{$static_path}img/pin-lottery-over.png" />
				</div>
				<else/>
				<div class="pin-zt">
			...
			<div class="pin-pr">
				<div class="pin-pr-con">
                	...
                	<if condition="$k eq 1">
					<if condition="$user['uid'] eq $lottery['uid']">
					 <a>
                    ...
                    <else/>
						<if condition="$user['uid'] eq $lottery['uid']">
						 <a>
                        <img src="<php>echo getAvatar($user['uid']);</php>" />
                        <span style="background-color:#00bca3;">中奖</span>
                       </a>
						<else/>
						<a>
                        <img src="<php>echo getAvatar($user['uid']);</php>" />
				...
				<div style="clear: both;"></div>
			<switch name="participated">
					<case value="1">
						<if condition="$group_buy['status'] eq 1">
							<if condition="$nowuserid eq $lottery['uid']">
								<p class="pin-p">恭喜您中奖了，手气爆表了哥们！再去试试</p>
							<elseif condition="$now_group.is_lottery_group_buy eq 1"/>
								<p class="pin-p">感谢您的参与，不要灰心再去试试！</p>
								<else/>
								<p class="pin-p">拼团结束，欢迎再次拼团</p>
							</if>
							<elseif condition="$group_buy['status'] eq 2"/>
							<p class="pin-p">拼团失败,请再次发起拼团</p>
							<else/>
							<p class="pin-p">还差<span><php>echo $group_buy['need_num']-$group_buy['react_num'];</php></span>人<if condition="$now_group.is_lottery_group_buy eq 1">就可以开奖啦，赶快邀约小伙伴来拼奖~<else/>，盼你如南方人盼暖气~</if></p>
						</if>
					</case>
					<default />	
					  <if condition="$group_buy['status'] eq 2">
                        <p class="pin-p"><if condition="$now_group.is_lottery_group_buy eq 1">抽奖<else/>拼团</if>失败，请重新发起</p>
                        <elseif condition="$group_buy['status'] eq 1"/>
                        <p class="pin-p"><if condition="$now_group.is_lottery_group_buy eq 1">抽奖<else/>拼团</if>已结束，请重新发起</p>
                        <else/>
                        <p class="pin-p">还差<span><php>echo $group_buy['need_num']-$group_buy['react_num'];</php></span>人<if condition="$now_group.is_lottery_group_buy eq 1">就可以开奖啦，赶快邀约小伙伴来拼奖~{pigcms{$participated}<else/>，盼你如南方人盼暖气~</if></p>
                        </if>
				</switch>
				...
				<div class="pin-time">
					...
					<img class="ma-img" src=" {pigcms{:U('GroupBuy/qrpng2',array('sunid'=>$group_buy['sun_id']))}">
					<img class="circleimg user-img" src="http://www.xiaonongding.com/xnd.png">
					<p> 分享二维码，邀请好友<if condition="$now_group.is_lottery_group_buy eq 1">抽奖<else/>开团</if></p>
				</div>
				...
				<div class="tuan-main">
					<div class="tuan" style="margin-top: 20px;">
						...
						<if condition="$now_group.is_lottery_group_buy eq 1">
						<div class="tuan" style="margin-top: 20px; display:none;">
				...
				<div class="tuan-main">
                    ...
                    <div class="tuan ct-tuan">
							...
							<span class="right time ct-time"><php>echo '<font style="color:#f19325;font-weight:bold;">中奖啦</font> '.date("Y-m-d H:i:s",$user['add_time']);</php> 参团</span>
							<else/>
			...
			<div class="pin-footer">
				<span class="footer-bg"></span>
					<a class="btn-more left" href="{pigcms{:U('GroupBuy/index')}"><img src="{pigcms{$static_path}img/icon-more.png" />返回首页</a>
				<if condition="$group_buy[status] eq 0">
					<if condition="$now_group['end_time'] gt $_SERVER['REQUEST_TIME']">
						<switch name="now_group['is_lottery_group_buy']">
							<case value="1" break="1"><a class="btn-bao right" href="{pigcms{:U('NewGroup/buy',array('group_id'=>$group_buy['group_id'],'sun_id'=>$group_buy['sun_id']))}">参与抽奖</a></case>
							<case value="0" break="0"><a class="btn-bao right" href="{pigcms{:U('NewGroup/buy',array('group_id'=>$group_buy['group_id'],'sun_id'=>$group_buy['sun_id']))}">我也要参团</a></case>
					...
					<if condition="$now_group['end_time'] gt $_SERVER['REQUEST_TIME']">
						<switch name="now_group['is_lottery_group_buy']">
							<case value="1"><a class="btn-bao right" href="{pigcms{:U('NewGroup/detail',array('group_id'=>$group_buy['group_id']))}">发起抽奖</a></case>
							<case value="0"><a class="btn-bao right" href="{pigcms{:U('NewGroup/detail',array('group_id'=>$group_buy['group_id']))}">我也要开团</a></case>
							<default /><a class="btn-bao right" href="{pigcms{:U('NewGroup/detail',array('group_id'=>$group_buy['group_id']))}">我也要开团</a>
						</switch>
		...
		<script type="text/javascript">
			...
			var shareData={
						...
						title: <if condition="$now_group['is_lottery_group_buy'] eq 1">'{pigcms{$username}邀你试手气，价值{pigcms{$now_group.old_price}元{pigcms{$now_group.s_name}等你拿！'<else/><if condition="$username">'{pigcms{$username}喊你一起买"{pigcms{$now_group.s_name}"，仅售{pigcms{$now_group.price}元！'<else/>'小农丁喊你一起买"{pigcms{$now_group.s_name}"，仅售{pigcms{$now_group.price}元！'</if></if>,
