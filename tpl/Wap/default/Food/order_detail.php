...
<div data-role="container" class="container orderDetails processing pay_over">
	<header data-role="header">	
		...
		<ul class="pay" style="padding-right: 20px;">
			...
			<li style="display: none;">餐桌情况<label>{pigcms{$order['tablename']}</label></li>
			<li style="display: none;">人数<label>{pigcms{$order['num']}人</label></li>
			...
			<li style="display: none;">预约时间<label>{pigcms{$order['arrive_time'] | date="Y-m-d H:i",###}</label></li>
			...
			<span>
				...
				<div class="list">
					...
					<volist name="meallist" id="dishs">
						<li>
							<span class="col1">{pigcms{$dishs['name']}<if condition="isset($dishs['isadd']) AND $dishs['isadd']"><sup><i style="color:green;margin-left:5px;"><strong>加商品</strong></i></sup></if></span>
				...
				<else/>
				  <div class="tips" style="display:block;">亲，您还没有选购商品喔！</div>
				  	<div class="list">
					<ul>
					</ul>
				</if>
