<html>
<head>
    <meta charset="utf-8">
    <meta content="yes" name="apple-mobile-web-app-capable">
    <meta content="yes" name="apple-touch-fullscreen">
    <meta name="data-spm" content="a215s">
    <meta content="telephone=no,email=no" name="format-detection">
    <meta content="fullscreen=yes,preventMove=no" name="ML-Config">
    <meta name="viewport" content="width=device-width,minimum-scale=1,maximum-scale=1,user-scalable=no">
    <title>项目支持者-小农丁众筹</title>

 <link href="{pigcms{$static_path}css/supporter.css" rel="stylesheet">
          <style type="text/css">#wasubX {
            margin: 10px 0;
            background-color: #fff;
            display: table;
            position: relative;
            z-index: 9990
        }

        #wasubX * {
            font-size: 12px;
            font-family: Microsoft YaHei, Tahoma, Helvetica, \\5B8B\4F53, sans-serif;
            outline: 0;
            margin: 0;
            padding: 0
        }

        #wasubX li {
            list-style: none
        }

        #wasubX img {
            border: 0;
            display: block;
            outline: 0
        }

        #wasubX .wasubX-ul {
            position: relative;
            z-index: 9991
        }

        #wasubX .wasubX-ul ul {
            *zoom: 1;
            height: 30px
        }

        #wasubX .wasubX-ul ul:after {
            content: " ";
            display: block;
            clear: both
        }

        #wasubX .wasubX-ul ul li {
            float: left;
            height: 30px
        }

        #wasubX .wasubX-ul ul .wasubX-li {
            -ms-box-sizing: border-box;
            -o-box-sizing: border-box;
            box-sizing: border-box;
            border: 1px solid #e4e4e4;
            border-left: 0;
            background: #fff;
            overflow: inherit
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-title {
            height: 28px;
            line-height: 28px;
            padding: 0 10px;
            cursor: pointer
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-title i {
            width: 16px;
            height: 16px;
            background: red;
            display: inline-block;
            *display: inline;
            *zoom: 1;
            position: relative;
            top: 4px;
            background: url(data:image/png;base64,) no-repeat
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-title i.wasubX-icon-plan {
            background-position: -84px -2px
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-title i.wasubX-icon-coupon {
            background-position: -1px 0
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-title i.wasubX-icon-queqiao {
            background-position: -64px 0
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-title i.wasubX-icon-wx {
            background-position: -110px 0;
            width: 20px;
            height: 20px
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-title span {
            display: inline-block;
            *display: inline;
            *zoom: 1
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-title em {
            display: inline-block;
            *display: inline;
            *zoom: 1;
            font-style: normal;
            color: #ff464e;
            font-weight: 700;
            margin-left: 1px
        }

        #wasubX .wasubX-ul ul .wasubX-li.wasubX-li-last {
            border-right: 0;
            position: relative
        }

        #wasubX .wasubX-ul ul .wasubX-li.wasubX-li-last .wasubX-title {
            padding: 0 5px
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box {
            display: none;
            background-color: #fff;
            position: absolute;
            top: 29px;
            left: 0;
            z-index: -1;
            border-radius: 0 5px 5px 5px;
            border: 1px solid #e4e4e4;
            border-bottom: 3px solid #ff464e;
            width: 540px;
            padding: 15px 20px
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info {
            width: 100%;
            *zoom: 1;
            height: 50px
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info:after {
            content: " ";
            display: block;
            clear: both
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info .wasubX-info-l {
            float: left
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info .wasubX-info-l p {
            color: #666;
            height: 25px;
            line-height: 25px
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info .wasubX-info-l p a {
            color: #2db7f5;
            text-decoration: none
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info .wasubX-info-l p span {
            font-size: 14px;
            color: #ff464e;
            font-weight: 700
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info .wasubX-info-r {
            float: right;
            *zoom: 1
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info .wasubX-info-r:after {
            content: " ";
            display: block;
            clear: both
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info .wasubX-info-r div {
            float: right;
            width: 100px;
            border-left: 1px solid #e4e4e4
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info .wasubX-info-r div:last-child {
            border-left: 0
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info .wasubX-info-r div p {
            text-align: center;
            height: 25px;
            line-height: 25px
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-info .wasubX-info-r div p:first-child {
            font-size: 14px;
            font-weight: 700
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-table {
            table-layout: fixed;
            width: 100%;
            line-height: 1.5;
            margin-top: 15px;
            border-collapse: collapse;
            border-spacing: 0
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-table thead {
            display: table-header-group;
            vertical-align: middle;
            border-color: inherit
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-table tr {
            display: table-row;
            vertical-align: inherit;
            border-color: inherit
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-table tr:hover {
            background-color: #e9f7fd
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-table th {
            font-weight: 700;
            background-color: #f7f7f7;
            padding: 6px 10px;
            border: 1px solid #e9e9e9;
            text-align: center
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-table td {
            padding: 6px 10px;
            border: 1px solid #e9e9e9;
            text-align: center
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-table td a {
            color: #2db7f5;
            cursor: pointer
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-box .wasubX-table td a:hover {
            color: red
        }

        #wasubX .wasubX-ul ul .wasubX-li .wasubX-wx {
            width: auto;
            height: auto;
            left: -87px
        }

        #wasubX .wasubX-ul ul .wasubX-li:hover {
            border-bottom: none;
            border-top: 2px solid #ff464e
        }

        #wasubX .wasubX-ul ul .wasubX-li:hover .wasubX-title {
            line-height: 26px
        }

        #wasubX .wasubX-ul ul .wasubX-li:hover .wasubX-box {
            display: block
        }

        #wasubX .wasubX-ul ul .wasubX-li:hover .wasubX-wx {
            top: 27px
        }

        #wasubX .wasubX-ul ul .wasubX-tui {
            width: 32px
        }

        #wasubX .wasubX-ul ul .wasubX-tui a {
            display: block;
            width: 100%;
            height: 100%;
            background: url(data:image/png;base64,) no-repeat -82px -34px
        }

        #wasubX .wasubX-ul ul .wasubX-logo {
            width: 74px
        }

        #wasubX .wasubX-ul ul .wasubX-logo a {
            display: block;
            width: 100%;
            height: 100%;
            background: url(data:image/png;base64,) no-repeat -1px -35px
        }

        #wasubX .wasubX-ul ul .wasubX-no-news-l {
            border-radius: 5px 0 0 5px
        }

        #wasubX .wasubX-ul ul .wasubX-no-news-r {
            border-radius: 0 5px 5px 0
        }

        #wasubX .wasubX-new {
            width: 100%;
            -ms-box-sizing: border-box;
            -o-box-sizing: border-box;
            box-sizing: border-box;
            border: 1px solid #e4e4e4;
            border-top: 0;
            border-radius: 0 0 5px 5px;
            padding: 5px 15px;
            overflow-x: hidden
        }

        #wasubX .wasubX-new .wasubX-new-p {
            height: 22px;
            line-height: 22px;
            color: #666;
            overflow: hidden
        }

        #wasubX .wasubX-new .wasubX-new-p span {
            display: inline-block;
            *display: inline;
            *zoom: 1;
            padding: 0 4px;
            background: #ff464e;
            color: #fff;
            border-radius: 3px
        }

        #wasubX .wasubX-new .wasubX-new-p a {
            display: inline-block;
            *display: inline;
            *zoom: 1
        }

        #wasubX .wasubX-new .tips-list, #wasubX .wasubX-new .wasubX-dl {
            line-height: 24px;
            overflow: hidden;
            *zoom: 1
        }

        #wasubX .wasubX-new .tips-list:after, #wasubX .wasubX-new .wasubX-dl:after {
            content: " ";
            display: block;
            clear: both
        }

        #wasubX .wasubX-new .tips-list dt, #wasubX .wasubX-new .wasubX-dl dt {
            background: #ff464e;
            color: #fff;
            border-radius: 3px;
            float: left;
            padding: 0 2px;
            margin-top: 2px;
            line-height: 18px;
            margin-right: 6px
        }

        #wasubX .wasubX-new .tips-list dd, #wasubX .wasubX-new .wasubX-dl dd {
            display: block;
            line-height: 22px;
            position: relative;
            text-align: left;
            margin-left: 0 !important
        }

        #wasubX .wasubX-new .tips-list dd a, #wasubX .wasubX-new .wasubX-dl dd a {
            display: block;
            height: 22px;
            overflow: hidden;
            white-space: nowrap;
            text-overflow: ellipsis;
            text-align: left;
            color: #666
        }

        #wasubX .wasubX-new .tips-list dd a:hover, #wasubX .wasubX-new .wasubX-dl dd a:hover {
            text-decoration: underline
        }

        #wasubX .wasubX-footer {
            height: 54px;
            margin-top: 10px
        }

        #wasubX .wasubX-footer p {
            height: 26px;
            line-height: 26px
        }

        #wasubX .wasubX-footer p span {
            margin-right: 8px
        }

        #wasubX .wasubX-footer p a:hover {
            color: #2db7f5
        }

        #wasubX .wasubX-footer p:last-child {
            height: 28px;
            line-height: 28px
        }

        #wasubX .wasubX-footer p:last-child a {
            width: 90px;
            height: 22px;
            display: inline-block;
            *display: inline;
            *zoom: 1;
            background: url(data:image/png;base64,) no-repeat -141px -1px;
            position: relative;
            top: 6px
        }</style>
</head>
<body>
<div class="paddingBottom-40">
    <div class="public-project-background" id="app1" _v-2263fa60="">
        <ul class="public-supporter-list clearfix" _v-2263fa60="">
            <empty name="supporters">
                <li _v-2263fa60="">
                    <div _v-2263fa60="">
                        <a href="javascript:;" _v-2263fa60="">该项目暂时没有支持者</a>
                        </div>
                </li>
                <else />
                <volist name="supporters" id="supporter">
                    <li _v-2263fa60="">
                        <img _v-2263fa60="" src="<?php echo  $supporter['avator'] ?>">
                        <div _v-2263fa60="">
                            <a href="javascript:;" _v-2263fa60="">{pigcms{$supporter.name}</a>
                            <p _v-2263fa60=""><?php echo date("Y-m-d H:i:s",$supporter['paytime']);?> 支持了 <?php echo $supporter['payment_money'];?> 元</p>
                        </div>
                    </li>
                </volist>
            </empty>



        </ul>
        <div class="public-load-bt" _v-2263fa60="" style="display: none;"><span _v-2263fa60="">加载更多</span></div>
    </div>
</div>
<div id="loadingToast" class="weui_loading_toast" style="display: none;">
    <div class="weui_mask_transparent"></div>
    <div class="weui_toast">
        <div class="weui_loading">
            <div class="weui_loading_leaf weui_loading_leaf_0"></div>
            <div class="weui_loading_leaf weui_loading_leaf_1"></div>
            <div class="weui_loading_leaf weui_loading_leaf_2"></div>
            <div class="weui_loading_leaf weui_loading_leaf_3"></div>
            <div class="weui_loading_leaf weui_loading_leaf_4"></div>
            <div class="weui_loading_leaf weui_loading_leaf_5"></div>
            <div class="weui_loading_leaf weui_loading_leaf_6"></div>
            <div class="weui_loading_leaf weui_loading_leaf_7"></div>
            <div class="weui_loading_leaf weui_loading_leaf_8"></div>
            <div class="weui_loading_leaf weui_loading_leaf_9"></div>
            <div class="weui_loading_leaf weui_loading_leaf_10"></div>
            <div class="weui_loading_leaf weui_loading_leaf_11"></div>
        </div>
        <p class="weui_toast_content">Loading</p></div>
</div>
<div>
    <div class="bar">
        <div class="back icon-kaistart-arrows-left"></div>
        <div class="right"></div>
    </div>
    <div class="fixed-space"></div>
</div>

</body>
</html>