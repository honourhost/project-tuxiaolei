$.fn.amount = function(num, callback){
	...
	function add(){
        ...
        var usermoney=parseFloat($(".nowmoney").val()).toFixed(3);
        var totalPrice = parseFloat($("#allmoney").text()) || 0;
        var tempprice= parseFloat($(this).parents('li').find('.tureunit_price').val());
      //  alert(tempprice);
        if(usermoney<(totalPrice+tempprice)){
            layer.alert("余额不足");
            return false;
        }
	...
	return this.each(function(){
      ...
      //  $("body").on("click",$(this).before('<span'+ isShow +'><a href="javascript:void(0);" class="btn minus '+ activeClass +'"></a><span class="num"><input type="text" readonly="true" value="'+num+'"></span></span>'),add);
		$(this).prev().find('.minus').bind('click', del);
	//	$("body").on("click",$(this).prev().find('.minus'),del);
