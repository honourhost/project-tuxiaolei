...
$(function(){
	...
	$('#shoucang').click(function(){
		if($(this).hasClass('liu_group_ico_sc_on')){
			var action="del";
		}else{
			var action="add";
		}
		$.post(collect_url,{action:action,type:$(this).attr('fav-type'),id:$(this).attr('fav-id')},function(result){
			$('body').append('<div class="ts"><p>'+result.info+'</p></div>');
            setTimeout("$('.ts').remove()",3000);
			if(result.status == '1'){
				if(action == 'add'){
					$('#shoucang').addClass('liu_group_ico_sc_on');
				}else{
					$('#shoucang').removeClass('liu_group_ico_sc_on');
                    $('#shoucang').addClass('liu_group_ico_sc');
				}
			}
