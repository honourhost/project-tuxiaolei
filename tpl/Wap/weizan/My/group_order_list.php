...
<body id="index">
        ...
        <dl class="list" style="margin-top:0px;">
		    <dd>
				<ul class="orderindex">
					<li>
                        <a href="{pigcms{:U('My/group_order_list')}" class="react <if condition="empty($_GET['status'])">hover</if>">
    						<span>全部</span>
    					</a>
					</li>
                    <li>
                        <a href="{pigcms{:U('My/group_order_list',array('status'=>-1))}" class="react <if condition="$_GET['status'] == '-1'">hover</if>">
    						<span>待付款</span>
    					</a>
					</li>
                    <li>
			                    ...
			                    <div class="dealcard-block-right">
			                        ...
			                        <div class="liu_list_an">
									    <if condition="$vo['status'] eq '3'">
										   <a class="liu_hui">已取消</a>
			                        	<elseif condition="empty($vo['paid'])" />
			                        		<a class="liu_hong" href="{pigcms{:U('Pay/check',array('type'=>'group','order_id'=>$vo['order_id']))}">付款</a>
			                        	<elseif condition="empty($vo['third_id']) AND $vo['pay_type'] eq 'offline'"/>
			                        		<a class="liu_hong">线下未付款</a>
			                        	<elseif condition="empty($vo['status'])"/>
			                        		<a class="liu_lv">未消费</a>
			                        	<elseif condition="$vo['status'] == '1'"/>
			                        		<a class="liu_huang" href="{pigcms{:U('My/group_feedback',array('order_id'=>$vo['order_id']))}">去评价</a>
