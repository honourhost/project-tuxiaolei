...
<body id="index">
        ...
        <dl class="list" style="margin-top:0rem;">
		    <dd>
				<ul class="orderindex">
					<li>
                        <a href="{pigcms{:U('My/meal_order_list')}" class="react <if condition="empty($_GET['status'])">hover</if>">
    						<span>全部</span>
    					</a>
					</li>
                    <li>
                        <a href="{pigcms{:U('My/meal_order_list',array('status'=>-1))}" class="react <if condition="$_GET['status'] == '-1'">hover</if>">
    						<span>待付款</span>
    					</a>
					</li>
                    <li>
                    ...
                    <li>
                        <a href="{pigcms{:U('My/meal_order_list',array('status'=>2))}" class="react <if condition="$_GET['status'] == '2'">hover</if>">
    						<span>待评价</span>
    					</a>
					</li>
				</ul>
		    			...
		    			<volist name="order_list" id="order">
					        ...
					        <dd class="dealcard dd-padding" onclick="window.location.href = '{pigcms{:U('Food/order_detail', array('mer_id' => $order['mer_id'], 'store_id' => $order['store_id'], 'order_id' => $order['order_id']))}';">
			                    ...
			                    <div class="dealcard-block-right">
			                        ...
			                        <small>总价：<if condition="$order['total_price'] gt 0">{pigcms{$order['total_price'] - $order['minus_price']}<else />{pigcms{$order['price']}</if> 元&nbsp;&nbsp;数量：{pigcms{$order['total']}</small>　<small style="color: green"><if condition="$order['meal_type']">外卖<else />{pigcms{$config.meal_alias_name}</if></small>
			                        <div class="liu_list_an">
									<if condition="$order['status'] eq 3">
			                        		<a class="liu_hui">已取消并退款</a>
			                        	<elseif condition="empty($order['paid'])" />
			                        		<if condition="$order['meal_type'] eq 1">
			                        		<a class="liu_hong" href="{pigcms{:U('Pay/check',array('order_id' => $order['order_id'], 'type'=>'takeout'))}">付款</a>
			                        		<else />
			                        		<a class="liu_hong" href="{pigcms{:U('Pay/check',array('order_id' => $order['order_id'], 'type'=>'food'))}">付款</a>
			                        		</if>
			                        		
			                        	<elseif condition="$order['pay_type'] eq 'offline' AND empty($order['third_id'])"/>
			                        		<a class="liu_hong">线下未付</a>
			                        	<elseif condition="$order['paid'] eq 2"/>
			                        		<a class="liu_hong" href="{pigcms{:U('Pay/check',array('order_id' => $order['order_id'], 'type'=>'food'))}">付款</a>
			                        	<elseif condition="$order['paid'] eq 1 AND empty($order['status'])"/>
			                        		<a class="liu_lv">未消费</a>
			                        	<elseif condition="$order['status'] eq '1'"/>
			                        		<a class="liu_huang" href="{pigcms{:U('My/meal_feedback',array('order_id'=>$order['order_id']))}">去评价</a>
			                        	<elseif condition="$order['paid'] eq 1 AND $order['status'] gt 0"/>
			                        		<a class="liu_huang">已完成</a>
			                        	</if>
			                        </weak>
			                    </div>
			                </dd>
