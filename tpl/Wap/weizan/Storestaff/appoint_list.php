...
<body>
	<dl class="list"  style="border-top:none;margin-top:0rem;">
		<dd id="filtercon">
			<div class="find_div">
				<form name="find_form" method="get" action="{pigcms{:U('Storestaff/appoint_list')}">
					...
					<div class="find_type_div">
						<select name="ft" id="find_type" onchange="toJmupURl(this.value);">							
							...
							<option value="xm" <php>if($ftype=='xm') echo "selected='selected'";</php>>昵称</option>
							<option value="dh" <php>if($ftype=='dh') echo "selected='selected'";</php>>电话</option>									
						</select>
					</div>
				...
				<volist name="order_list" id="vo">
					<dd class="dealcard dd-padding" onclick="Jumpto({pigcms{$vo['order_id']})">
						<ul class="dealcard-block-right">
							<li class="btm_li"><span class="dth">预约名称：</span>
							<span class="ttd">{pigcms{$vo.appoint_name}</span></li>
                            <li class="btm_li"><span class="dth">预约客户：</span>
                            <span class="ttd">{pigcms{$vo.nickname}({pigcms{$vo.phone})</span></li>
							<li class="btm_li"><span class="dth">预约费用：</span><span class="td"><if condition="$vo['dingjin_money'] gt 0">订金: ￥{pigcms{$vo.dingjin_money}<else/>无需订金</if>&nbsp;&nbsp;全价: ￥{pigcms{$vo.appoint_price}</span></li>
							<li><span class="dth">订单状态：</span>
							<if condition="$vo['service_status'] eq 3">
			                  <span style="color: red">已取消并退款</span> 
							<elseif condition="$vo['paid'] eq 0" />
                            <span style="color: red">未支付</span>
							<else/>
							<span style="color: green">已支付</span></if> &nbsp;&nbsp; <if condition="$vo['service_status'] eq 0"><span style="color: red">未服务</span><elseif condition="$vo['service_status'] eq 1" /><span style="color: green">已服务</span></if>
							</if>
							</li>
						</ul>
					</dd>
...
<script type="text/javascript">
    function Jumpto(oid){
		window.location.href="{pigcms{:U('Storestaff/appoint_edit')}&order_id="+oid;
	}
	function toJmupURl(vv){
	   vv=$.trim(vv);
	   if(vv=='st'){
	     window.location.href="{pigcms{:U('Storestaff/appoint_list')}&st=1";
	   }
	}
