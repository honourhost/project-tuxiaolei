...
<html lang="zh-CN">
	<head>
		...
		<script type="text/javascript" src="{pigcms{$static_path}js/meallist.js?rand=<?php echo rand(1000,50000); ?>" charset="utf-8"></script>
	</head>
	<body>
					...
					{{# for(var i = 0, len = d.meal_list.length; i < len; i++){ }}
						<dd class="link-url" data-url="{{ d.meal_list[i].wap_url }}">
							...
							<div class="dealcard-block-right">									
								<div class="brand">{{ d.meal_list[i].name }}</div>					
								<div class="title">{{ d.meal_list[i].store_notice }}</div>
								<div class="price">
									{{# if(d.meal_list[i].range){ }}<span class="location-right">{{ d.meal_list[i].range }}</span>{{# } }}
									{{# if(d.meal_list[i].mean_money){ }}<strong>{{ d.meal_list[i].mean_money }}</strong>
									<span class="strong-color">元(人均)</span>{{# } }}&nbsp;<span class="line-right">已售{{ d.meal_list[i].sale_count }}</span>
								</div>
							</div>
						</dd>
					{{# } }}
