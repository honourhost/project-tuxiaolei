	...
	if(navigator.geolocation){
		navigator.geolocation.getCurrentPosition(function(position){
			...
			if(options.reportLocation){
				$.post(window.location.pathname+'?c=Userlonglat&a=report',{userLong:userLong,userLat:userLat},function(data){
					var result= $.parseJSON(data);
					if(!(result.errCode)){
						//如果不出错则将城市信息改掉
						$("div .areaname").html(result.errMsg);
					}
