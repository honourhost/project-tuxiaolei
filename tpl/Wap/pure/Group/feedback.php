...
<html lang="zh-CN">
	<head>
		...
		<script type="text/javascript" src="{pigcms{$static_path}js/feedback.js?rand=<?php echo rand(1000,50000); ?>" charset="utf-8"></script>
	...
	<body>
								...
								<dd>
									...
									<div class="replyCon">
										...
										<if condition="$vo['pics']">
											<ul class="imgList" data-pics="<volist name="vo['pics']" id="voo">{pigcms{$voo.image}<if condition="count($vo['pics']) gt $i">,</if></volist>">
												<volist name="vo['pics']" id="voo">
													<li><img src="{pigcms{$voo.image}"/></li>
												</volist>
											</ul>
										</if>
					...
					{{# for(var i = 0, len = d.list.length; i < len; i++){ }}
						<dd>
							...
							<div class="replyCon">
								...
								{{# if(d.list[i].pics){ }}
									<ul class="imgList" data-pics="{{# for(var j = 0, jlen = d.list[i].pics.length; j < jlen; j++){ }}{{ d.list[i].pics[j].image }},{{# } }}">
										{{# for(var j = 0, jlen = d.list[i].pics.length; j < jlen; j++){ }}
											<li><img src="{{ d.list[i].pics[j].image }}"/></li>
										{{# } }}
									</ul>
								{{# } }}
								{{# if(d.list[i].merchant_reply_content){ }}
								<div class="textDiv">
									<div class="text">商家回复：{{ d.list[i].merchant_reply_content }}</div>
								</div>
								{{# } }}
							</div>
						</dd>
		...
		<script type="text/javascript">
			var shareData={
						imgUrl: "", 
						link: "{pigcms{$config.site_url}{pigcms{:U('Group/feedback', array('group_id' => $now_group['group_id']))}",
						title: "【{pigcms{$now_group.merchant_name}】评价列表",
						desc: "【{pigcms{$now_group.merchant_name}】评价列表"
			};
		</script>
