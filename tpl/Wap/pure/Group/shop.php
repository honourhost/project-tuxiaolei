...
<html lang="zh-CN">
	<head>
		...
		<script type="text/javascript" src="{pigcms{$static_path}js/shop.js?rand=<?php echo rand(1000,50000); ?>" charset="utf-8"></script>
	</head>
	<body>
		<div id="container">
			<div id="scroller">
				<section class="shopDetail">
					<div class="imgInfo">
						<div class="dealcard-img imgbox" data-pics="<volist name="now_store['all_pic']" id="vo">{pigcms{$vo}<if condition="count($now_store['all_pic']) gt $i">,</if></volist>">
							<img src="{pigcms{$person_image}" alt="{pigcms{$now_store.name}">
						</div>
						<div class="dealcard-block-right">
							...
							<a href="{pigcms{:U('Index/index',array('token'=>$now_store['mer_id']))}" class="btn">进入农场主页</a>
						</div>
					</div>
				...
				<if condition="$index_sort_group_list">
		...
		<script type="text/javascript">
			var shareData={
						...
						link: "{pigcms{$config.site_url}{pigcms{:U('Group/shop', array('store_id' => $now_store['store_id']))}",
						title: "{pigcms{$now_store.name}-小农丁",
						desc: "{pigcms{$now_store.area_name}{pigcms{$now_store.adress}"
			};
		</script>
