...
<html lang="zh-CN">
	<head>
		...
		<script type="text/javascript" src="{pigcms{$static_path}js/groupsearch.js?rand=<?php echo rand(1000,50000); ?>" charset="utf-8"></script>
	</head>
	<body>
		...
		<section class="searchBox pageSliderHide">
			<ul>	
				<li class="dropdown-toggle active"><span class="nav-head-name">{pigcms{$config.group_alias_name}</span></li>
						...
						<volist name="group_list" id="vo">
							<dd class="link-url" data-url="{pigcms{$vo.url}">
								...
								<div class="dealcard-block-right">									
									...
									<div class="price">
										<strong>{pigcms{$vo.price}</strong><span class="strong-color">元</span><if condition="$vo['wx_cheap']"><span class="tag">微信再减{pigcms{$vo.wx_cheap}元</span><else/><del>{pigcms{$vo.old_price}</del></if> <span class="line-right">已售<?php echo $vo['sale_count']+$vo["virtual_num"];?></span>
									</div>
								</div>
							</dd>
						</volist>
