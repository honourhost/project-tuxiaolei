...
<html lang="zh-CN">
	<head>
		...
		<script type="text/javascript" src="{pigcms{$static_path}js/mealsearch.js?rand=<?php echo rand(1000,50000); ?>" charset="utf-8"></script>
	</head>
	<body>
		...
		<section class="searchBox pageSliderHide">
			<ul>	
				<li class="dropdown-toggle link-url" data-url="{pigcms{:U('Search/group',array('w'=>urlencode($keywords)))}" data-url-type="openLeftWindow"><span class="nav-head-name">{pigcms{$config.group_alias_name}</span></li>
				<li class="dropdown-toggle active"><span class="nav-head-name">{pigcms{$config.meal_alias_name}</span></li>
			</ul>
		</section>
		<div id="container">
			<div id="scroller">
