					...
					<div class="order-footer-left">
						<p style="color: #fff;">
							<a class="phone-group-xq-footer-add" style="cursor:pointer;" onclick="addCollect({pigcms{$now_group.group_id});">
								...
								<else/>
								<img class="img-ysc2" src="{pigcms{$static_path}images/xnd_img/iconfont-guanzhu.png" height="20px" style="margin-top: 7px;" />
								<span id="collect">收藏</span>
								</if>
							</a>
							<a class="phone-group-xq-footer-add" style="cursor:pointer;" href="{pigcms{:U('Merchant/detail',array('mer_id'=>$now_group['mer_id']))}">
								<img src="{pigcms{$static_path}images/xnd_img/iconfont-dianpu.png" height="22px" />
								<span>店铺</span> 
							</a>
							<a class="phone-group-xq-footer-add" style="cursor:pointer;">
		...
		<script type="text/javascript">
			function addCollect(id){
			 $.post("{pigcms{:U('Group/addGroupCollect')}",{id:id},function(result){
    			...
    			if($("#collect").html()=="收藏"){
    				$("#collect").html("已收藏");
    				$("img-ysc").attr("src","http://www.xiaonongding.com/tpl/Static/weizan/images/xnd_img/iconfont-shoucangyishoucang.png");
    			}else{
    				$("#collect").html("收藏");
    				$("img-ysc2").attr("src","http://www.xiaonongding.com/tpl/Static/weizan/images/xnd_img/iconfont-iconfont-guanzhu.png");
