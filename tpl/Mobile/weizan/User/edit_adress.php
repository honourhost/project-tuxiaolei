...
<body id="index" data-com="pagecommon">
        ...
        <form id="form" method="post" action="{pigcms{:U('User/edit_adress')}">
		    ...
		    <div class="btn-wrapper">
				...
				<button type="submit" class="btn btn-block btn-larger" style="background: #00b081;"><if condition="$now_adress">保存<else/>添加</if></button>
		    </div>
		</form>
		...
		<script>
					...
					$.each(form_input,function(i,item){
						...
						if(i+1 == form_input.size()){
							...
							$.post($('#form').attr('action'),$('#form').serialize(),function(result){
								layer.closeAll();
								if(result.status == 1){
									window.location.href="{pigcms{:U('User/adress',$params)}";
								}else{
			...
			function show_city(id){
				$.post("{pigcms{:U('User/select_area')}",{pid:id},function(result){
