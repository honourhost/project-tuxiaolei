...
<div class="mainbox">
    ...
    <table class="search_table" width="100%">
        <tr>
            <td>
                <form action="{pigcms{:U('Group/productdaishenhe')}" method="get">
                    <input type="hidden" name="c" value="Group"/>
                    <input type="hidden" name="a" value="productdaishenhe"/>
                ...
                <if condition="is_array($group_list)">
                    <volist name="group_list" id="vo">
                        <tr>
                            ...
                            <td class="textcenter"><a href="{pigcms{:U('Group/order_list',array('group_id'=>$vo['group_id']))}">订单列表</a> | <a href="{pigcms{:U('Group/reply_list',array('group_id'=>$vo['group_id']))}">评论列表</a> | <a href="{pigcms{:U('Merchant/merchant_login',array('mer_id'=>$vo['mer_id'],'group_id'=>$vo['group_id']))}">编辑</a> | <if condition="$vo['group_lock'] eq 0"> <a href="{pigcms{:U('Group/lockGroup',array('group_id'=>$vo['group_id']))}">锁定</a><else/>已锁定</if> | <a href="javascript:void(0);" class="cancle_row" parameter="group_id={pigcms{$vo['group_id']}" url="{pigcms{:U('Group/group_cancle')}">删除</a></td>
                        </tr>
                    </volist>
