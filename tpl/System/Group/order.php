<include file="Public:header"/>
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<if condition="$_GET['searchtype'] eq 'unsend'">
					<a href="{pigcms{:U('Group/order')}">普通特卖订单列表</a>
					<a href="{pigcms{:U('Group/order',array('searchtype'=>'unsend'))}"  class="on">商家未处理普通订单列表</a>
					<a href="{pigcms{:U('Group/order',array('searchtype'=>'groupbuy'))}">拼团订单列表</a>
						<a href="{pigcms{:U('Group/order',array('searchtype'=>'groupbuyunsend'))}">商家未处理拼团订单列表</a>
					<elseif condition="$_GET['searchtype'] eq 'groupbuy'"/>
					<a href="{pigcms{:U('Group/order')}">普通特卖订单列表</a>
					<a href="{pigcms{:U('Group/order',array('searchtype'=>'unsend'))}">商家未处理普通订单列表</a>
					<a href="{pigcms{:U('Group/order',array('searchtype'=>'groupbuy'))}"  class="on">拼团订单列表</a>
						<a href="{pigcms{:U('Group/order',array('searchtype'=>'groupbuyunsend'))}">商家未处理拼团订单列表</a>
						<elseif condition="$_GET['searchtype'] eq 'groupbuyunsend'"/>
						<a href="{pigcms{:U('Group/order')}">普通特卖订单列表</a>
						<a href="{pigcms{:U('Group/order',array('searchtype'=>'unsend'))}">商家未处理普通订单列表</a>
						<a href="{pigcms{:U('Group/order',array('searchtype'=>'groupbuy'))}"  >拼团订单列表</a>
						<a href="{pigcms{:U('Group/order',array('searchtype'=>'groupbuyunsend'))}" class="on">商家未处理拼团订单列表</a>
					<else/>
					<a href="{pigcms{:U('Group/order')}" class="on">普通特卖订单列表</a>
					<a href="{pigcms{:U('Group/order',array('searchtype'=>'unsend'))}" >商家未处理普通订单列表</a>
					<a href="{pigcms{:U('Group/order',array('searchtype'=>'groupbuy'))}">拼团订单列表</a>
						<a href="{pigcms{:U('Group/order',array('searchtype'=>'groupbuyunsend'))}">商家未处理拼团订单列表</a>
							...
							<if condition="is_array($order_list)">
								<volist name="order_list" id="vo">
									<tr>
										...
										<td><a  target="_blank" href="{pigcms{$config['config_site_url']}/group/{pigcms{$vo.group_id}.html">{pigcms{$config.group_alias_name}ID：{pigcms{$vo.group_id}　{pigcms{$config.group_alias_name}价：￥{pigcms{$vo.g_price}<br/>{pigcms{$config.group_alias_name}名称：{pigcms{$vo.s_name}</a></td>
