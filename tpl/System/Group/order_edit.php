<include file="Public:header"/>
	...
	<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
		...
		<tr>
		<tr>
			<th width="15%">市场负责人</th>
			<td width="35%">{pigcms{$fuzeren.market_leader}</td>
			<th width="15%">品牌编辑人</th>
			<td width="35%">{pigcms{$fuzeren.brands_editor}</td>
		</tr>
		</tr>
			...
			<tr>
                ...
                <tr>
                    ...
                    <td width="85%" colspan="3">
                     ...
                     <?php if($now_order['balance_pay']>0&&$now_order['payment_money']==0){?>
                     <tr>
                         <th width="15%">农丁鲜生退货操作</th>
                         <td width="85%" colspan="3">
                             <button id="refundvip_id_btn">退货</button></td>
                     </tr>
                     <?php }?>
    ...
    $(function(){
        ...
        $('#refundvip_id_btn').click(function(){
            refundvip_order();
        });


        function refundvip_order() {
            $('#refundvip_id_btn').html('提交中...').prop('disabled',true);
            $.post("{pigcms{:U('Group/group_refundvip',array('order_id'=>$now_order['order_id']))}",{},function(result){
                if(result.status == 1){
                    window.location.href = window.location.href;
                }
                $('#refundvip_id_btn').html('退货').prop('disabled',false);
                alert(result.info);
            });
        ...
        function express_post(){
            ...
            $.post("{pigcms{:U('Group/group_express',array('order_id'=>$now_order['order_id'],'mer_id'=>$now_order['mer_id'],'store_id'=>$now_order['store_id']))}",{express_type:$('#express_type').val(),express_id:$('#express_id').val()},function(result){
                if(result.status == 1){
                    fahuo=0;
                    window.location.href = window.location.href;
                }
