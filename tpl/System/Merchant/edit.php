		...
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			...
			<tr>
				...
				<td><select name="marketer_id">
					<volist name="marketers" id="marketer" key="index">
						<if condition="$marketer.id eq $merchant.marketer_id ">
							<option value="{pigcms{$marketer.id}" selected="selected">{pigcms{$marketer.name}</option>
							<else />
							<option value="{pigcms{$marketer.id}" >{pigcms{$marketer.name}</option>
						</if>
