                ...
                <if condition="is_array($cash_list)">
                    <volist name="cash_list" id="vo">
                        <tr>
                            ...
                            <td><if condition="$vo['cash_status']==0">等待审核<elseif condition="$vo['cash_status']==2"/>已驳回<else/>已经完成提现</if></td>
                            <td><a href="javascript:void(0);" class="delete_row" parameter="id={pigcms{$vo.id}" url="{pigcms{:U('Cash/finish')}">完成提现</a> | <a href="javascript:void(0);"  onclick="window.top.artiframe('{pigcms{:U('Cash/reject',array('id'=>$vo['id']))}','编辑驳回信息',480,240,true,false,false,editbtn,'edit',true);"">驳回</a></td>
