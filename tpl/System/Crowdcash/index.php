            ...
            <table width="100%" cellspacing="0">
                ...
                <if condition="is_array($cash_list)">
                    <volist name="cash_list" id="vo">
                        <tr>
                            <td><if condition="$vo['status']==0"><h6 style="color: red;">{pigcms{$vo.id}</h6><else /> {pigcms{$vo.id}</if></td>
                            ...
                            <td><a href="javascript:void(0);" class="delete_row" parameter="id={pigcms{$vo.id}" url="{pigcms{:U('Usercash/finish')}">完成提现</a> | <a href="javascript:void(0);"  onclick="window.top.artiframe('{pigcms{:U('Crowdcash/reject',array('id'=>$vo['id']))}','编辑驳回信息',480,240,true,false,false,editbtn,'edit',true);"">驳回</a></td>
