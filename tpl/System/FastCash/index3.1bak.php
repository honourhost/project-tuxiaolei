                ...
                <if condition="is_array($cash_list)">
                    <volist name="cash_list" id="vo">
                        <tr>
                            ...
                            <td>{pigcms{$vo.marketing_director}</td>
                            <td>{pigcms{$vo.merchant_real_telephone}</td>
                            <td>{pigcms{$vo.bank_name}</td>
                            <td>{pigcms{$vo.bank_card}</td>
                            <td>{pigcms{$vo.cash_num}</td>
                            <td><if condition="$vo['cash_status']==0"><h6 style="color: red;">等待审核</h6><elseif condition="$vo['cash_status']==2"/>已驳回<else/>已经完成提现</if></td>
                            <td><a href="javascript:void(0);" class="delete_row" parameter="id={pigcms{$vo.id}" url="{pigcms{:U('FastCash/finish')}">完成提现</a> | <a href="javascript:void(0);"  onclick="window.top.artiframe('{pigcms{:U('FastCash/reject',array('id'=>$vo['id']))}','编辑驳回信息',480,240,true,false,false,editbtn,'edit',true);"">驳回</a></td>
