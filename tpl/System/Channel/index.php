                ...
                <if condition="is_array($channels)">
                    <volist name="channels" id="vo">
                        <tr>
                            ...
                            <td class="textcenter"><a href="{pigcms{$config.site_url}/wap.php?g=Wap&c=Topic&a=index&topic={pigcms{$vo.url}" target="blank;">点击预览</a> | <a href="{pigcms{:U('Channel/banner_list',array('id'=>$vo['id']))}">频道下轮播图</a> | <a href="{pigcms{:U('Channel/personList',array('id'=>$vo['id']))}">分类下股东列表</a> | <a href="{pigcms{:U('Channel/catList',array('id'=>$vo['id']))}">频道分类管理</a> | <a href="{pigcms{:U('Channel/edit',array('id'=>$vo['id']))}">编辑</a> | <a href="javascript:void(0);" class="delete_row" parameter="id={pigcms{$vo.id}" url="{pigcms{:U('Channel/del')}">删除</a></td>
                        </tr>
                    </volist>
