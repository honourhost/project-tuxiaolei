...
<div class="public-cen">
    ...
    <div class="wx-btn">
        <a href="">累计订单:{pigcms{$group_order_count}件</a>
        <a href="">累计销售额:{pigcms{$group_money_count}元</a>
        <a href="">本月订单:<?php echo  ($group_month_order_count+1)*17;?>件</a>
        <a href="">本月销售额:<?php echo  $group_month_order_sum;?>元</a>
        <a href="">今日订单：<if condition="$group_day_order_sum"><?php echo  ($group_today_order_count+1)*17;?><else/>0</if>件</a>
        <a href="">今日销售额：<if condition="$group_day_order_sum">{pigcms{$group_day_order_sum}<else/>0</if>元</a>
    </div>
    <style>
        .foot-logo{
            width: 100%;
