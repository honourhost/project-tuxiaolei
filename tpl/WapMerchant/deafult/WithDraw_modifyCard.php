...
<head>
    ...
    <script type="text/javascript">
        ...
        function sendVerify() {
            $.post("<?php echo U("Api/Index/sendNewVerifyCode");?>", {mobile: <?php echo  $phone;?>}, function (result) {
        ...
        function verifycode(yanzheng){
            $.post("<?php echo U("Api/Index/verifyCode");?>", {mobile: <?php echo  $phone;?>,verify_code:yanzheng}, function (result) {
                var jsondata2 = eval("(" + result + ")");
                console.log(result);
                console.log(typeof(jsondata2.status));
                if(jsondata2.status==1){
                    $("form").submit();
                }else {
                    alert(jsondata2.errorMsg);
                }
            ...
            <if condition="$cardinfo">
                <ul>
                    ...
                    <li>
                        ...
                        <input type="text" name="merchant_real_name" placeholder="银行卡姓名" onfocus="this.placeholder=''" onblur="this.placeholder='银行卡姓名'" value="{pigcms{$cardinfo.user_name}"/>

                    </li>

                    <li>
                        <label><img src="{pigcms{$static_path}images/kaihuihang.png"></label>
                        <input type="text" name="bank_name" placeholder="开户行(如:中国农业银行青岛崂山秦岭路支行)" onfocus="this.placeholder=''" onblur="this.placeholder='开户行(如：中国农业银行青岛崂山秦岭路支行)'"  value="{pigcms{$cardinfo.bank_name}" />
    ...
    $(document).ready(function(){
        ...
        $(".tx-btn").click(function () {
            ...
            if(yanzhengma==null||yanzhengma==undefined||yanzhengma==''){
                alert("请输入验证码");
                return;
            }else  if(yanzhengma.length!==6||!parseInt(yanzhengma)){
                alert("请输入正确的验证码");
                return;
            }else {
