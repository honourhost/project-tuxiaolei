...
function checkSubmit(){
	...
	if($("input[name='phone']")[0] && $("input[name='phone']").val() != '' ){
		...
		if(type == 0){
			...
			var result=str.match(/1[3|5|7|8|][0-9]{9}/);
			if( result == null || str.length != 11){
				message += '联系电话<br>';
			}
		}
		postdata.phone=$.trim(str);
		postdata.account=$.trim(str);
...
function checkLogin(){
	...
	var referer=$.trim($("input[name='referurl']").val());
	...
	$.ajax({
	   ...
	   data: "account="+lgaccount+"&pwd="+$("input[name='pwd']").val()+"&referer="+referer,
	   success: function(msg){
		var	data=$.parseJSON(msg);
			if(data.error==0){
...
function staff_add(){
	$.ajax({
	   ...
	   data: "tel="+$("input[name='staff_phone']").val()+"&store_id="+$("input[name='store_id']").val()+"&username="+$("input[name='staff_username']").val()+"&password="+$("input[name='staff_password']").val()+"&name="+$("input[name='staff_name']").val(),
	   success: function(msg){
		var	data=$.parseJSON(msg);
			if(data.error==0){
				alert_open('添加成功');
...
function staff_edit(){
	$.ajax({
	   ...
	   data: "tel="+$("input[name='staff_phone']").val()+"&store_id="+$("input[name='store_id']").val()+"&id="+$("input[name='staff_id']").val()+"&password="+$("input[name='staff_password']").val()+"&name="+$("input[name='staff_name']").val(),
	  success: function(msg){
		var	data=$.parseJSON(msg);
			if(data.error==0){
				alert_open('修改成功');
