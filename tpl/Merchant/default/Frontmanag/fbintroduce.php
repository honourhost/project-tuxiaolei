							...
							<div id="basicinfo" class="tab-pane active">
							  <if condition="!$notitle">
								<div class="form-group">
									<label class="col-sm-1"><label for="title">模板</label></label>
									<if condition="$modules">
									<select name="module_id" onchange="giveTitle();" id="module_select">
										<volist name="modules" id="module">
											<!-- <option value="{pigcms{$module.id}">{pigcms{$module.module_name}</option> -->
											<if condition="$fbintroduce['module_id']==$module['id']">
											<option value="{pigcms{$module.id}" selected>{pigcms{$module.module_name}</option>
											<else/>
												<option value="{pigcms{$module.id}">{pigcms{$module.module_name}</option>
											</if>
											</volist>
								  ...
								  <div class="form-group">
									  <label class="col-sm-1">背景颜色</label>
									  <label><input  value="1" name="color_flag" type="radio" <if condition="$fbintroduce['color_flag'] eq 1"> checked="checked" </if> />&nbsp;&nbsp;白色</label>
									  &nbsp;&nbsp;&nbsp;
									  <label><input  value="0" name="color_flag" type="radio" <if condition="$fbintroduce['color_flag'] neq 1"> checked="checked" </if> />&nbsp;&nbsp;灰色</label>
...
<script type="text/javascript">
	...
	function giveTitle(){
		$("#title").attr("value",$('#module_select option:selected').text());
	}
