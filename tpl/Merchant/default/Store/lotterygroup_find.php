            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="form-group" style="border:1px solid #c5d0dc;padding:10px;">
                        ...
                        <button class="btn btn-success" id="exportunsend" style="float:right;margin-top: -40px;margin-left: 20px;" >导出未发货</button>
                        <button class="btn btn-success" id="exportall" style="float:right;margin-top: -40px;margin-left: 10px;">导出全部</button>
                    </div>
...
<script type="text/javascript">
    ...
    $('#find-form').submit(function(){
        ...
        $('#exportall').attr("onclick","javascript:window.location.href='http://www.xiaonongding.com/merchant_order.php?merid={pigcms{$merid}"+"&find_type="+post_type+"&find_value="+post_value+"&storeid={pigcms{$storeid}'");
        $('#exportunsend').attr("onclick","javascript:window.location.href='http://www.xiaonongding.com/merchant_order.php?merid={pigcms{$merid}"+"&find_type="+post_type+"&find_value="+post_value+"&storeid={pigcms{$storeid}&unsend=1'");
        $('#find_submit').removeClass('btn-success').addClass('btn-error').prop('disabled',true).html('请求中...');
        $('#order_html').empty();
        $('#order_list').hide();
        $.post("{pigcms{:U('Store/lotterygroup_find')}",{find_type:post_type,find_value:post_value},function(result){
