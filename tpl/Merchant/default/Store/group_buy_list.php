    ...
    <div class="breadcrumbs" id="breadcrumbs">
        <ul class="breadcrumb">
            <li>
                ...
                拼团{pigcms{$config.group_alias_name}订单列表
            </li>
        </ul>
    </div>
    <!-- 内容头部 -->
            ...
            <div class="row">
                <div class="col-xs-12">
                    <button class="btn btn-success" onclick="CreateShop()">查找订单</button>&nbsp;
                            <a class="btn btn-success" href="{pigcms{:U('Store/group_buy_list')}">所有订单</a>&nbsp;
                    <a class="btn btn-success" href="{pigcms{:U('Store/group_buy_list',array('unsend'=>1))}">未发货订单</a>&nbsp;
                 <!--   <a class="btn btn-success" href="{pigcms{:U('Store/group_list',array('special'=>1))}">支付与下单时间相差超过一天的未处理订单</a>&nbsp;-->
              
                    <a class="btn btn-success" href="{pigcms{:U('Store/group_buy_list',array('hassend'=>1))}">已发货订单</a>&nbsp;
                  <!--  <a class="btn btn-success" href="{pigcms{:U('Store/group_list',array('unpaid'=>1))}"></a>-->
                <!--    <p>&nbsp;</p>此页面只列出已归属到此店铺的订单。若想验证新订单或查找订单，请点击上面按钮，另外需要注意的是拼团订单是单独显示的，区别于所有前面的订单。-->
                    <div id="shopList" class="grid-view">
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <volist name="order_list" id="vo">
                                ...
                                <td width="200"><a href="javascript:void(0);" target="_blank">产品id:{pigcms{$vo.group_id}<br/>{pigcms{$vo.order_name}&nbsp;<br/>团长:<?php echo getUserName($vo['uid']);?></a></td>
                                <td width="150">
                                    数量：{pigcms{$vo.num}<br/>
                                ...
                                <td width="200">
                                    <if condition="$vo['status'] == 3">
                                        <if condition="$vo['paid']">
                                            <font color="red">已支付，已取消</font>
                                            <else/>
                                            <font color="red">未支付，已取消</font>
                                        </if>
                                        ...
                                        <if condition="$vo['third_id'] eq '0' AND $vo['pay_type'] eq 'offline'">
                                            ...
                                            <php>}else{</php>
                                            <php>if($vo['user_confirm'] != 1){</php>
                                            <font color="green">已发货 未确认 </font>
                                            <php>}else{</php>
                                ...
                                <td class="button-column" width="40">
                                    <a title="操作订单" class="green handle_btn" style="padding-right:8px;" href="{pigcms{:U('Store/group_edit',array('order_id'=>$vo['order_id'],'sun_id'=>$vo['sun_id']))}">
						...
						<else/>
							 <div ><h6 style="text-align:center;color:red;">没有查找到相关订单</h6></div>
						</if>
...
<script>
    ...
    function CreateShop(){
        window.location.href = "{pigcms{:U('Store/pingroup_find')}";
    }
</script>
