							...
							<tbody>
								<if condition="$store">
										<tr>
											...
											<td>{pigcms{$store.sort}</td>
											<td>{pigcms{$store.name}</td>
											<td>{pigcms{$store.phone}</td>
											<td>{pigcms{$store.area_name} - {pigcms{$store.adress}</td>
											<td><if condition="$store['have_meal']">有<else/>无</if></td>
											<td><if condition="$store['have_group']">有<else/>无</if></td>
											<td>
												<switch name="store['status']">
											...
											<td>
												<a href="{pigcms{$config.site_url}/index.php?g=Index&c=Recognition&a=see_qrcode&type=meal&id={pigcms{$store['store_id']}&img=1" class="see_qrcode">查看二维码</a>
											</td>
											<td>
												<if condition="$store['status'] neq 2">
													<a class="label label-sm label-info" title="店员管理" href="{pigcms{:U('Config/staff',array('store_id'=>$store['store_id']))}">店员管理</a>
												<else/>
													审核中
												</if>
											</td>
											<td class="button-column" nowrap="nowrap">
												<a title="修改" class="green" style="padding-right:8px;" href="{pigcms{:U('Config/store_edit',array('id'=>$store['store_id']))}">
													<i class="ace-icon fa fa-pencil bigger-130"></i>
												</a>
												<a title="删除" class="red" style="padding-right:8px;" href="{pigcms{:U('Config/store_del',array('id'=>$store['store_id']))}">
