						...
						<div class="tab-content">
							<div id="basicinfo" class="tab-pane active">
								<div class="form-group">
									...
									<select name="is_group_buy" class="col-sm-2">
										<if condition="$now_group['is_group_buy']">
								...
								<div class="form-group">
									...
									<input class="col-sm-3" maxlength="100" name="need_num" type="text" value="{pigcms{$now_group.need_num}" /><span class="form_tips">填写需要多少人参加该拼团才是成功，如果非拼团商品可不填</span>
								</div>
									<div class="form-group">
								...
								<div class="form-group">
									...
									<input class="col-sm-3" maxlength="100" name="max_group_num" type="text" value="{pigcms{$now_group.max_group_num}" /><span class="form_tips">填写该抽奖团最多开团次数，如果非拼团商品可不填</span>
								</div>
								...
								<div class="form-group">
									...
									<select name="is_first_free" class="col-sm-2">
										<if condition="$now_group['is_first_free']">
											<option value="0">否</option>
											<option value="1" selected>是</option>
											<else/>
											<option value="0" selected>否</option>
								...
								<div class="form-group">
									...
									<input class="col-sm-1" maxlength="100" name="first_free_fee" type="text" value="{pigcms{$now_group.first_free_fee}" /><span class="form_tips">必填。最多支持1位小数</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1">关联特卖的id：</label>
									<input class="col-sm-3" maxlength="100" name="related_id" type="text" value="{pigcms{$now_group.related_id}" /><span class="form_tips">填写原价的特卖id，如果非拼团商品可不填</span>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<input class="col-sm-1" maxlength="30" name="wx_cheap" type="text" value="{pigcms{$now_group.wx_cheap|floatval=###}" /><span class="form_tips"><span style="color:red;">（优惠金额，非优惠后金额）</span>单位元，最多支持1位小数，不填则不显示微信优惠！实际购买价=（{pigcms{$config.group_alias_name}价-微信优惠）</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1">佣金：</label>
									<input class="col-sm-1" maxlength="30" name="commission" type="text" value="{pigcms{$now_group.commission|floatval=###}" /><span class="form_tips">可不填，则为0。最多支持2位小数</span>
								</div>
								    <div class="form-group">
                                    <label class="col-sm-1">第二件价格：</label>
                                    <input class="col-sm-1" maxlength="30" name="secondprice" type="text" value="{pigcms{$now_group.secondprice|floatval=###}" /><span class="form_tips">可不填，则为0,0代表原价。最多支持2位小数</span>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-1">是否需要补差价：</label>
                                    <select name="youcha" class="col-sm-2">
                                        <if condition="$now_group['youcha']">
                                            <option value="0">否</option>
                                            <option value="1" selected>是</option>
                                            <else/>
                                            <option value="0" selected>否</option>
							...
							<div id="txtstore" class="tab-pane">
								<div class="form-group">
									<volist name="store_list" id="vo">
										<div class="radio">
											<label>
												<input class="paycheck ace" type="checkbox" name="store[]" value="{pigcms{$vo.store_id}" id="store_{pigcms{$vo.store_id}" <if condition="in_array($vo['store_id'],$store_arr)">checked="checked"<else/>checked="checked"</if>/>
							...
							<div id="txtintro" class="tab-pane">
								...
								<else />
									...
									<div class="form-group" style="margin-top:10px;color:red;">此特卖已经有人下单并且支付了，该用户还未确认收货，暂时不可再更改它的分类及其对应内容！</div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div id="upload_pic_box">
                                        <ul id="upload_pic_ulapp">
                                            <?php if(!empty($now_group["pic_arr_app"][0]["title"])) { ?>
                                                <volist name="now_group['pic_arr_app']" id="vo">
                                                    <li class="upload_pic_liapp"><img src="{pigcms{$vo.url}"/><input type="hidden" name="picapp[]" value="{pigcms{$vo.title}"/><br/><a href="#" onclick="deleteappImage('{pigcms{$vo.title}',this);return false;">[ 删除 ]</a></li>
                                                </volist>
                                           <?php }?>
                                ...
                                <div class="form-group">
                                    ...
                                    <div id="upload_pic_box">
                                        <ul id="upload_pic_ulappthumb">
                                            <?php if(!empty($now_group["pic_arr_appthumb"][0]["title"])) { ?>
                                                <volist name="now_group['pic_arr_appthumb']" id="vo">
                                                    <li class="upload_pic_liappthumb"><img src="{pigcms{$vo.url}"/><input type="hidden" name="picthumb[]" value="{pigcms{$vo.title}"/><br/><a href="#" onclick="deleteappthumbImage('{pigcms{$vo.title}',this);return false;">[ 删除 ]</a></li>
                                                </volist>
                                            <?php }?>
                                        </ul>
                                    </div>
        ...
        $.ajax({url:"{pigcms{:U('Group/ajax_get_tags')}"}).done(function (res) {
            ...
            $.each(dat,function (index, re) {
               // alert(re.id);
                ...
                chtml +="<label style=\"float:left;padding:0px\"><input type=\"checkbox\" name=\"tagname\" value=\""+re.id+"\"  /><span style=\"margin-left:10px\">"+re.tagname+"</span></label>";
                chtml += "</div>";
