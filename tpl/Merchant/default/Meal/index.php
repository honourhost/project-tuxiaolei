						...
						<table class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
									...
									<th class="button-column" width="140">商品添加</th>
								</tr>
							</thead>
							<tbody>
								<if condition="$store_list">
									<volist name="store_list" id="vo">
										<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
											...
											<td class="button-column">
												<a style="width: 60px;" class="label label-sm label-info" title="添加商品" href="{pigcms{:U('Meal/addSell',array('store_id'=>$vo['store_id']))}">添加商品</a>
											</td>
										</tr>
			<button class="btn btn-success" onclick="window.location.href='{pigcms{:U('Meal/meal_sort',array('store_id'=>$vo['store_id']))}'">商品分类</button> &nbsp;
			<button class="btn btn-success" onclick="window.location.href='{pigcms{:U('Meal/addSell',array('store_id'=>$vo['store_id']))}'">添加商品</button>
									</volist>
								<else/>
