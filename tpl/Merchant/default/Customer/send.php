										...
										<ul id="upload_pic_ul">
											<volist name="fans_list" id="vo">
												<li class="upload_pic_li">
													<label for="{pigcms{$vo.openid}" style="width:90px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">
														<img src="{pigcms{$vo.avatar}" style="width:70px;" title="{pigcms{$vo.nickname}"/><br/>
														<input type="checkbox" name="openid[]" value="{pigcms{$vo.openid}" id="{pigcms{$vo.openid}" class="user_select"/>{pigcms{$vo.nickname}
													</label>
												</li>
											</volist>
										</ul>
...
$(document).ready(function(){
	...
	$('#submit').click(function(){
		var openids = '', pre = '', source_id = $('#source_id').val();
		...
		$.post('{pigcms{:U('Customer/send')}', {'openids':openids, 'source_id':source_id}, function(data) {
			if (data.status) {
				alert(data.info);
