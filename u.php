...
$mer_id=$_GET['mer_id']? $_GET['mer_id'] : 629; //629;
...
if($_GET['mer_id']){
    ...
    $sql = 'SELECT * FROM `pigcms_group_order` WHERE `group_id` ='.$group_id.'  AND `paid` ='.$paid.' AND `order_id` >'.$begin_order_id.' AND `mer_id`='.$mer_id;

}elseif($_GET['begin_order_id'] && !$_GET['group_id'] ){
    $sql = 'SELECT * FROM `pigcms_group_order` WHERE   `paid` ='.$paid.' AND `order_id` >'.$begin_order_id.' AND `mer_id`='.$mer_id;
}

else{
	$sql = 'SELECT * FROM `pigcms_group_order` WHERE   `paid` ='.$paid.' AND `mer_id`='.$mer_id;
}
}
...
}elseif($_GET['group_id']&&!$_GET['begin_order_id']){
	 $sql = 'SELECT * FROM `pigcms_group_order` WHERE   `paid` ='.$paid.' AND `group_id` >'.$group_id;
}else{
	$sql = 'SELECT * FROM `pigcms_group_order` WHERE   `paid` ='.$paid;
}
}
