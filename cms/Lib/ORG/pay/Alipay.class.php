...
class Alipay{
	...
	public function web_pay(){
		...
		if($this->order_info['order_type']=="merchant_recharge"){
			$parameter = array(
			"service" => "create_direct_pay_by_user",
			"partner" => $this->pay_config['pay_alipay_pid'],
			"seller_email" => $this->pay_config['pay_alipay_name'],
			"payment_type"	=> '1',
			"notify_url"	=> C('config.site_url').'/source/web_merchant_alipay_notice.php',
			"return_url"	=> C('config.site_url').'/source/web_merchant_alipay_return.php',
		...
		$parameter = array(
			...
			"out_trade_no"	=> $this->order_info['order_type'].'_'.$this->order_info['order_id'],
			"subject"	=> '订单编号：'.$this->order_info['order_id'],
			"total_fee"	=> round($this->pay_money,2),
			"body"	=> '订单编号：'.$this->order_info['order_id'],
			"show_url"	=> C('config.site_url'),
			"anti_phishing_key"	=> '',
			"exter_invoke_ip"	=> '',
	...
	public function app_pay(){
		...
		$parameter = array(
				"name"	=> $this->order_info['order_name'] ,
				"order_sn"	=> $this->order_info['order_type'].'_'.$this->order_info['order_id'],
				"amount"	=> $this->pay_money,
				"notify_url"	=> C('config.site_url').'/source/app_alipay_notice.php',
		);
		return array('error'=>0,'form'=>$parameter);
	}
    ...
    public function  cardapp_pay(){
        ...
        $parameter = array(
            "name"	=> "xndshopcart_".$this->order_info['record_id']."_".$this->order_info['uid'] ,
            "order_sn"	=> 'cardpay_'.$this->order_info['record_id'],
            "amount"	=> $this->pay_money,
            "notify_url"	=> C('config.site_url').'/source/cardapp_alipay_notice.php',
        );
        return array('error'=>0,'form'=>$parameter);
    }
	...
	public function web_return(){
		...
		if($verify_result){
			...
			if($_POST['trade_status'] == 'TRADE_SUCCESS'){
				...
				//判断是否是商家订单
				if($order_id_arr[0]=="merchant"){
					$order_param['order_type']=$order_id_arr[0].'_'.$order_id_arr[1];
					$order_param['order_id'] = $order_id_arr[2];
				}else{
	...
	public function app_return_url(){
		...
		$alipay_config['input_charset']= 'utf-8';
		$alipay_config['transport']    = 'http';
		$alipay_config['path']    = __DIR__.'/Alipay/Rsa/rsa_public_key.pem';
		$alipayNotify = new AlipayNotify($alipay_config);
		$verify_result = $alipayNotify->verifyNotify();
		if($verify_result){
			//商户订单号
	...
	public function  sign($data){
        $path   = __DIR__.'/Alipay/Rsa/rsa_private_key.pem';
        import("@.ORG.pay.Alipay.alipay_sign");
        $alipaysign = new Alipaysign();
      return  $alipaysign->sign($data,$path);
    }
}
?>