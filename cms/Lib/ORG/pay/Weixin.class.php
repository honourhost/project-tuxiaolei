...
class Weixin{
	...
	public function pay(){
		...
		if($this->is_mobile==2){
			return $this->newfast_pay();
		}elseif ($this->is_mobile==3){
       return $this->crowd_pay();
        }elseif ($this->is_mobile==1){
            return $this->mobile_pay();
        }elseif($this->is_mobile==5){
            return $this->giftpay();
        }
		else{
			return $this->web_pay();
	...
	public function mobile_app_pay(){
		if(empty($this->pay_config['pay_weixin_appid']) || empty($this->pay_config['pay_weixin_mchid']) || empty($this->pay_config['pay_weixin_key']) || empty($this->pay_config['pay_weixin_appsecret'])){
			return array('error'=>1,'msg'=>'微信支付缺少配置信息！请联系管理员处理或选择其他支付方式。');
		}
		return $this->mobile_pay();
	}
	public function newfast_pay(){
		...
		//自定义订单号，此处仅作举例
		$unifiedOrder->setParameter("out_trade_no",'fastorder_'.$this->order_info['order_id']);//商户订单号 
		$unifiedOrder->setParameter("total_fee",floatval($this->pay_money*100));//总金额

		//如果是拼团订单则使用新的异步通知方法
		$unifiedOrder->setParameter("notify_url",C('config.site_url').'/source/new_fast_wap_weixin_notice.php');//通知地址 
	...
	public function  giftpay(){
        ...
        $unifiedOrder->setParameter("body",$this->order_info['order_name']);//商品描述
        //自定义订单号，此处仅作举例
        $unifiedOrder->setParameter("out_trade_no",'giftorder_'.$this->order_info['order_id']);//商户订单号
        $unifiedOrder->setParameter("total_fee",floatval($this->pay_money*100));//总金额

        //如果是拼团订单则使用新的异步通知方法
        $unifiedOrder->setParameter("notify_url",C('config.site_url').'/source/gift_wap_weixin_notice.php');//通知地址
	...
	public function web_pay(){
		...
		//如果是拼团订单则使用新的异步通知方法
		if(!empty($_GET['sun_id'])||!empty($this->order_info['sun_id'])){
		$unifiedOrder->setParameter("notify_url",C('config.site_url').'/source/new_web_weixin_notice.php');//通知地址 
		}else{
	...
	public function mobile_pay(){
		...
		//如果是拼团订单则使用新的异步通知方法
		if(!empty($_GET['sun_id'])||!empty($this->order_info['sun_id'])){
		$unifiedOrder->setParameter("notify_url",C('config.site_url').'/source/new_wap_weixin_notice.php');//通知地址 
		}else{
    ...
    public function crowd_pay(){
        ...
        $unifiedOrder = new UnifiedOrder_pub($this->pay_config['pay_weixin_appid'],$this->pay_config['pay_weixin_mchid'],$this->pay_config['pay_weixin_key'],$this->pay_config['pay_weixin_appsecret']);
        $unifiedOrder->setParameter("openid",$_SESSION['openid']);//用户微信唯一标识
        $unifiedOrder->setParameter("body",$this->order_info['name'].'_'.$this->order_info['num']);//商品描述
        //自定义订单号，此处仅作举例
        $unifiedOrder->setParameter("out_trade_no",'crowd_'.$this->order_info['corder_id']."_adam");//商户订单号
        $unifiedOrder->setParameter("total_fee",floatval($this->pay_money)*100);//总金额

            $unifiedOrder->setParameter("notify_url",C('config.site_url').'/source/crowd_weixin_notice.php');//通知地址
        ...
        //=========步骤3：使用jsapi调起支付============
        $jsApi->setPrepayId($prepay_result['prepay_id']);
      //  file_put_contents("crowdfunding4.txt",$jsApi->getParameters(),FILE_APPEND);
        return array('error'=>0,'weixin_param'=>$jsApi->getParameters());
    }
	...
	public function app_pay(){
		import('@.ORG.pay.Weixinnewpay.WxPayPubHelper');
		//使用jsapi接口
		$appApi = new App_pub(C("MOBILE_APPID"),C("MOBILE_MERCHANT_ID"),C("MOBILE_KEY"),C("MOBILE_APPSECRET"));
		//使用统一支付接口
		$unifiedOrder = new UnifiedOrder_pub(C("MOBILE_APPID"),C("MOBILE_MERCHANT_ID"),C("MOBILE_KEY"),C("MOBILE_APPSECRET"));
		//$unifiedOrder->setParameter("openid",$_SESSION['openid']);//用户微信唯一标识
		$unifiedOrder->setParameter("body",$this->order_info['order_name'].'_'.$this->order_info['order_num']);//商品描述
		//自定义订单号，此处仅作举例
		$unifiedOrder->setParameter("out_trade_no",$this->order_info['order_type'].'_'.$this->order_info['order_id']);//商户订单号 
		$unifiedOrder->setParameter("total_fee",floatval($this->pay_money*100));//总金额
		$unifiedOrder->setParameter("notify_url",C('config.site_url').'/source/app_weixin_notice.php');//通知地址 
		...
		//=========步骤3：使用jsapi调起支付============
		$appApi->setPrepayId($prepay_result['prepay_id']);
		
		return array('error'=>0,'weixin_param'=>$appApi->getParameters());
	}


	public function  cardapp_pay(){
        import('@.ORG.pay.Weixinnewpay.WxPayPubHelper');
        //使用jsapi接口
        $appApi = new App_pub(C("MOBILE_APPID"),C("MOBILE_MERCHANT_ID"),C("MOBILE_KEY"),C("MOBILE_APPSECRET"));
        //使用统一支付接口
        $unifiedOrder = new UnifiedOrder_pub(C("MOBILE_APPID"),C("MOBILE_MERCHANT_ID"),C("MOBILE_KEY"),C("MOBILE_APPSECRET"));
        //$unifiedOrder->setParameter("openid",$_SESSION['openid']);//用户微信唯一标识
        $unifiedOrder->setParameter("body","购物车订单".$this->order_info['adam_id'].'_'.$this->order_info['order_num']);//商品描述
        //自定义订单号，此处仅作举例
        $unifiedOrder->setParameter("out_trade_no","cardpay".'_'.$this->order_info['record_id']);//商户订单号
        $unifiedOrder->setParameter("total_fee",floatval($this->pay_money*100));//总金额
        $unifiedOrder->setParameter("notify_url",C('config.site_url').'/source/cardapp_weixin_notice.php');//通知地址
        $unifiedOrder->setParameter("trade_type","APP");//交易类型
        $unifiedOrder->setParameter("attach",'weixin');//附加数据
        $prepay_result = $unifiedOrder->getPrepayId();
        if($prepay_result['return_code'] == 'FAIL'){
            return array('error'=>1,'msg'=>'没有获取微信支付的预支付ID，请重新发起支付1！微信支付错误返回：'.$prepay_result['return_msg']);
        }
        if($prepay_result['err_code']){
            return array('error'=>1,'msg'=>'没有获取微信支付的预支付ID，请重新发起支付！<br/><br/>微信支付错误返回：'.$prepay_result['err_code_des']);
        }
    ...
    public function  cardwap_pay(){
        ...
        $unifiedOrder->setParameter("notify_url",C('config.site_url').'/source/cardwap_weixin_notice.php');//通知地址
	...
	public function app_return_url(){
		...
		import('@.ORG.pay.Weixinnewpay.WxPayPubHelper');
		//使用通用通知接口
		$notify = new Notify_pub(C("MOBILE_APPID"),C("MOBILE_MERCHANT_ID"),C("MOBILE_KEY"),C("MOBILE_APPSECRET"));
		//存储微信的回调
		$xml = $GLOBALS['HTTP_RAW_POST_DATA'];
		$notify->saveData($xml);
	...
	public function  cardapp_return_url(){
    ...
    public function  cardwap_return_url(){
        if(empty($this->pay_config['pay_weixin_appid']) || empty($this->pay_config['pay_weixin_mchid']) || empty($this->pay_config['pay_weixin_key']) || empty($this->pay_config['pay_weixin_appsecret'])){
            return array('error'=>1,'msg'=>'微信支付缺少配置信息！请联系管理员处理或选择其他支付方式。');
        }
     ...
     * 众筹订单查询
     */
    public function crowdquery_order(){
        if(empty($this->pay_config['pay_weixin_appid']) || empty($this->pay_config['pay_weixin_mchid']) || empty($this->pay_config['pay_weixin_key']) || empty($this->pay_config['pay_weixin_appsecret'])){
            return array('error'=>1,'msg'=>'微信支付缺少配置信息！请联系管理员处理或选择其他支付方式。');
        }
        import('@.ORG.pay.Weixinnewpay.WxPayPubHelper');
        ...
        $orderQuery = new OrderQuery_pub($this->pay_config['pay_weixin_appid'],$this->pay_config['pay_weixin_mchid'],$this->pay_config['pay_weixin_key'],$this->pay_config['pay_weixin_appsecret']);
        //设置必填参数
        $orderQuery->setParameter('out_trade_no','crowd'.'_'.$this->order_info['corder_id']."_adam");//商户订单号
        //获取订单查询结果
        $orderQueryResult = $orderQuery->getResult();
        if($orderQueryResult['return_code'] == 'FAIL') {
            return array('error'=>1,'msg'=>'通信出错：'.$orderQueryResult['return_msg']);
            ...
            $order_param['pay_type'] = 'weixin';
            $order_param['is_mobile'] = '3';
            $order_param['order_type'] = 'crowd';
            $order_param['order_id'] = $this->order_info['corder_id'];
            $order_param['third_id'] = $orderQueryResult['transaction_id'];
            $order_param['pay_money'] = $orderQueryResult['total_fee']/100;
            return array('error'=>0,'order_param'=>$order_param);
        }
