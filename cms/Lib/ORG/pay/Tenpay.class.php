...
class Tenpay{
	...
	public function mobile_pay(){
		...
		$reqHandler->setParameter("sp_billno", $this->order_info['order_type'].'_'.$this->order_info['order_id']); //商户订单号
	...
	public function mobile_refund(){
		...
		//设置支付参数 
		$reqHandler->setParameter("partner",$this->pay_config['pay_tenpay_partnerid']);  //商户号
		$reqHandler->setParameter("out_trade_no",$this->order_info['order_type'].'_'.$this->order_info['order_id']);  //订单号
		$reqHandler->setParameter("input_charset", "utf-8");   	  //字符集
