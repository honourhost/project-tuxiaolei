...
class Common_util_pub
{
	...
	function arrayToXml($arr)
    {
        ...
        $xml.="</xml>";
      //  file_put_contents("crowdfund2.txt",$xml,FILE_APPEND);
        return $xml; 
    }
...
{	
	...
	function createXml()
	{
		try
		{
		    ...
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
            	file_put_contents("crowdfund3.txt",json_encode($this->parameters),FILE_APPEND);
		    return  $this->arrayToXml($this->parameters);
...
class App_pub extends Common_util_pub
{
	...
	public function getParameters()
	{
		...
		$appObj['partnerid']= $this->mchid;
		$appObj['prepayid']=$this->prepay_id;
		$appObj["package"] = "Sign=WXPay";
		$appObj["noncestr"] = $this->createNoncestr();
		$timeStamp = time();
		$appObj["timestamp"] = "$timeStamp";
