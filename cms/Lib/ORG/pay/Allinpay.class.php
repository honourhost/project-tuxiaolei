...
class Allinpay{
	...
	public function __construct($order_info,$pay_money,$pay_type,$pay_config,$user_info,$is_mobile=0){
		...
		$this->payUrlDomain = ($_SERVER['HTTP_HOST'] != 'www.group.com') ? 'https://service.allinpay.com' : 'http://ceshi.allinpay.com';
		$this->refundUrl = ($_SERVER['HTTP_HOST'] != 'www.group.com') ? 'service.allinpay.com' : 'ceshi.allinpay.com';
	}
	public function pay(){
	...
	public function mobile_pay(){
		...
		$allinpayClass->setParameter('pickupUrl',C('config.site_url').'/wap.php?c=Pay&a=return_url&pay_type=allinpay&is_mobile=1'); //跳转通知地址
		$allinpayClass->setParameter('receiveUrl',C('config.site_url').'/wap.php?c=Pay&a=notify_url&pay_type=allinpay&is_mobile=1'); //异步通知地址
		$allinpayClass->setParameter('merchantId',$this->pay_config['pay_allinpay_merchantid']); //商户号
		$allinpayClass->setParameter('orderNo',$this->order_info['order_type'].'_'.$this->order_info['order_id']); //订单号
	...
	public function web_pay(){
		...
		$allinpayClass->setParameter('pickupUrl',C('config.site_url').'/index.php?c=Pay&a=return_url&pay_type=allinpay'); //跳转通知地址
		$allinpayClass->setParameter('receiveUrl',C('config.site_url').'/index.php?c=Pay&a=notify_url&pay_type=allinpay'); //异步通知地址
		$allinpayClass->setParameter('merchantId',$this->pay_config['pay_allinpay_merchantid']); //商户号
		$allinpayClass->setParameter('orderNo',$this->order_info['order_type'].'_'.$this->order_info['order_id']); //订单号
		$allinpayClass->setParameter('orderAmount',floatval($this->pay_money*100)); //订单金额(单位分)
		$allinpayClass->setParameter('orderDatetime',date('YmdHis',$_SERVER['REQUEST_TIME'])); //订单提交时间
