...
class Merchant_storeModel extends Model
{
	 ...
	 * @param number $limit
	 */
	public function get_list_by_option($area_id = 0, $circle_id = 0, $order = 'store_id', $lat = 0, $long = 0, $cat_id = 0, $cat_fid = 0, $store_type = 2,$city_id=2268)
	{
		...
		$city_id && $condition_where .= " AND s.city_id={$city_id}";
		...
		foreach ($res as $key=>&$v) {
			//先判断是否存在农小店产品，没有不执行下面并且将该值置空
			$resMeal=D("Meal")->where(array("store_id"=>$v['store_id'],"status"=>1))->count();
			if($resMeal>0){
				...
				$v['wap_url'] = U('Food/menu', array('mer_id' => $v['mer_id'], 'store_id' => $v['store_id']));
		...
		$return['meal_list'] = array_merge($result);
...
// 		$condition_table = array(C('DB_PREFIX') . 'merchant_store' => 's', C('DB_PREFIX') . 'merchant_store_meal' => 'm');
		$condition_where = "s.have_meal=1 AND s.status=1 AND s.store_id <> 700";
...
// 					break;
	...
	public function get_hot_list($limit = 6, $lat = 0, $long = 0)
	{
        ...
        //城市判断power by  taorong
        $condition_where = "s.have_meal=1 AND s.status=1 AND s.city_id=$_SESSION[selectcityid]";
		 //城市判断power by  taorong
