...
class MerchantModel extends Model{
	...
	public function saveRelation($openid, $mer_id, $from_merchant){
		...
		if (empty($relation)) {
			...
			$where['follow_num'] = 1;
			$this->where(array('mer_id' => $mer_id))->setInc('fans_count', 1);
			//$issubscribe && $this->add_fans($mer_id);
			$from_merchant && $this->update_group_indexsort($mer_id);
		}
	...
	// 		$sql = 'SELECT DISTINCT(openid) FROM  ' . C('DB_PREFIX') . 'merchant_user_relation AS a INNER JOIN  ' . C('DB_PREFIX') . 'user_long_lat as c ON c.open_id=a.openid WHERE NOT EXISTS (SELECT openid FROM  ' . C('DB_PREFIX') . 'merchant_user_relation AS b WHERE a.openid = b.openid AND b.mer_id='.$mer_id.') ORDER BY ' . $order . ' LIMIT ' . $limit;
	// 		$res = $mod->query($sql);
