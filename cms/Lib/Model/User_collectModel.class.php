...
class User_collectModel extends Model{
        ...
        public function getCollects($type,$uid)
        {
            ...
            if($type=="goods"){
                ...
                $count_collect = $this->alias("collect")->where($condition)->count();
                //加入页码最大限制，超过最大值返回空
                $req_page=$_GET['p'];
                ...
                $Model=new Model();
                $sql="SELECT 1 as type,guc.collect_id,guc.id,g.s_name,g.old_price,g.price,g.pic as image,g.sale_count as sale_count FROM ". C('DB_PREFIX') ."user_collect guc JOIN " . C('DB_PREFIX') . "group g ON guc.id=g.group_id WHERE guc.type='group_detail' AND guc.uid=".$uid;
                $sql .= ' UNION ALL ';
                ...
                if(!empty($_GET['lat'])&&!empty($_GET['long'])) {
                    $x = $_GET['lat'];
                    $y = $_GET['long'];
                    $sql = "SELECT mucres.*,ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$x}*PI()/180-ms.`lat`*PI()/180)/2),2)+COS({$x}*PI()/180)*COS(ms.`lat`*PI()/180)*POW(SIN(({$y}*PI()/180-ms.`long`*PI()/180)/2),2)))*1000) AS distance from (SELECT muc.collect_id,m.name,m.phone,m.fans_count,m.person_image as image,m.mer_id  FROM " . C('DB_PREFIX')
                    ...
                    $sql = "SELECT mucres.*,'unknown' as distance from (SELECT muc.collect_id,m.name,m.phone,m.fans_count,m.person_image as image,m.mer_id FROM " . C('DB_PREFIX')
                        . "user_collect muc LEFT JOIN " . C('DB_PREFIX') . "merchant m ON muc.id=m.mer_id WHERE muc.type='merchant_id' AND muc.uid=" . $uid .
                        ") as mucres LEFT JOIN " . C('DB_PREFIX') . "merchant_store ms ON mucres.mer_id=ms.mer_id WHERE ms.ismain=1";
                    $sql .= " ORDER BY collect_id DESC LIMIT {$p->firstRow}, {$p->listRows}";
                }
        ...
        public function addCollect($type,$id,$uid){
            ...
            if(!empty($res)){
                ...
                if($result){
                    if($type=="merchant_id"){
                    $res=D("merchant")->where("mer_id=".$id)->find();
                    if($res['fans_count']>0){
                    D("Merchant")->where("mer_id=".$id)->setDec("fans_count");
                    }
                    }
                    return true;
                }else{
            ...
            if($this->data($data)->add()){
                if($type=="merchant_id"){
                D("Merchant")->where("mer_id=".$id)->setInc("fans_count");
                }
                return true;
            }else{
                return false;
