...
class Merchant_recharge_orderModel extends Model{
	...
	public function get_order_by_id($mer_id,$order_id){
		$condition_user_recharge_order['mer_id'] = $mer_id;
	...
	public function after_pay($order_param){
		...
		if(empty($now_order)){
			...
			if($order_param['is_mobile']){
				return array('error'=>1,'msg'=>'该订单已付款！','url'=>U('Merchant/depositeReturn'));
			}else{
				return array('error'=>1,'msg'=>'该订单已付款！','url'=>U('Merchant/depositeReturn'));
			}
		}else{
			...
			if($this->where($where)->save($data_user_recharge_order)){
				//插入到记录缴纳押金的表中
				$data['mer_id']=$now_order['mer_id'];
				$data['order_id']=$now_order['order_id'];
				$data['pay_money']=$order_param['pay_money'];
				$data['pay_info']="缴纳押金";
				$data['status']=0;
				$data['create_time']=time();
				D('Merchant_pay_deposite')->add($data);
			}else{
	...
	public function get_weidian_url($data){
		...
		return 'http://d.wei8du.com/api/pay_callback.php?'.http_build_query($data);
	}
