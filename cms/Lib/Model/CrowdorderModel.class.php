...
class CrowdorderModel extends Model
{
    ...
    public function save_post_form($group,$uid,$order_id){
        ...
        $data_group_order["is_lottery_order"]=$group["is_lottery_grade"]==2?1:0;
      ...
      //  $now_adress = $group['user_adress']['adress'];
            if (empty($now_adress)) {
    ...
    public function after_pay($order_param){
        ...
        if(empty($now_order)){
            ...
            if($this->where($condition_group_order)->data($data_group_order)->save()){
                ...
                D('Crowdfunding')->where($condition_grade)->setInc('total',round(floatval($order_param['pay_money']),2));
                ...
                if (C('config.sms_success_order') == 2 || C('config.sms_success_order') == 3) {
                    ...
                    Sms::sendSms($sms_data);
                       ...
                       //todo
                $crowdgrade=D("Crowdgrade")->where($condition_grade)->find();
                if(!empty($crowdgrade)){
                    if($crowdgrade["is_lottery_grade"]){
                        $lotterylimit=$crowdgrade["lottery_num"];
                        $condition_order["cgrade_id"]=$now_order['cgrade_id'];
                        $condition_order["lottery_confirm"]=0;
                        $condition_order['paid']=1;
                    $noworddercount=    D("Crowdorder")->where($condition_order)->count();
                    if($noworddercount>$lotterylimit-1){   //如果达到抽奖条件
                        $noworders=D("Crowdorder")->where($condition_order)->select();
                        $randomnum= rand(0,$noworddercount-1);
                        $lotteryorder=$noworders[$randomnum];
                        file_put_contents("crowdtest.txt",json_encode($lotteryorder),FILE_APPEND);
                        D("Crowdorder")->where($condition_order)->data(array("lottery_confirm"=>1))->save();
                        D("Crowdorder")->where(array("corder_id"=>$lotteryorder["corder_id"]))->data(array("lottery"=>1,"lottery_confirm"=>1))->save();
                        $lotteryuser=D("User")->where(array("uid"=>$lotteryorder['uid']))->find();
                        if(!empty($lotteryorder)){
                            if(!empty($lotteryuser["openid"])){
                                $href = C('config.site_url').'/wap.php?c=My&a=crowdorder&order_id='.$lotteryorder['corder_id'];
                                $model = new templateNews(C('config.wechat_appid'), C('config.wechat_appsecret'));

                                $model->sendTempMsg('OPENTM201752540', array('href' => $href, 'wecha_id' => $now_user['openid'], 'first' => $this->config['group_alias_name'].'众筹提醒', 'keyword1' => $now_order['name'], 'keyword2' => $now_order['corder_id'], 'keyword3' => $now_order['total_money'], 'keyword4' => date('Y-m-d H:i:s'), 'remark' => '中奖，感谢您的使用'));
