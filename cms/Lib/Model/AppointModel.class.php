...
class AppointModel extends Model{
	...
	public function wap_get_appoint_list_by_catid($get_grouplist_catid,$get_grouplist_catfid,$area_id,$order, $lat = 0, $long = 0, $circle_id = 0){
		...
		if($area_id || $circle_id){
			$condition_field  = 'DISTINCT `g`.`appoint_id`,`g`.`name` AS `group_name`,`m`.`name` AS `merchant_name`,`g`.*,`m`.*';
			if ($circle_id) {
			...
			$condition_field  = '`g`.`appoint_name` AS `appoint_name`,`m`.`name` AS `merchant_name`,`g`.*,`m`.*';
			if ($order == 'juli') {
