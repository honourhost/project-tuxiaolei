...
class Group_orderModel extends Model{
	public function save_post_form($group,$uid,$order_id){
		...
		if(($group['secondprice']>0)&&($data_group_order['num']>1)){
            $data_group_order['total_money'] = $group['price'] * ($data_group_order['num']-1)+$group['secondprice'];
        }else{
...
//				}
    ...
    //获取大订单详情
    public function new_get_order_detail_by_id($record_id){
        //$sql = "select * from 'pigcms_group_order' where 'adam_id'=(select 'adam_id' from 'pigcms_usercardrecord' where 'record_id'=".$record_id.")";
	...
	public function get_pay_order($uid,$order_id,$is_web=false){
		...
		if($now_group['end_time']<$now_time){
			return array('error'=>1,'msg'=>'当前'.$this->config['group_alias_name'].'商品已下架！');
		}
		if(empty($now_group)){
...
//            }
	...
	public function orderConfirm($order_id){
		...
		if (!($this->where($condition_group_order)->data($data_group_order)->save())) {
			...
			$now_order=$this->alias("go")->field("go.order_name,go.num,go.group_id,go.distribution_id,go.total_money,g.commission")->join(C('DB_PREFIX')."group g ON go.group_id=g.group_id")->where("go.order_id=".$order_id)->find();
                ...
                if(!empty($fenxiaozhe)){
                    ...
                    if(!empty($fenxiaozhe)){
                        ...
                        $model = new templateNews(C('config.wechat_appid'), C('config.wechat_appsecret'));
                        if(floatval($group["commission"])>0){
                            $model->sendTempMsg('OPENTM207873652', array('href' => 'http://www.xiaonongding.com/wap.php', 'wecha_id' => $fenxiaouser['openid'], 'first' => '分销消费提醒', 'keyword1' => $now_order['total_money'], 'keyword2' => ($group["commission"]*$now_order["num"]*1)."元", 'keyword3' => date("y/m/d H:i:s",time()) , 'remark' => '感谢您的分销'));
			...
			$money=floatval($now_order['commission']*$now_order['num']*1);
			if(!empty($money)){
		...
		if($thisorder['sun_id']!=""){
			...
			if(!empty($now_order_list)){
				foreach($now_order_list as $k=>$val){
					$money=floatval($val['commission']*$val['num']*1);
					if(!empty($money)){
					//判断是否属于频道商品，如果是的话则增加频道总营收和收益
