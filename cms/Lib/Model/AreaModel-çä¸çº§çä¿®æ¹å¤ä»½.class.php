...
class AreaModel extends Model{
	...
	//	$condition_area['area_pid'] = C('config.now_city');
         ...
         */
        	...
        	if(isset($_SESSION['selectcityid'])) {
				$condition['area_id'] = $_SESSION['selectcityid'];
				$condition_area['area_pid'] = $_SESSION['selectcityid'];
			}elseif(isset($_SESSION['provinceid'])){
				$condition['area_id'] = $_SESSION['provinceid'];
				$condition_area['area_pid'] = $_SESSION['provinceid'];
			}else{
				$condition_area['area_pid'] = 0;
			}
			...
			foreach($area_list as $key=>$value){
				foreach ($area_list as $key => $value) {
					if(isset($_SESSION['provinceid'])) {
						$area_list[$key]['url'] = C('config.site_url')."/?cityid=".($value['area_id']);
					}elseif($value['area_pid']==0){
						$area_list[$key]['url'] = C('config.site_url')."/?provinceid=".($value['area_id']);
					}else{
	...
	/*通过父级ID得到子区域*/
	public function get_arealist_by_areaPid($area_pid=2,$is_open=false,$cat_url='', $type = 'category'){
