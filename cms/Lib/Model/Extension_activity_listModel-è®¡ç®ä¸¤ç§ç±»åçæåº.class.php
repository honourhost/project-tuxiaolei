...
class Extension_activity_listModel extends Model{
    ...
    public function getActivityListByMerchantOrder($mer_id,$order){
        ...
        switch($order){
            case 'price-asc':
                $order = '`a`.`price` ASC,`a`.`pigcms_id` DESC';
                break;
            case 'price-desc':
                $order = '`a`.`price` DESC,`a`.`pigcms_id` DESC';
                break;
            case 'hot':
        ...
        $count_activity=$this->where(array("status"=>1,"mer_id"=>$mer_id))->count();
        $p = new Page($count_activity, 10,"p");
        $result=$this->alias("a")->field("a.*,t.begin_time,t.end_time")->join(C("DB_PREFIX")."extension_activity t ON a.activity_term=t.activity_id")->where(array("a.status"=>1,"a.mer_id"=>$mer_id,"t.begin_time"=>array("lt",$now_time),"t.end_time"=>array("gt",$now_time)))->limit($p->firstRow,$p->listRows)->order($order)->select();
        if(!empty($result)) {
            foreach($result as $k=>$val) {
                $extend_image = new Extension_image();
                $result[$k]['pic'] = $extend_image->get_allImage_by_path($val['pic']);
