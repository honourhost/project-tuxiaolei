...
class User_collectModel extends Model{
        ...
        public function getCollects($type,$uid)
        {
            ...
            if($type=="goods"){
                ...
                $p = new Page($count_collect, 10);
                $Model=new Model();
                $sql="SELECT 1 as type,guc.collect_id,g.name,g.old_price,g.price,g.pic as image,g.sale_count as sale_count FROM ". C('DB_PREFIX') ."user_collect guc JOIN " . C('DB_PREFIX') . "group g ON guc.id=g.group_id WHERE guc.type='group_detail' AND guc.uid=".$uid;
                $sql .= ' UNION ALL ';
                $sql.="SELECT 2 as type,muc.collect_id,m.name,m.old_price,m.price,m.image as image,m.sell_count as sale_count  FROM ". C('DB_PREFIX') ."user_collect muc JOIN " . C('DB_PREFIX') . "meal m ON muc.id=m.meal_id WHERE muc.type='meal_detail' AND muc.uid=".$uid;
