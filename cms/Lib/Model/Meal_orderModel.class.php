...
// 			return array('error'=>1,'msg'=>'该订单已付款！','url'=>U('User/Index/meal_order_view',array('order_id' => $now_order['order_id'])));
	...
	public function get_rated_order($uid,$order_id){
		...
		$condition_where = "`o`.`uid`='$uid' AND `o`.`store_id`=`s`.`store_id` AND `o`.`order_id`=".$order_id;
		...
		if(!empty($order)) {
			$store_image_class = new store_image();
			$images = $store_image_class->get_allImage_by_path($order['pic_info']);
			$order['image'] = $images ? array_shift($images) : array();
			//$order['url'] = C('config.site_url') . '/meal/' . $order['store_id'] . '.html';
	...
	public function get_order_by_mer_id($mer_id, $is_system = false)
	{
		...
		foreach ($res as $r) {
            ...
            $alltotal=$alltotal-$r['commission'];//除去佣金的未对账总额
		}
		$sql = "SELECT 1 as name, order_id, info as order_name, uid, mer_id, store_id, phone, total, (balance_pay+payment_money) as price, price as order_price, dateline, paid, status, pay_type, pay_time, third_id, is_mobile_pay, balance_pay, payment_money, card_id, merchant_balance, is_pay_bill,0 as commission FROM ". C('DB_PREFIX') . "meal_order WHERE mer_id={$mer_id} AND paid=1 AND status in (1,2) AND (pay_type<>'offline' OR balance_pay<>'0.00')";
		$sql .= ' UNION ALL ';
		$sql .= "SELECT 2 as name, gorder.order_id, gorder.order_name, gorder.uid, gorder.mer_id, gorder.store_id, gorder.phone, gorder.num as total, (gorder.balance_pay+gorder.payment_money) as price, gorder.total_money as order_price, gorder.add_time as dateline, gorder.paid, gorder.status, gorder.pay_type, gorder.pay_time, gorder.third_id, gorder.is_mobile_pay, gorder.balance_pay, gorder.payment_money, gorder.card_id, gorder.merchant_balance, gorder.is_pay_bill,g.commission FROM ". C('DB_PREFIX') . "group_order as gorder LEFT JOIN pigcms_group as g ON gorder.group_id=g.group_id  WHERE gorder.mer_id={$mer_id} AND gorder.paid=1 AND gorder.status in (1,2) AND (gorder.pay_type<>'offline' OR gorder.balance_pay<>'0.00')";
		...
		foreach ($res as &$l) {
			...
			$total +=($l['price']-$l['commission']);//本页的总额(出去佣金)
			$l['is_pay_bill'] && $finshtotal += ($l['price']-$l['commission']);	//除去佣金本页已对账的总额
		}
		$pagebar = $p->show();
	...
	//新增的选取只有用户确认的订单，并且可以将未确认超过15天的订单自动确认
	//总后台只判断用户已经确认的那部分订单（另外，在订单付账之后处于消费状态的订单，超过15天，则首先将用户确认状态改为1）
	public function get_confirmorder_by_mer_id($mer_id,$is_system){
		//首先查看处于消费状态的订单，status in (1,2)中是否有pay_time小于time()-15*24*60*60秒之前并且没有经过用户确认的信息，将这些订单的用户确认直接改为1
		$this->where(array('mer_id' => $mer_id, 'paid' => 1, 'pay_type' => array('NEQ', 'offline'), 'status' => array('in', '1, 2'),'use_time'=>array('LT',time()-15*24*60*60),"user_confirm"=>0))->setField(array("user_confirm"=>1,"is_pay_bill"=>1));
		D('Group_order')->where(array('mer_id' => $mer_id, 'pay_type' => array('NEQ', 'offline'), 'paid' => 1, 'status' => array('in', '1, 2'),'use_time'=>array('LT',time()-15*24*60*60),"user_confirm"=>0))->setField(array("user_confirm"=>1,"is_pay_bill"=>1));
		//修改完超过15天的消费未确认订单之后，将所有已经确认的订单查出
		if ($is_system) {
			import('@.ORG.system_page');
		} else {
			import('@.ORG.merchant_page');
		...
		$count1 = $this->where(array('mer_id' => $mer_id,"user_confirm"=>1, 'paid' => 1, 'pay_type' => array('NEQ', 'offline'), 'status' => array('in', '1, 2')))->count();
		$count2 = D('Group_order')->where(array('mer_id' => $mer_id,"user_confirm"=>1, 'pay_type' => array('NEQ', 'offline'), 'paid' => 1, 'status' => array('in', '1, 2')))->count();
		$p = new Page($count1 + $count2, 20);
		...
		$sql = "SELECT 1 as name, order_id, info as order_name, uid, mer_id, store_id, phone, total,user_confirm, (balance_pay+payment_money) as price, price as order_price, dateline, paid, status, pay_type, pay_time, third_id, is_mobile_pay, balance_pay, payment_money, card_id, merchant_balance, is_pay_bill FROM " . C('DB_PREFIX') . "meal_order WHERE mer_id={$mer_id} AND paid=1 AND status in (1,2)  AND user_confirm=1  AND (pay_type<>'offline' OR balance_pay<>'0.00')";
		$sql .= ' UNION ALL ';
		$sql .= "SELECT 2 as name, order_id, order_name, uid, mer_id, store_id, phone,user_confirm, num as total, (balance_pay+payment_money) as price, total_money as order_price, add_time as dateline, paid, status, pay_type, pay_time, third_id, is_mobile_pay, balance_pay, payment_money, card_id, merchant_balance, is_pay_bill FROM " . C('DB_PREFIX') . "group_order WHERE mer_id={$mer_id} AND paid=1 AND status in (1,2)  AND user_confirm=1  AND (pay_type<>'offline' OR balance_pay<>'0.00')";
		$sql .= " ORDER BY dateline DESC LIMIT {$p->firstRow}, {$p->listRows}";
