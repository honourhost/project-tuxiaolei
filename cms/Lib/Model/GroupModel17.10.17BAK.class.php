...
// 				$group_list[$key]['wx_cheap'] = floatval($value['wx_cheap']);
	...
	public function get_single_store_group_list($store_id,$limit=6,$is_wap=false){
		$now_time = $_SERVER['REQUEST_TIME'];
		$group_list = D('')->field('`g`.`group_id`,`g`.`name` AS `group_name`,`g`.`price`,`g`.`pic`,`g`.`old_price`,`g`.`wx_cheap`,`g`.`sale_count`,`g`.`virtual_num`,`g`.`end_time`')->table(array(C('DB_PREFIX').'group'=>'g',C('DB_PREFIX').'group_store'=>'gc'))->where("`g`.`group_id`=`gc`.`group_id` AND `gc`.`store_id`='$store_id' AND `g`.`status`='1' AND `g`.`is_group_buy`='0' AND `g`.`type`='1' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time'")->order('`g`.`sort` DESC,`g`.`group_id` DESC')->limit($limit)->select();
		if($group_list){
			$group_image_class = new group_image();
	...
	public function get_group_list_by_catid($get_grouplist_catid,$get_grouplist_catfid,$cat_url,$area_id,$circle_id,$order,$attrs,$category_cat_field){
		...
		$area_idstr=implode(',',$_SESSION['areaarr']);
		$now_time = $_SERVER['REQUEST_TIME'];
		$condition_where = "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`type`='1' AND `g`.`is_group_buy`='0' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time'  AND `m`.`status`='1' AND `m`.`area_id` in (".$area_idstr.")  AND `g`.`mer_id` <> '890' AND `g`.`group_id` <> '3246' AND `g`.`group_id` <> '3254' ";
		}else{
		$now_time = $_SERVER['REQUEST_TIME'];
		$condition_where = "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`type`='1' AND `g`.`is_group_buy`='0' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time'  AND `m`.`status`='1'  AND `g`.`mer_id` <> '890' AND `g`.`group_id` <> '3246' AND `g`.`group_id` <> '3254' ";
		}
		//分类相关
	...
	public function get_group_list_by_keywords($keywords,$order,$is_wap=false){
		$now_time = $_SERVER['REQUEST_TIME'];
		$condition_where = "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `m`.`status`='1' AND `g`.`type`='1' AND `g`.`is_group_buy`='0' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time' AND (`g`.`name` LIKE '%$keywords%' OR `m`.`name` like '%$keywords%' OR `g`.`s_name` LIKE '%$keywords%' ) AND `g`.`mer_id` <> '747' AND `g`.`mer_id` <> '890' AND `g`.`group_id` <> '3246' AND `g`.`group_id` <> '3254' ";
		$condition_table = array(C('DB_PREFIX').'group'=>'g',C('DB_PREFIX').'merchant'=>'m');
		$condition_field  = '`g`.`name` AS `group_name`,`m`.`name` AS `merchant_name`,`g`.*,`m`.*';
	...
	public function wap_get_group_list_by_catid($get_grouplist_catid,$get_grouplist_catfid,$area_id,$order, $lat = 0, $long = 0, $circle_id = 0){
		$now_time = $_SERVER['REQUEST_TIME'];
		$condition_where = "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`is_group_buy`='0' AND `g`.`type`='1' AND `m`.`status`='1' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time' AND `g`.`mer_id` <> '747' AND `g`.`mer_id` <> '621' AND `g`.`mer_id` <> '890' AND `g`.`group_id` <> '3246' AND `g`.`group_id` <> '3254' ";
		//分类相关
		if(!empty($get_grouplist_catfid)){
			$condition_where .= " AND `g`.`cat_fid`='$get_grouplist_catfid'";
		}else if(!empty($get_grouplist_catid)){
...
// 		echo D('')->_sql();
	...
	public function wap_get_storeList_by_catid($get_grouplist_catid,$get_grouplist_catfid,$area_id,$order, $lat = 0, $long = 0, $circle_id = 0){
		$now_time = $_SERVER['REQUEST_TIME'];
		$condition_where = "`g`.`status`='1' AND `g`.`is_group_buy`='0' AND `g`.`type`='1' AND `s`.`status`='1' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time' AND `gc`.`group_id`=`g`.`group_id` AND `gc`.`store_id`=`s`.`store_id` AND `g`.`mer_id` <> '629' AND `s`.`store_id` <> '700' AND `g`.`mer_id` <> '621' AND `g`.`mer_id` <> '890' AND `g`.`group_id` <> '3246'  AND `g`.`group_id` <> '3254'";
		//分类相关
		if(!empty($get_grouplist_catfid)){
			$condition_where .= " AND `g`.`cat_fid`='$get_grouplist_catfid'";
		}else if(!empty($get_grouplist_catid)){
	...
	public function get_grouplist_by_catId($cat_id,$cat_fid,$limit=6,$is_wap=false,$sort=''){
		...
		$area_idstr=implode(',',$_SESSION['areaarr']);
		$now_time = $_SERVER['REQUEST_TIME'];
		$condition_where = "`g`.`mer_id`=`m`.`mer_id` AND `m`.`status`='1' AND `g`.`status`='1' AND `g`.`type`='1' AND `g`.`is_group_buy`='0' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time' AND `m`.`area_id` in (".$area_idstr.") AND `g`.`mer_id` <> '747' AND `g`.`mer_id` <> '890' AND `g`.`group_id` <> '3246' AND `g`.`group_id` <> '3254'";
		}else{
		$now_time = $_SERVER['REQUEST_TIME'];
		$condition_where = "`g`.`mer_id`=`m`.`mer_id` AND `m`.`status`='1' AND `g`.`status`='1' AND `g`.`type`='1' AND `g`.`is_group_buy`='0' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time' AND `g`.`mer_id` <> '747' AND `g`.`group_id` <> '3254' AND `g`.`mer_id` <> '890' ";
		}
		// $now_time = $_SERVER['REQUEST_TIME'];
