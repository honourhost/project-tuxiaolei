...
// 		echo D('')->_sql();
    ...
    /*wap版得到指定分类ID或分类父ID下的分类，带有分页功能*/
    public function new_wap_get_group_buy_list_by_merid($listrow=6,$merid,$get_grouplist_catid,$get_grouplist_catfid,$area_id,$order, $lat = 0, $long = 0, $circle_id = 0){
        $now_time = $_SERVER['REQUEST_TIME'];
        $condition_where = "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`is_group_buy`='1' AND `g`.`type`='1' AND `m`.`status`='1' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time' AND `g`.`is_lottery_group_buy` <> '1'";
        //分类相关
        if(!empty($get_grouplist_catfid)){
        ...
        if(!empty($merid)){
            $condition_where.=" AND `g`.`mer_id`='$merid'";
        }
        //区域
        if($area_id || $circle_id){
            $condition_field  = 'DISTINCT `g`.`group_id`,`g`.`s_name` AS `group_name`,`m`.`name` AS `merchant_name`,`g`.*,`m`.*';
...
// 		echo D('')->_sql();
     ...
     //获取某商户的抽奖团
    public function new_wap_get_group_buy_lottery_list_by_merid($listrow=6,$mer_id,$get_grouplist_catid,$get_grouplist_catfid,$area_id,$order, $lat = 0, $long = 0, $circle_id = 0){
        $now_time = $_SERVER['REQUEST_TIME'];
        $condition_where = "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`is_group_buy`='1' AND `g`.`type`='1' AND `m`.`status`='1' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time' AND `g`.`is_lottery_group_buy`='1'";
        //分类相关
        if(!empty($get_grouplist_catfid)){
        ...
        if(!empty($mer_id)){
            $condition_where.=" AND `g`.`mer_id`='$mer_id'";
        }
...
// 		echo D('')->_sql();
    ...
    public function new_wap_get_group_buy_firstfree_by_merid($listrow=6,$merid,$get_grouplist_catid,$get_grouplist_catfid,$area_id,$order, $lat = 0, $long = 0, $circle_id = 0){
        $now_time = $_SERVER['REQUEST_TIME'];
        $condition_where = "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`is_group_buy`='1' AND `g`.`type`='1' AND `m`.`status`='1' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time' AND `g`.`is_lottery_group_buy` <> '1' AND `g`.`is_first_free` = '1'";
        //分类相关
        if(!empty($get_grouplist_catfid)){
