    ...
    public function find($type,$keywords,$lat,$long,$order){
        if(!empty($type)){
            ...
            if($type=="goods") {
                ...
                $condition_field = "DISTINCT `g`.`group_id`,`g`.`name`,`g`.`s_name`,`g`.`intro`,g.sale_count,`g`.`pic` `group_pic`,`g`.`price`,`g`.`wx_cheap`,`g`.`end_time`,ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$lat}*PI()/180-`ms`.`lat`*PI()/180)/2),2)+COS({$lat}*PI()/180)*COS(`ms`.`lat`*PI()/180)*POW(SIN(({$long}*PI()/180-`ms`.`long`*PI()/180)/2),2)))*1000) AS distance";
