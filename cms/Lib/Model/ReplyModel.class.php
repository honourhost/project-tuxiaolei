...
class ReplyModel extends Model{
     ...
     * 单张图评论
     */
    public function get_reply_list2($parent_id,$order_type,$store_count,$limit){
        ...
        if($order_type == 0 && $store_count > 1){
            ...
            $reply_list = D('')->field('`u`.`nickname`,`u`.`avatar`,`r`.*')->table(array(C('DB_PREFIX').'reply'=>'r',C('DB_PREFIX').'user'=>'u'))->where("`r`.`parent_id`='$parent_id' AND `r`.`uid`=`u`.`uid`")->order($condition_order)->limit($limit)->select();
        }
        ...
        if($reply_list){
            ...
            foreach($reply_list as $key=>$value){
                ...
                if(empty($value["avatar"])){
                    ...
                    if(!(strpos($value["avatar"],"http://") === 0)){
                        ...
                        $reply_list[$key]['avatar']=$value["avatar"];
                    }
                }
                if(!empty($value['pic'])){
                    $tmp_arr = explode(',',$value['pic']);
            ...
            if($pic_arr){
                ...
                $reply_image_class = new reply_image();
                foreach($reply_pic_list as $key=>$value){
                    $tmp = $reply_image_class->get_image_by_path($value['pic'],$pic_filepath);
                    $tmp_value=$tmp["image"];
                    $reply_list[$new_pic_arr[$value['pigcms_id']]]['pics'][] = $tmp_value;
                }
            }
