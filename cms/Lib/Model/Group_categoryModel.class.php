...
class Group_categoryModel extends Model{
	...
	public function get_category($get_all=true,$cat_fid=0,$limit_num=0){
		...
		if($get_all){
			...
			foreach($tmp_group_category as $key=>$value){
				$value['url'] = $this->get_category_url($value);
					if($group_category[$tmp_category[$value['cat_fid']]]['cat_count']<4){
	...
	//返回农场列表的分类
	public function get_farmcategory($get_all=true,$cat_fid=0,$limit_num=0){
		if(empty($limit_num)){
			$limit_num = '';
		}
		if($get_all){
			...
			foreach($tmp_group_category as $key=>$value){
				if(empty($value['cat_fid'])){
					...
					$value['cat_count'] = 0;
					$value['url'] = $this->get_farmcategory_url($value);
					if(!empty($value['cat_pic'])){
						$value['cat_pic'] = C('config.site_url').'/upload/system/'.$value['cat_pic'];
					}
					$group_category[$key] = $value;
	...
	//得到所有的农场分类
	public function get_all_farmcategory(){
		$condition_group_category['cat_status'] = '1';
		$tmp_group_category = $this->field(true)->where($condition_group_category)->order('`cat_sort` DESC')->limit()->select();
		$group_category = array();
		$tmp_category = array();
		foreach($tmp_group_category as $key=>$value){
			if(empty($value['cat_fid'])){
				...
				$value['cat_count'] = 0;
				$value['url'] = $this->get_farmcategory($value);
	...
	/*根据cat_url得到农场分类*/
	public function get_farmcategory_by_catUrl($cat_url){
		$condition_group_category['cat_url'] = $cat_url;
		$condition_group_category['cat_status'] = '1';
		$now_category = $this->field(true)->where($condition_group_category)->find();
		if(!empty($now_category)){
			$now_category['url'] = $this->get_farmcategory_url($now_category);
		}
		return $now_category;
	}
	//根据cat_id得到农场分类
	public function get_farmcategory_by_id($cat_id){
	...
	//返回有关农场分类的url
	public function get_farmcategory_url($category){
		return C('config.site_url').'/farm/'.$category['cat_url'];
	}
	/*根据农场顶级ID或子分类ID 得到子分类或子分类的同级分类*/
	public function get_son_farmcategory_list_byid($cat_fid,$cat_id){
		if(!empty($cat_fid)){
		...
		foreach($son_category_list as $key=>$value){
			$son_category_list[$key]['url'] = $this->get_farmcategory_url($value);
		}
