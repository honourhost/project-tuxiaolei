...
//     	$this->assign('index_right_adver',$index_right_adver);
		...
		$now_group['person_image'] = $merchant_image_class->get_image_by_path($now_group['person_image'],$this->config['site_url'],'s');
		$now_group['merchant_theme_image'] = $merchant_image_class->get_image_by_path($now_group['merchant_theme_image'],$this->config['site_url'],'s');
		...
		//商家团购列表
		$merchant_group_list = D('Group')->get_grouplist_by_MerchantId($now_group['mer_id'],2,false,$now_group['group_id']);
		...
		//分类下最热门团购，销售量
		$category_hot_group_list = D('Group')->get_grouplist_by_catId($now_group['cat_id'],$now_group['cat_fid'],2,false,'`sale_count` DESC');
		...
		// 	'user_confirm'=>1
		// );
		$where="go.group_id=".$now_group['group_id']." AND go.mer_id=".$now_group['mer_id']." AND go.paid=1 AND (go.user_confirm=1 OR go.status=2)";
		$all_count=D('Group_order')->alias("go")->join(C('DB_PREFIX')."user u ON go.uid=u.uid")->where($where)->count();
	...
	public function getOrderListDemo(){
		...
		$where="go.group_id=".$group_id." AND go.mer_id=".$mer_id."  AND go.paid=1 AND go.status<3";//AND go.paid=1 AND (go.user_confirm=1 OR go.status=2)";
		...
		if($get_page==($total+1)){
			...
			$group_order_list=D('Group_order')->alias("go")->field("go.price,go.pay_time as last_time,u.nickname,u.avatar,go.num,go.order_name")->join(C('DB_PREFIX')."user u ON go.uid=u.uid")->where($where)->limit($p->firstRow.','.$p->listRows)->order("last_time DESC")->select();
