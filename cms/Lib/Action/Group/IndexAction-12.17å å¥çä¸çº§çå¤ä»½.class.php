...
class IndexAction extends BaseAction{
    public function index(){
		//地区名字
		$this->assign("area_name",$_SESSION['area_name']);
    	//右侧广告
		...
		if($cat_url != 'all'){
				...
				foreach($cat_field  as $key=>$value){
					if($value['use_field']){
						...
						if($value['use_field'] == 'area'){
							...
							if($area_url == 'all'&&empty($circle_list)&&!isset($_SESSION['provinceid'])&&!isset($_SESSION['selectcityid'])){
								array_unshift($all_area_list,array('area_name'=>'全部','area_url'=>'all'));
								$cat_option_list[] = array('txt_desc'=>'省直辖市','row_type'=>'province','area_list'=>$all_area_list);
								//$cat_option_list[] = array('txt_desc'=>'区域','row_type'=>'area','area_list'=>$all_area_list);
							}elseif($area_url == 'all'&&empty($circle_list)&&isset($_SESSION['provinceid'])){
								array_unshift($all_area_list,array('area_name'=>'全部','area_url'=>'all'));
								$cat_option_list[] = array('txt_desc'=>'城市','row_type'=>'city','area_list'=>$all_area_list);
							}elseif($area_url == 'all'&&empty($circle_list)&&isset($_SESSION['selectcityid'])){
								array_unshift($all_area_list,array('area_name'=>'全部','area_url'=>'all'));
								$cat_option_list[] = array('txt_desc'=>'区域','row_type'=>'area','area_list'=>$all_area_list);
							}else{
								$this->assign('area_list',$all_area_list);
			...
			// 	$cat_option_list[] = array('txt_desc'=>'商圈','row_type'=>'circle','circle_list'=>$circle_list);
			// }
			if(empty($circle_list)&&!isset($_SESSION['provinceid'])&&!isset($_SESSION['selectcityid'])){
				array_unshift($all_area_list,array('area_name'=>'全部','area_url'=>'all'));
				$cat_option_list[] = array('txt_desc'=>'省直辖市','row_type'=>'province','area_list'=>$all_area_list);
				//$cat_option_list[] = array('txt_desc'=>'区域','row_type'=>'area','area_list'=>$all_area_list);
			}elseif(empty($circle_list)&&isset($_SESSION['provinceid'])){
				array_unshift($all_area_list,array('area_name'=>'全部','area_url'=>'all'));
				$cat_option_list[] = array('txt_desc'=>'城市','row_type'=>'city','area_list'=>$all_area_list);
			}elseif(empty($circle_list)&&isset($_SESSION['selectcityid'])){
				array_unshift($all_area_list,array('area_name'=>'全部','area_url'=>'all'));
				$cat_option_list[] = array('txt_desc'=>'区域','row_type'=>'area','area_list'=>$all_area_list);
			}else{
				$this->assign('area_list',$all_area_list);
	...
	protected function get_cat_option_html($cat_option_list,$cat_url,$area_url,$circle_url,$order,$attrs){
		...
		foreach($cat_option_list as $key=>$value){
			...
			if($value['row_type'] == 'category'){
			...
			}else if($value['row_type'] == 'province'){
				foreach ($value['area_list'] as $k => $v) {
					$cat_option_html .= '<li ' . ($area_url == $v['area_url'] ? 'class="current"' : '') . '><a href="' .$v['url'] . '">' . $v['area_name'] . '</a></li>';
				}
			}else if($value['row_type'] == 'city'){
				foreach ($value['area_list'] as $k => $v) {
					$cat_option_html .= '<li ' . ($area_url == $v['area_url'] ? 'class="current"' : '') . '><a href="' .$v['url'] . '">' . $v['area_name'] . '</a></li>';
				}
			}
				...
				foreach($value['area_list'] as $k=>$v){
					$cat_option_html .= '<li '.($_SESSION['provinceid']==$v['area_id'] ? 'class="current"' : '').'><a href="'.$v['url'].'">'.$v['area_name'].'</a></li>';
				}
				}else{
				foreach($value['area_list'] as $k=>$v){
					if($v['area_pid']==0&&!isset($_SESSION['provinceid'])){
					$cat_option_html .= '<li '.($area_url==$v['area_url'] ? 'class="current"' : '').'><a href="'.$v['url'].'">'.$v['area_name'].'</a></li>';
					}else{
