...
class IndexAction extends BaseAction {
	...
	public function meal_order_confirm()
	{
		$now_order = D('Meal_order')->get_order_by_id($this->now_user['uid'],$_GET['order_id']);
		if(empty($now_order)){
			$this->error('当前订单不存在！');
			...
			D('Meal_order')->where(array('uid' => $this->now_user['uid'], 'order_id' => $_GET['order_id']))->save(array('user_confirm' => 1,'is_pay_bill'=>1));
			$this->success('订单确认成功！', U('User/Index/meal_list'));
		}
	}
