...
class WithDrawAction extends BaseAction
{
    ...
    public  function index(){
        ...
        $this->assign("lingqian",$real_can_cash);
      //  echo "总共".$res["0"]["price"]."提现了".$countcah[0]["countcash"];die;
       $this->display();
    }
    ...
    if(IS_POST){
      ...
      //  $sql="status in (0,1) and merid =".$mer_id;
       ...
       if($countcah){
        ...
        if($result){
            $this->success("提交提现申请成功！", U("WithDraw/widthdraw"));
        }else{
            $this->error("提交提现申请失败！");
        }
//        if($result){
...
public function  detail(){
    ...
    $sql2="SELECT * FROM ".C('DB_PREFIX') ."fast_bank_cash_info as c WHERE c.status in(0,1,2) and c.mer_id = ".$this->merid;
