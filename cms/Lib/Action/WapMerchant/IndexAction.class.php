...
class IndexAction extends BaseAction {
    ...
    public function __construct()
    {
        parent::__construct();
        //检查自动登录
        if($GLOBALS['_SESSION']['openid']){
            if(D("Merchant_wechat")->where(array("openid"=>$GLOBALS['_SESSION']['openid']))->find()){
                $this->merchant_autologin();
            }
        }
  ;
    }
    ...
    public function merchant_autologin(){
        $database_merchant = D('Merchant');
        $merchant_wechat=   D("Merchant_wechat")->where(array("openid"=>$GLOBALS['_SESSION']['openid']))->find();
        $now_merchant=D("Merchant")->where(array("mer_id"=>$merchant_wechat["mer_id"]))->find();
    ...
    private function getallincomecount() {
        ...
        //echo $this->meal_orderDb->_sql();
        $meal_price = $tmp_m_price['tprice'] - $tmp_m_price['offprice'];
        $tmp_g_price = $this->group_orderDb->where('mer_id=' . $this->merid . ' AND paid="1" AND status != 3')->field('sum(total_money) as tprice,sum(wx_cheap) as offprice')->find();
        $group_price = $tmp_g_price['tprice'] - $tmp_g_price['offprice'];
        return ($meal_price + $group_price);
    }
...
// 			}
    ...
    public function staff_add() {
        if (IS_AJAX) {
            ...
            $data['username'] = $this->_post('username');
            $where['store_id'] = $this->_post['store_id'];
    ...
    public function staff_edit() {
        if (IS_AJAX) {
            ...
            if ($sql == false) {
                exit(json_encode(array('error' => '2', 'msg' => M('Merchant_store_staff')->getLastSql())));
            } else {
    ...
    public function login() {
        if ($this->isAjax()) {
            ...
            if ($database_merchant->data($data_merchant)->save()) {
                ...
                session('merchant_session', serialize($now_merchant));
                $referer = isset($_POST['referer']) ? htmlspecialchars_decode(urldecode($_POST['referer']), ENT_QUOTES) : '';
                if($this->is_wexin_browser){
                    if(D("Merchant_wechat")->where(array("openid"=>$GLOBALS['_SESSION']['openid']))->find()){

                    }else{

                        D("Merchant_wechat")->data(array("mer_id"=>$now_merchant['mer_id'],"openid"=>$GLOBALS['_SESSION']['openid'],"addtime"=>time(),"modifytime"=>time()))->add();
                    }
                }


                if(!empty($referer)){
                    exit(json_encode(array('error' => '0', 'msg' => '登录成功,现在跳转~', 'dom_id' => 'account','url'=>$referer)));
                }
                exit(json_encode(array('error' => '0', 'msg' => '登录成功,现在跳转~', 'dom_id' => 'account')));
            ...
            if(!empty(session("merchant_session"))&&$_GET['referer']){
              //  echo  $_GET['referer'] ;die;
                redirect(htmlspecialchars_decode(urldecode($_GET['referer']), ENT_QUOTES));
            }else{
    ...
    public function mer_reg() {
        if (IS_AJAX && IS_POST) {
            ...
            //帐号
            $database_merchant = D('Merchant');
            $condition_merchant['account'] = $_POST['phone'];
