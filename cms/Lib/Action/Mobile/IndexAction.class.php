...
//          $condition_where= " AND `area_id` in (".$area_idstr.")";
        ...
        if(!empty($condition_where)) {
            $count=D("Merchant")->field("mer_id,pic_info,name,person_name,merchant_theme_image")->limit(3)->where("`share_open`='1' AND `status`='1' AND merchant_theme_image <> ''" . $condition_where)->count();
            if(($count<3)&&($count!=0)){
                $oldthreeTuiMerchant = D("Merchant")->field("mer_id,pic_info,name,person_name,merchant_theme_image")->limit(3)->where("`share_open`='1' AND `status`='1' AND merchant_theme_image <> ''" . $condition_where)->order("reg_time desc")->select();
                $newthreeTuiMerchant = D("Merchant")->field("mer_id,pic_info,name,person_name,merchant_theme_image")->limit((3-$count))->where("`share_open`='1' AND `status`='1' AND merchant_theme_image <> ''")->order("reg_time desc")->select();
                $threeTuiMerchant=array_merge($oldthreeTuiMerchant,$newthreeTuiMerchant);
            }elseif($count>=3){
                $threeTuiMerchant = D("Merchant")->field("mer_id,pic_info,name,person_name,merchant_theme_image")->limit(3)->where("`share_open`='1' AND `status`='1' AND merchant_theme_image <> ''" . $condition_where)->order("reg_time desc")->select();
            }else{
                $threeTuiMerchant = D("Merchant")->field("mer_id,pic_info,name,person_name,merchant_theme_image")->limit(3)->where("`share_open`='1' AND `status`='1' AND merchant_theme_image <> ''")->order("reg_time desc")->select();
            }
        }else{
            $threeTuiMerchant = D("Merchant")->field("mer_id,pic_info,name,person_name,merchant_theme_image")->limit(3)->where("`share_open`='1' AND `status`='1' AND merchant_theme_image <> ''")->order("reg_time desc")->select();
        }

       // echo  json_encode($threeTuiMerchant);die;
        //转换图片
        if(!empty($threeTuiMerchant)) {
            $Merchant_image_class = new Merchant_image();
            foreach ($threeTuiMerchant as $k => $val) {
