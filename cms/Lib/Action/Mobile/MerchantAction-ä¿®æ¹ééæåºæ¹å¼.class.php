...
class MerchantAction extends BaseAction{
    ...
    public function getGroups(){
        ...
        $group_list = D('')->field('`g`.`name` AS `group_name`,`m`.`name` AS `merchant_name`,`g`.*,`m`.*')->table(array(C('DB_PREFIX').'group'=>'g',C('DB_PREFIX').'merchant'=>'m'))->where($condition_where)->order($order)->limit($p->firstRow,$p->listRows)->select();
    ...
    public function getPicturesList(){
        $mer_id=$_GET['mer_id']=81;
        if(empty($mer_id)){
            $this->error("未获取到农场id!");
        }
        $result=D("Merchant")->where("mer_id=".intval($mer_id))->find();
        $merchant_image_class = new merchant_image();
        $result['images']=$merchant_image_class->get_allImage_by_path($result['pic_info']);
        print_r($result);
        $this->display();
    }
    //获取特卖地址
