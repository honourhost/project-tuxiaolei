 ...
 * Time: 11:18
 */
class Newindex2Action extends NewbaseAction
{
    ...
    public function  header(){
        ...
        $data[2+$goodcount]=array("styleID"=>"goodrecommend_00","header"=>1,"headertype"=>"2","title"=>"精品推荐","subtitle"=>"最新鲜，最营养，最健康，最爱你！","image"=>$this->logo,"link"=>array(),"items"=>$goodtuijian);
...
//        }
        ...
        foreach ($appcatesix as $kk=>$vv){
            $data[3+$goodcount+$kk]=$vv;
        }
        $data[3+$goodcount+$goodcountsix]=array("styleID"=>"farmrecommend_01","header"=>1,"headertype"=>"2","title"=>"农场推荐","subtitle"=>"科学管理，生态种植，我负责食材，您享受美味！","image"=>$this->logo,"link"=>array(),"items"=>$merchants);
...
//        );
    ...
    public function  getallgoods(){
        ...
        if($group_list){
            ...
            $result=array("styleID"=>"youlike_02","header"=>1,"headertype"=>"2","title"=>"猜你喜欢","subtitle"=>"你不说，我都懂  你想吃，我全有","image"=>$this->logo,"link"=>array(),"items"=>$data);
    ...
    public function  searchMenulist()
    {
        ...
        $now_time = $_SERVER['REQUEST_TIME'];
        if(empty($now_category["cat_fid"])){
            $condition=" is_group_buy =0 AND type=1 AND begin_time < ".$now_time." AND end_time > ".$now_time." AND mer_id <> 747 AND group_id<>3428 AND mer_id <> 621 AND mer_id <> 821 AND cat_fid =".$now_category['cat_id'];

        }else{
            $condition=" is_group_buy =0 AND type=1 AND begin_time < ".$now_time." AND end_time > ".$now_time." AND mer_id <> 747 AND group_id<>3428 AND mer_id <> 621 AND mer_id <> 821 AND cat_id =".$now_category['cat_id'];
