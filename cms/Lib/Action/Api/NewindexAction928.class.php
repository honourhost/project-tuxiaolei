...
//        );
    ...
    public function  getallgoods(){
        ...
        $now_time = $_SERVER['REQUEST_TIME'];
        $condition_where= "`g`.`status`='1' AND `g`.`is_group_buy`='0' AND `g`.`type`='1' AND `g`.`begin_time`< ".$now_time." AND `g`.`end_time`> ".$now_time." AND `g`.`cat_id` <> 181 AND `g`.`mer_id` <> 890 AND `g`.`group_id` <> '3246' AND `g`.`group_id` <> '3254' ";
