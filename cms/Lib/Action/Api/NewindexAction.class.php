...
class NewindexAction extends NewbaseAction
{
    ...
    public function  header(){
        ...
        $data[0]=array("styleID"=>"slider_00","header"=>0,"headertype"=>"header_00","title"=>"顶部轮播","subtitle"=>"顶部轮播","image"=>$this->logo,"link"=>array(),"items"=>$result);
        //菜单
//        $menu = $this->getList($this->homeappcate, $area_id);
//         $menu = $this->getListnative($this->homeappcatenew, $area_id);
          $menu = $this->getListnativecat($this->homeappcate, $area_id);
          $data[1]=array("styleID"=>"menu_00","header"=>0,"headertype"=>"header_00","title"=>"主页菜单","subtitle"=>"主页菜单","image"=>$this->logo,"link"=>array(),"items"=>$menu);
    ...
    //    $data[1]=array("styleID"=>"slider_01","header"=>0,"headertype"=>"0","title"=>"跑马灯","subtitle"=>"怕吗等广告","image"=>$this->logo,"link"=>array(),"items"=>$textbanner);
        $appcatethree=$this->getAppcatethree();
        $goodcount=count($appcatethree);
        foreach ($appcatethree as $k=>$v){
            $data[2+$k]=$v;
        }
        //精品推荐
        //   $goodtuijian=$this->goodtuijian();
//        $goodtuijian=$this->getListnativegood(25);
//        $data[2+$goodcount]=array("styleID"=>"goodrecommend_00","header"=>1,"headertype"=>"1","title"=>"精品推荐","subtitle"=>"最新鲜，最营养，最健康，最爱你！","image"=>$this->logo,"link"=>array(),"items"=>$goodtuijian);
...
//        }
    ...
    //    $merchants=$this->getListfarm($this->homefarmrecommend);
        ...
        foreach ($appcatesix as $kk=>$vv){
            $data[2+$goodcount+$kk]=$vv;
        }
      //  $data[3+$goodcount+$goodcountsix]=array("styleID"=>"farmrecommend_00","header"=>1,"headertype"=>"1","title"=>"农场推荐","subtitle"=>"科学管理，生态种植，我负责食材，您享受美味！","image"=>$this->logo,"link"=>array(),"items"=>$merchants);
...
//        );
    ...
    private function getListnativecat($cat_id,$area_id){
//        $condition_where=array(
//            "cat_id"=>$cat_id,
//            "area_id"=>$area_id,
//            "status"=>1
        ...
        if(!empty($result)){
            foreach($result as $key=>$val){
                ...
                if($val["iswebjump"]==1){
                    ...
                    $data[$key]["link"]=array("platform"=>"category","url"=>$val["url"],"id"=>$val["url_id"],"title"=>$val["name"]);
...
//        );
    ...
    public function  goodtuijian(){
        ...
        $now_time = $_SERVER['REQUEST_TIME'];
        $condition_where= "`g`.`status`='1' AND `g`.`index_tui`='1' and `g`.`is_group_buy` = '0' AND `g`.`type`='1' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time' AND `g`.`cat_id` <> 181  AND `g`.`group_id` <> 3428 ";
    ...
    public function  getallgoods(){
        ...
        $now_time = $_SERVER['REQUEST_TIME'];
        $condition_where= "`g`.`status`='1' AND `g`.`is_group_buy`='0' AND `g`.`type`='1' AND `g`.`begin_time`< ".$now_time." AND `g`.`end_time`> ".$now_time." AND `g`.`cat_id` <> 181 AND `g`.`mer_id` <> 890 AND `g`.`mer_id` <> '1115' AND `g`.`group_id` <> '3246' AND `g`.`group_id` <> '3428' AND `g`.`group_id` <> '3254' ";
        ...
        if($group_list){
            ...
            foreach($group_list as $k=>$v){
                ...
                $data[$k]["placeoforigin"]=$v["placeoforigin"];
                $data[$k]["manufactureddate"]=$v["manufactureddate"];
                $data[$k]["barcodenumer"]=$v["barcodenumer"];
                $data[$k]["featuretile1"]=$v["featuretitle1"];
                $data[$k]["featuretile2"]=$v["featuretitle2"];
                $data[$k]["featuredesc1"]=$v["featuredesc1"];
                $data[$k]["featuredesc2"]=$v["featuredesc2"];
                //   $data[$k]["link"]=array("platform"=>"native","url"=>$v["group_id"]);
                $data[$k]["link"]=array("platform"=>"good","url"=>$v["group_id"],"id"=>$v["group_id"],"title"=>$v["s_name"]);
                $data[$k]["promotion"]=array("text"=>"","background"=>"");
            }
            $result=array("styleID"=>"youlike_00","header"=>1,"headertype"=>"header_00","title"=>"猜你喜欢","subtitle"=>"你不说，我都懂  你想吃，我全有","image"=>$this->logo,"link"=>array(),"items"=>$data);
            $this->returnAjax($result, 1,"",1,$pagecount);
        }else{
    ...
    public function  searchMenulist()
    {
        ...
        $now_time = $_SERVER['REQUEST_TIME'];
        if(empty($now_category["cat_fid"])){
            $condition=" is_group_buy =0 AND type=1 AND begin_time < ".$now_time." AND end_time > ".$now_time." AND mer_id <> 747 AND mer_id <> 621 AND mer_id <> 821 AND group_id<>3428 AND cat_fid =".$now_category['cat_id'];

        }else{
            $condition=" is_group_buy =0 AND type=1 AND begin_time < ".$now_time." AND end_time > ".$now_time." AND mer_id <> 747 AND mer_id <> 621 AND mer_id <> 821 AND group_id<>3428 AND cat_id =".$now_category['cat_id'];
...
//        $end[1]["items"]=$data;
    ...
    public function  searchMenulistnew(){
        ...
        $now_time = $_SERVER['REQUEST_TIME'];
        $page=empty($_GET['page'])?1:$_GET['page'];
        $condition=" is_group_buy = 0 AND type=1 AND begin_time < ".$now_time." AND end_time > ".$now_time." AND mer_id <> 747 AND mer_id <> 621 AND mer_id <> 821 AND group_id<>3428 AND tags LIKE '%;".$tagid.";%'";
        $condition1=" is_group_buy = 0 AND type=1 AND begin_time < ".$now_time." AND end_time > ".$now_time." AND mer_id <> 747 AND mer_id <> 621 AND mer_id <> 821 AND group_id<>3428 AND tags LIKE '".$tagid.";%'";
...
//        );
     ...
     * @return bool
     */
    public function  getAppcatethree(){
        $appcate=D("Appcate")->where(" status =1  AND isthree=1 ")->order("sort DESC")->select();
        if(empty($appcate)){
            return false;
        }else{
        ...
        foreach ($appcate as $kk=>$vv){
            $data[$kk]["styleID"]="at_banner_00";
            $data[$kk]["header"]=0;
            $data[$kk]["headertype"]="header_01";
            ...
            foreach ($vv["goods"] as $kkk=>$vvv){
                ...
                $item[$kkk]["featuretile1"]=$vvv["good"]["featuretitle1"];
                $item[$kkk]["featuretile2"]=$vvv["good"]["featuretitle2"];
                $item[$kkk]["featuredesc1"]=$vvv["good"]["featuredesc1"];
                $item[$kkk]["featuredesc2"]=$vvv["good"]["featuredesc2"];
