...
class VipAction extends BaseAction
{
    ...
    public function  index(){
        ...
        if(strpos($this->user_session['avatar'],"http")){

        }else{
            $avatarimage=new avatar_image();
            $this->user_session['avatar']=$avatarimage->get_image_by_path($this->user_session['avatar'],"http://www.xiaonongding.com")["image"];
    ...
    public function  exchangemoney(){
      ...
      //  echo  D()->getLastSql();die;
        ...
        $this->success_tips("恭喜您获取".$usercard["denomination"]."元VIP余额，赶紧去免费领取小农丁特色农产品吧！",U("Vip/vipgoods"));
    ...
    public function  vipgoods(){
       // echo json_encode($this->user_session);die;
        ...
        $condition_where= "`g`.`status`='1' AND `g`.`is_group_buy`='0' AND `g`.`type`='1' AND `g`.`begin_time`< ".$now_time." AND `g`.`end_time`> ".$now_time." AND `g`.`mer_id` = '629'";

        $condition_table = array(C('DB_PREFIX').'group'=>'g');

        $groups = D('')->table($condition_table)->where($condition_where)->limit(3)->select();
