...
class IndexAction extends BaseAction{
    ...
    public function phoneExist(){
        $phone=$_POST['mobile'];
        if(!preg_match("/^0{0,1}(13[0-9]|15[7-9]|153|156|17[0-9]|18[5-9])[0-9]{8}$/",$phone)){
    ...
    public function sendVerifyCode(){
        ...
        if(!preg_match("/^0{0,1}(13[0-9]|15[7-9]|153|156|17[0-9]|18[5-9])[0-9]{8}$/",$sms_data['mobile'])){
            $this->returnAjax("手机号格式不正确，请重新输入！",0);
        }
    ...
    public function sendNewVerifyCode(){
        ...
        // }
        if(!preg_match("/^0{0,1}(13[0-9]|15[7-9]|153|17[0-9]|156|18[5-9])[0-9]{8}$/",$sms_data['mobile'])){
            $this->returnAjax("手机号格式不正确，请重新输入！",0);
        }
        $sms_data['content']="您的验证码为：".$verify_code.",有效期为10分钟，请不要把验证码泄露给其他人。如非本人操作，可不用理会！";
        $result=Sms::sendVerifySms($sms_data,"","utf-8",$verify_code);
