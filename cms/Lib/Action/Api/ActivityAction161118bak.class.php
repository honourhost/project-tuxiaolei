...
//            $this->returnAjax("没有获取到数据",0);
            ...
            if($_SESSION['areaarr']!="all"){
                $now_time = $_SERVER['REQUEST_TIME'];
                $area_str=implode(',',$_SESSION['areaarr']);
                $condition_where= "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`type`='1'  AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time'  AND `m`.`status`='1' AND `m`.`area_id` in (".$area_str.")";
            }else{
                $now_time = $_SERVER['REQUEST_TIME'];
            ...
            $field_condition="`g`.`name` AS `product_name`,`g`.`s_name` AS `merchant_name`,`g`.pic,`g`.price,`g`.mer_id,'' AS `all_count`, `g`.virtual_num AS `part_count`, `g`.price AS `money`, '' AS 'type',
        '' AS `pigcms_id`,`g`.begin_time , `g`.end_time ,`g`.group_id";
            $condition_table = array(C('DB_PREFIX').'group'=>'g',C('DB_PREFIX').'merchant'=>'m');
            if($_SESSION['areaarr']!="all"){
                $count=D('')->field($field_condition)->table($condition_table)->where($condition_where)->count();
                if($count!=0){
                    $group_list = D('')->field($field_condition)->table($condition_table)->where($condition_where)->order("g.group_id desc")->limit(5)->select();
                }else{
                    $now_time = $_SERVER['REQUEST_TIME'];
                    $condition_where= "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`type`='1'  AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time'  AND `m`.`status`='1'";
                    $group_list = D('')->field($field_condition)->table($condition_table)->where($condition_where)->order("g.group_id desc")->limit(5)->select();
...
//7.26新增
    ...
    public  function  getGroupListByIds($catid,$cat_fid,$area_url){
        if($area_url!="all"){
            $now_time = $_SERVER['REQUEST_TIME'];
            $condition_where= "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`type`='1' AND `g`.`cat_id`='$catid' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time'  AND `m`.`status`='1' AND `m`.`area_id` in (".$area_url.")";
        }else{
            $now_time = $_SERVER['REQUEST_TIME'];
            $condition_where= "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`type`='1' AND `g`.`cat_id`='$catid' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time'  AND `m`.`status`='1'";
        ...
        if($area_url!="all"){
            $count=D('')->field($field_condition)->table($condition_table)->where($condition_where)->count();
            if($count!=0){
                $group_list = D('')->field($field_condition)->table($condition_table)->where($condition_where)->order("g.index_tui desc")->limit(20)->select();
            }else{
                $now_time = $_SERVER['REQUEST_TIME'];
                $condition_where= "`g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`type`='1' AND `g`.`cat_id`='$catid' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time'  AND `m`.`status`='1'";
                $group_list = D('')->field($field_condition)->table($condition_table)->where($condition_where)->order("g.index_tui desc")->limit(20)->select();
     ...
     * 获取跟官网类似的一样的商品内容
     */
    public function  getNewTypeActivity(){
        $category_url=$_GET["cat_url"];
        $area_url=$_GET["area_url"];
        if(!empty($category_url)){
            $category=     D("group_category")->get_category_by_catUrl($category_url);
            $catid=$category['cat_id'];
            $cat_fid=$category['cat_fid'];
            $this-> getGroupListByIds($catid,$cat_fid,$area_url);
