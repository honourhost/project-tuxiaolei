...
class CashAction extends BaseAction{
    public function index(){
        ...
        $cash_list=$Bank_cash_info->alias("b")->field("b.status as cash_status,b.*,m.*")->join(C('DB_PREFIX')."merchant as m ON b.mer_id=m.mer_id")->limit($p->firstRow . ',' . $p->listRows)->order("b.create_time desc")->select();
        $pagebar = $p->show();
        ...
        if (!empty($_POST)) {
            ...
            if($result){
                ...
                if ($res) {
                    ...
                    $this->frame_submit_tips(0,"提现操作记录失败，但是已经修改为驳回状态！");
                }
            }else{
