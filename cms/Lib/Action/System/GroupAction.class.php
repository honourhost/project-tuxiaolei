...
// 			}
			...
			if (!$image['error']) {
              ...
              //  $this->frame_submit_tips(0,json_encode($_POST));
    ...
    /*待审核商品管理*/
    public function productdaishenhe(){
        //搜索
        $condition_where = "`gs`.`group_id`=`g`.`group_id`";
        if(empty($_GET['mer_id'])){
            $condition_where .= " AND `g`.`status`='2'";
    ...
    public function  group_refundvip(){
        ...
        if($now_order["status"]==3){
            $this->error('此订单已退款，请勿重复提交！');
        }

        $condition_group_order['order_id'] = $now_order['order_id'];
        $data_group_order['express_id'] = "已退货";
        $data_group_order['status'] = 3;


        if(D('Group_order')->where($condition_group_order)->data($data_group_order)->save()){
            D("User")->where(array("uid"=>$now_order['uid']))->setInc("cardmoney",round(floatval($now_order["balance_pay"]),2));
    ...
    protected function check_group($store_id){
        ...
        if(empty($_GET['keyword'])){
         ...
         //   echo  D()->getLastSql();die;
            ...
            $order_list = D('')->field('`o`.`phone` AS `group_phone`,`o`.*,`g`.`s_name`,`g`.`price` as g_price,`u`.`uid`,`u`.`nickname`,`u`.`phone`,`m`.`phone` as m_phone,`m`.`name` as m_name,`m`.`mer_id`,`g`.`group_id`,gb.status as gbstatus ,`g`.`remarks`')->group('`o`.`order_id`,`o`.`sun_id`')->where($condition_where)->table($condition_table)->order('`o`.`status` ASC,`o`.`pay_time` DESC')->limit($p->firstRow.','.$p->listRows)->select();
            if(empty($order_list)){
                $this->error_tips('当前'.$this->config['group_alias_name'].'并未产生订单！');
            }
	   ...
	   public function  temai_order_new (){
        ...
        $condition_table = array(C('DB_PREFIX').'group'=>'g', C('DB_PREFIX').'group_order'=>'o', C('DB_PREFIX').'user'=>'u', C('DB_PREFIX').'merchant'=>'m');
           ...
           if(empty($_GET['keyword'])){
            ...
            $order_list = D('')->field('`o`.`phone` AS `group_phone`,`o`.*,`g`.`s_name`,`g`.`price` as g_price,`u`.`uid`,`u`.`nickname`,`u`.`phone`,`m`.`phone` as m_phone,`m`.`name` as m_name,`m`.`mer_id`,`g`.`group_id`,`g`.`remarks`')->where($condition_where)->table($condition_table)->order('`o`.`status` ASC,`o`.`pay_time` DESC')->limit($p->firstRow.','.$p->listRows)->select();
            if(empty($order_list)){
                $this->error_tips('当前'.$this->config['group_alias_name'].'并未产生订单！');
            }
            ...
            $this->display();die;
               ...
               $order_list = D('')->field('`o`.`phone` AS `group_phone`,`o`.*,`g`.`s_name`,`g`.`price` as g_price,`u`.`uid`,`u`.`nickname`,`u`.`phone`,`m`.`phone` as m_phone,`m`.`name` as m_name,`m`.`mer_id`,`g`.`group_id`')->where($condition_where)->table($condition_table)->order('`o`.`status` ASC,`o`.`pay_time` DESC')->limit($p->firstRow.','.$p->listRows)->select();
               if(empty($order_list)){
                   $this->error_tips('当前'.$this->config['group_alias_name'].'并未产生订单！');
               }
