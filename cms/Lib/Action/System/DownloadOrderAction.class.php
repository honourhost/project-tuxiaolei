...
class DownloadOrderAction extends BaseAction{
   ...
   foreach ($row as $i => $v) {
    ...
    public function  exportGroup(){
        ...
        $stmt = M("Group")->alias("go")->field("go.group_id,go.is_group_buy,go.mer_id,go.name,go.s_name,go.intro,go.old_price,go.price,go.cat_id,go.cat_fid,go.content")->limit(1800)->select();
        // 打开PHP文件句柄，php://output 表示直接输出到浏览器
        $fp = fopen('php://output', 'a');

        // 输出Excel列名信息
        $head = array("商品id","是否拼团","农场id","名称","短名称","介绍","原价","现价","团长id","分类id","分类父id","描述");
        foreach ($head as $i => $v) {
            // CSV的Excel支持GBK编码，一定要转换，否则乱码
            $head[$i] = iconv('utf-8', 'gbk', $v);
        }
    ...
    public function  exportGroupOrder(){
        ...
        $stmt = M("Group_order")->alias("go")->field("go.order_id,go.order_name,go.num,go.price,go.total_money,go.contact_name,go.phone,go.adress,go.express_id,FROM_UNIXTIME(go.add_time)")->limit(150000)->where(array("paid"=>1))->select();
        // 打开PHP文件句柄，php://output 表示直接输出到浏览器
        $fp = fopen('php://output', 'a');

        // 输出Excel列名信息
        $head = array("订单id","订单名称","数量","价格","付款总价","姓名","电话","地址","快递单号","下单时间");
        foreach ($head as $i => $v) {
            // CSV的Excel支持GBK编码，一定要转换，否则乱码
            $head[$i] = iconv('utf-8', 'gbk', $v);
        }
