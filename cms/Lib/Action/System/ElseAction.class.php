...
class ElseAction extends BaseAction{
 ...
 //echo  json_encode($marketers);die;
    ...
    public function allstatis(){
        ...
        $lastmonthtime=mktime(0, 0, 0, date("m")-1, 1, date("Y"));
        $smonthtime = mktime(0, 0, 0, date("m"), 1, date("Y"));
        $emonthtime = mktime(0, 0, 0, date("m") + 1, 1, date("Y"));

        $totallastmonth = $grouporder->field('sum(payment_money) as totalmoney')->where("`paid`=1 AND `add_time`>'$lastmonthtime' AND `add_time`<'$smonthtime'")->select();
        $totalimportlastmonth = $grouporder->field('sum(payment_money) as totalmoney')->where("`paid`=1 AND `add_time`>'$lastmonthtime' AND `add_time`<'$smonthtime' AND `mer_id` = 812 ")->select();
        //  return $total[0]["totalmoney"];
        echo "<br>上月实际特卖流水".($totallastmonth[0]['totalmoney']-$totalimportlastmonth[0]['totalmoney'])."元"."<br>";
