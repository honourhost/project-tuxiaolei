...
class FastCashAction extends BaseAction
{
    ...
    public function index(){
        ...
        $cash_list=$Bank_cash_info->alias("b")->field("b.status as cash_status,b.*,m.*,mar.name as marname")->join(C('DB_PREFIX')."merchant as m ON b.mer_id=m.mer_id")->join(C('DB_PREFIX')."marketers as mar ON m.marketer_id =mar.id")->limit($p->firstRow . ',' . $p->listRows)->order("b.status asc ")->select();
