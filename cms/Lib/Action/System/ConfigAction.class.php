...
class ConfigAction extends BaseAction {
    public function index(){	
		...
		foreach($group_list as &$gListValue){
			$gListValue['gname'] = str_replace('农小店',$this->config['meal_alias_name'],$gListValue['gname']);
			$gListValue['gname'] = str_replace('特卖',$this->config['group_alias_name'],$gListValue['gname']);
		}
		...
		foreach($tmp_config_list as $key=>$value){
			$value['info'] = str_replace('农小店',$this->config['meal_alias_name'],$value['info']);
