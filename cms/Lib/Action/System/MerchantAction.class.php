...
// 		$this->assign(D("Meal_order")->get_order_by_mer_id($mer_id, 1));
	...
	public function fastorder(){
        ...
        $mer_id = isset($_GET['mer_id']) ? intval($_GET['mer_id']) : 0;
        import('@.ORG.system_page');
        $count=D("FastOrder")->where(array("paid"=>1,"status"=>1,"mer_id"=>$mer_id))->count();
        $p = new Page($count, 20);
        $sql="SELECT * from pigcms_fast_order WHERE  paid =1 AND  status =1 AND mer_id= ".$mer_id." ORDER BY order_id DESC LIMIT {$p->firstRow}, {$p->listRows}";
        $result=M()->query($sql);
      $pagebar=  $p->show();
