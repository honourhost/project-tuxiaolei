...
class CrowdcashAction extends BaseAction
{
    ...
    public function  index(){
        ...
        $Bank_cash_info=D("Crowdcash");
        $count_cash=$Bank_cash_info->count();
        $p = new Page($count_cash,15);
        $cash_list=$Bank_cash_info->limit($p->firstRow . ',' . $p->listRows)->order("create_time DESC ")->select();
    ...
    public function finish(){
        $id=intval($_POST['id']);
        $cash=D("Crowdcash")->where("id=".$id)->find();
        if(empty($cash)){
            $this->error("不存在的提现申请！");
        }
        $result=D("Crowdcash")->where("id=".$id)->setField("status",1);
        D("User")->where(array("uid"=>$cash["uid"]))->setDec("distribute_money",$cash['cash_money']);
        //echo  D()->getLastSql();die;
        if($result){
            //成功后插入到提现记录表中

            D("Crowdcash")->where("id=".$id)->setField("modify_time",time());
...
//            }
    ...
    public function reject()
    {
        if (!empty($_POST)) {
            $id=$_POST['id'];
            $result = D("Crowdcash")->where("id=" . $id)->setField("status", 2);
            D("Crowdcash")->where("id=" . $id)->setField("note", $_POST['note']);
            if($result){
