...
class ActivityAction extends BaseAction{
	...
	public function activityType(){
		$database_extension_activity_type = D('Extension_activity_type');
		$type_list = $database_extension_activity_type->order('`id` DESC')->select();
		$this->assign('type_list',$type_list);
		$this->display();
	}
	//活动类型添加
	public function activityTypeAdd(){
		if(!empty($_POST)){
		$data['type_name']=$_POST['type_name'];
		$data['create_time']=time();
		$data['status']=1;
		if(empty($_POST['type_name'])){
			$this->error_tips("请先填写数据后再提交！",1,U("Activity/activityType"));
		}
		$database_extension_activity_type = D('Extension_activity_type');
		$return = $database_extension_activity_type->add($data);
		if($return){
			$this->frame_main_ok_tips('添加成功！',1,U("Activity/activityType"));
		}else{
			$this->error_tips('添加失败！',1,U("Activity/activityType"));
		}
		}else{
			$this->display();
		}
	...
	//活动类型修改
	public function activityTypeEdit(){
		if(!empty($_POST)){
		$id=$_POST['id'];
		if(empty($_POST['type_name'])){
			$this->error_tips("请先填写数据后再提交！");
		}
		$database_extension_activity_type = D('Extension_activity_type');
		$return = $database_extension_activity_type->where("id=".$id)->setField("type_name",$_POST['type_name']);
		if($return){
			$this->frame_main_ok_tips('修改成功！',1,U("Activity/activityType"));
		}else{
			$this->error_tips('修改失败！',1,U("Activity/activityType"));
		}
		}else{
			$id=$_GET['id'];
			$type=D('Extension_activity_type')->where("id=".$id)->find();
	...
	//活动类型删除
	public function activityTypeDel(){
		$id=$_GET['id'];
		$database_extension_activity_type = D('Extension_activity_type');
		$return = $database_extension_activity_type->where("id=".$id)->setField("status",0);
		if($return){
			$this->frame_main_ok_tips('删除成功！',1,U("Activity/activityType"));
		}else{
			$this->error_tips('删除失败！',1,U("Activity/activityType"));
		}
	}
	//活动类型恢复
	public function activityTypeRes(){
		$id=$_GET['id'];
		$database_extension_activity_type = D('Extension_activity_type');
		$return = $database_extension_activity_type->where("id=".$id)->setField("status",1);
		if($return){
			$this->frame_main_ok_tips('恢复成功！',1,U("Activity/activityType"));
		}else{
			$this->error_tips('恢复失败！',1,U("Activity/activityType"));
		}
	}
	public function add(){
		//找到活动的下一个时间段
