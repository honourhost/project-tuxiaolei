...
class MerchantAction extends BaseAction{
	...
	public function wait_verify_merchant(){
		...
		$p = new Page($count, 20);
		$res=D("Merchant_verify")->where("status=0")->limit($p->firstRow.','.$p->listRows)->select();
		$pagebar = $p->show();
		$this->assign('pagebar',$pagebar);
	...
	public function has_verify_merchant(){
		...
		$p = new Page($count, 20);
		$res=D("Merchant_verify")->where("status=1")->limit($p->firstRow.','.$p->listRows)->select();
		$pagebar = $p->show();
		$this->assign('pagebar',$pagebar);
		$this->assign("merchants",$res);
		$this->display();
	...
	public function wait_verify_person(){
		...
		$p = new Page($count, 20);
		$res=D("Person_verify")->where("status=0")->limit($p->firstRow.','.$p->listRows)->select();
		$pagebar = $p->show();
		$this->assign('pagebar',$pagebar);
	...
	public function has_verify_person(){
		...
		$p = new Page($count, 20);
		$res=D("Person_verify")->where("status=1")->limit($p->firstRow.','.$p->listRows)->select();
		$pagebar = $p->show();
		$this->assign('pagebar',$pagebar);
		$this->assign("persons",$res);
		$this->display();
	...
	public function deposite_merchant(){
        ...
        if (!empty($_GET['keyword'])) {
            if ($_GET['searchtype'] == 'order_id') {
                $condition_user['order_id'] = $_GET['keyword'];
            } else if ($_GET['searchtype'] == 'mer_id') {
                $condition_user['mer_id'] = $_GET['keyword'];;
            }
            $count=$deposite_merchant->where($condition_user)->count();
            import('@.ORG.system_page');
            $p = new Page($count, 15);
            $list = $deposite_merchant->where($condition_user)->order('`id` DESC')->limit($p->firstRow . ',' . $p->listRows)->select();
            $this->assign('merchant_list', $list);
            $pagebar = $p->show();
            $this->assign('pagebar', $pagebar);
        }else{
		$count=$deposite_merchant->where(array("status"=>0,"paid"=>1))->count();
		import('@.ORG.system_page');
		$p = new Page($count, 15);
		$list = $deposite_merchant->where(array("status"=>0,"paid"=>1))->order('`id` DESC')->limit($p->firstRow . ',' . $p->listRows)->select();
		$this->assign('merchant_list', $list);
		$pagebar = $p->show();
		$this->assign('pagebar', $pagebar);
        }
