...
class ChannelAction extends BaseAction{
    ...
    public function add(){
        if(IS_POST){
            $Channel=D("Channel");
            //保存分享图片
            if($_FILES['share_pic']['error'] != 4){

            $image = D('Image')->handle($this->system_session['id'], 'channelshare');
            if (!$image['error']) {
                $_POST = array_merge($_POST, str_replace('/upload/channelshare/', '', $image['url']));
            } else {
                $this->error("新增频道失败！");
            }
            }
        ...
        if(IS_POST){
            ...
            if($data=$Channel->create()){
                ...
                if($Channel->save($data)){
                    //处理图片
                    D('Image')->update_table_id('/upload/channelshare/' . $_POST['share_pic'], $_POST['id'], 'channelshare');
                    S('sharepic_list_'.$_POST['c_id'],NULL);
                    if($_POST['pic']){
                    if(strpos($now_slider['pic'],'2014/') === false){
                        unlink('./upload/channelshare/'.$now_slider['pic']);
                        }
                    }

                    $this->success("修改频道成功！",U("Channel/index"));
                }else{
                    $this->error("修改频道失败！");
                }
            }else{
        ...
        if(IS_POST){
            ...
            if($data=$Channel_cat->create()){
                ...
                if($Channel_cat->add($data)){
                   $this->success("频道分类添加成功！",U("Channel/catList",array("id"=>$data['c_id'])),true);
                }else{
                    $this->error("频道分类添加失败！");
                }
            }else{
        ...
        if(IS_POST){
            if($data=$Channel_cat->create()){
                ...
                if($Channel_cat->save($data)){
                    $this->success("频道分类修改成功！",U("Channel/catList",array("id"=>$data['c_id'])),true);
                }else{
                    $this->error("频道分类修改失败！");
                }
            }else{
        ...
        if(IS_POST){
            ...
            if($data=$Channel_category_goods->create()){
                    ...
                    if(!($this->goodsExist($data['goods_id']))){
                        $this->error("要添加的商品不存在或者被关闭或已添加！");
                    }
                    $data['create_time']=time();
                    $data['status']=1;
                    if($Channel_category_goods->add($data)){
                        $this->success("新增分类下商品成功！",U("Channel/catList", array("id" => $data['c_id'])),true);
                    }else{
        ...
        if(IS_POST){
            ...
            if($data=$Channel_category_goods->create()){
                ...
                if($Channel_category_goods->save($data)){
                    $this->success("编辑分类下商品成功！",U("Channel/catList", array("id" => $data['c_id'])),true);
                }else{
    ...
    public function delGoods(){
        $Channel_category_goods=D("Channel_category_goods");
        $id=intval($_POST['id']);
        if($Channel_category_goods->where("id=".$id)->delete()){
            $this->success("删除频道分类成功！");
        }else{
            $this->error("删除频道分类失败！");
        }
        ...
        if(IS_POST){
            ...
            if($data=$Channel_person->create()){
                ...
                if($Channel_person->add($data)){
                    ...
                    //查看是否有分销权限有的话不再添加
                    if(D("Distribution_person")->where("user_id=".$user_id)->find()){
                        $this->success("频道股东添加成功！",U("Channel/personList", array("id" => $data['c_id'])),true);
                    }
                    if(D("Distribution_person")->add($disdata)) {
                        $this->success("频道股东添加成功且分销权限也新增成功！", U("Channel/personList", array("id" => $data['c_id'])),true);
                    }else{
    ...
    private function goodsExist($id){
        ...
        if(!empty($res)){
            //判断是否已经在频道商品表中
            $result=D("Channel_category_goods")->where("goods_id=".$id)->find();
