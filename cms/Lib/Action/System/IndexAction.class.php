...
class IndexAction extends BaseAction {
    ...
    public function main() {
        ...
        $pigcms_assign['group_2016_order_count']=D('Group_order')->where("add_time < ".strtotime("2016-12-31 23:59:59")." ")->count();
        $mmm=D('Group_order')->field("SUM(payment_money) as moneysum")->where("add_time < ".strtotime("2016-12-31 23:59:59")."")->find();
        $pigcms_assign['group_2016_order_sum']=$mmm["moneysum"];
       // echo  json_encode($pigcms_assign['group_2016_order_sum']);die;
      ...
      $pigcms_assign['fast_order_sum']=$fast1[0]['totalm'];
        ...
        $pigcms_assign['fast_order_count_year']=D("FastOrder")->get_all_oreder_count("year");
	//	echo json_encode($pigcms_assign);die;
        //订餐统计
        ...
        if ($this->system_session['area_id']) {
            ...
            $pigcms_assign['merchant_verify_store_count'] = D('Merchant_store')->where(array('status' => 2, 'area_id' => $this->system_session['area_id']))->count();
            //团购待审核
...
// 		}
    ...
    public function sitemap() {
		$xmlfilepath = './'.str_replace('.','_www.xiaonongding.com').'sitemap.xml';
    ...
    private function exeGenerateFile($UrlSetArr) {
        if (!empty($UrlSetArr)) {
            $xmlfilepath = './'.'sitemap.xml';//.str_replace('.','_',$_SERVER['HTTP_HOST'])去掉和域名关联的内容
