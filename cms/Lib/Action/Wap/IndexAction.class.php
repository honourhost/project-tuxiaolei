...
// 		dump($allflash);
		...
		if (empty($info)) {
			...
			$groups = D("Group")->field(true)->where(array('mer_id' => $this->token, 'begin_time' => array('lt', time()), 'end_time' => array('gt', time()), 'status' => 1,"group_id"=>array("neq",3254),"group_id"=>array("neq",3428)))->select();
		...
		//2015.12.18新增内容 by sunny
		$store_id=$_GET["store_id"];
		$mer_id=$this->token;
		if(empty($store_id)){
			//如果没有店铺id传来则查询出来
			$currentstore=D("Merchant_store")->where(array("mer_id"=>$mer_id,"status"=>1))->order("last_time desc")->find();
			$store_id=$currentstore['store_id'];
		}else{
			$currentstore=D("Merchant_store")->where(array("mer_id"=>$mer_id,"status"=>1))->order("last_time desc")->find();
		}
		$this->assign("currentstore",$currentstore);
		$currentmerchant=D("Merchant")->where("mer_id=".$mer_id)->find();
		$merchant_image_class=new merchant_image();
		if (!empty($currentmerchant['person_image'])) {
			$currentmerchant["person_image"] = $merchant_image_class->get_image_by_path($currentmerchant['person_image']);
		}
		if (!empty($currentmerchant['merchant_theme_image'])) {
			$currentmerchant["merchant_theme_image"] = $merchant_image_class->get_image_by_path($currentmerchant['merchant_theme_image']);
		}

		$Group=D("Group");
		$Meal=D("Meal");

		$storenum=$Group->where(array("mer_id"=>$mer_id,"status"=>1))->count();
		$mealnum=$Meal->where(array("store_id"=>$store_id,"status"=>1))->count();

		//选出所有个特卖
		$group_list=$Group->where(array("mer_id"=>$this->token,"status"=>1,'begin_time' => array('lt', time()), 'end_time' => array('gt', time()),'is_group_buy'=>0,"group_id"=>array("neq",3254),"group_id"=>array("neq",3428)))->order("group_id DESC")->select();
		//将图片解析
		$group_image_class = new group_image();
		foreach($group_list as $key=>$val) {
			$group_list[$key]['all_pic'] = $group_image_class->get_allImage_by_path($val['pic']);
		}
		//普通拼团
        $group_list_pin=$Group->where(array("mer_id"=>$this->token,"status"=>1,'begin_time' => array('lt', time()), 'end_time' => array('gt', time()),'is_group_buy'=>1,'is_lottery_group_buy'=>0))->order("group_id DESC")->select();
        //将图片解析
        $group_image_class2 = new group_image();
        foreach($group_list_pin as $key=>$val) {
            $group_list_pin[$key]['all_pic'] = $group_image_class2->get_allImage_by_path($val['pic']);
        }

        //抽奖团
        $group_list_lottery=$Group->where(array("mer_id"=>$this->token,"status"=>1,'begin_time' => array('lt', time()), 'end_time' => array('gt', time()),'is_group_buy'=>1,'is_lottery_group_buy'=>1))->order("group_id DESC")->select();
        //将图片解析
        $group_image_class3 = new group_image();
        foreach($group_list_lottery as $key=>$val) {
            $group_list_lottery[$key]['all_pic'] = $group_image_class3->get_allImage_by_path($val['pic']);
        }

        //团长免单
        $group_list_first_free=$Group->where(array("mer_id"=>$this->token,"status"=>1,'begin_time' => array('lt', time()), 'end_time' => array('gt', time()),'is_group_buy'=>1,'is_first_free'=>1))->order("group_id DESC")->select();
        //将图片解析
        $group_image_class4 = new group_image();
        foreach($group_list_first_free as $key=>$val) {
            $group_list_first_free[$key]['all_pic'] = $group_image_class4->get_allImage_by_path($val['pic']);
        }
		$this->assign("group_list",$group_list);
        $this->assign("group_list_pin",$group_list_pin);
        $this->assign("group_list_lottery",$group_list_lottery);
        $this->assign("group_list_first_free",$group_list_first_free);
		//选出所有热卖
		//菜品分类
		...
		$meal_list=$Meal->where(array("store_id"=>$store_id,'sort_id' => array('in', $ids),"status"=>1))->order("sort asc")->limit(4)->select();
		//解析图片
		$meal_image_class = new meal_image();
		foreach($meal_list as $key=>$val){
			$meal_list[$key]['image']=$meal_image_class->get_image_by_path($val['image']);
		}
		$this->assign("meal_list",$meal_list);
		$this->assign("storenum",$storenum);
		$this->assign("mealnum",$mealnum);
		$this->assign("currentmerchant",$currentmerchant);

		$this->assign("store_id",$store_id);
		$this->assign("mer_id",$this->token);


        $crowdfundings=D("Crowdfunding")->field("title,digest,hit,webpic,mer_id,crowd_id,progress,invator,invatorpic,crowdplace,funds,total")->where(" status = 1  AND mer_id = ".$this->token)->page(1,8)->order("crowd_id DESC")->select();
        $group_image_class = new group_image();
        $merchant_image=new merchant_image();
        $merchant=D("Merchant");
        foreach ($crowdfundings as $key=>$value){
            $crowdfundings[$key]['invatorpic']=$group_image_class->get_image_by_path($crowdfundings[$key]['invatorpic'], 's');
