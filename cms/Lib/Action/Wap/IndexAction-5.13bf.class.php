...
// 		dump($allflash);
		...
		//选出3个特卖
		$group_list=$Group->where(array("mer_id"=>$this->token,"status"=>1))->order("sort asc")->limit(3)->select();
		//将图片解析
		$group_image_class = new group_image();
		...
		//选出4个热卖
		$meal_list=$Meal->where(array("store_id"=>$store_id,"status"=>1))->order("sort asc")->limit(4)->select();
		//解析图片
