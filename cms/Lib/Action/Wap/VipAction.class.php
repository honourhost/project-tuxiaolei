...
class VipAction extends BaseAction
{
    ...
    public function  exchangemoneyajax(){
        ...
        D("Vipcard")->where(array("unique_id"=>$code))->data(array("used"=>1,"usetime"=>time(),"uid"=>$this->user_session['uid']))->save();
        $sq=D()->getLastSql();
        D("Vipcardrecord")->data(array("uid"=>$now_user["uid"],"usetime"=>time(),"denomination"=>$usercard["denomination"]))->add();
        D("User")->where("uid = ".$now_user['uid'])->setInc("cardmoney",$usercard['denomination']);
        echo json_encode(array("status"=>1,"message"=>"恭喜您获取".$usercard["denomination"]."元VIP余额，赶紧去免费领取小农丁特色农产品吧！".$sq)); exit;

    }

    public function  vipgoods2(){
        if(empty($this->user_session)){
            $this->error_tips('请先进行登录！',U('Login/index'));
        }
