...
class ApiAction extends CommonAction
{
	...
	public function go_im()
	{
		...
		/*$url = 'http://im-link.winworlds.cn/?app_id=' . $this->config['im_appid'] . '&openid=' . $GLOBALS['_SESSION']['openid'] . '&key=' . $key . ($_GET['title'] ? '&title=' . $_GET['title'] : '') . '#' . $_GET['hash'];*/

        $url ='../../../wap.php?g=Wap&c=My&a=index';
