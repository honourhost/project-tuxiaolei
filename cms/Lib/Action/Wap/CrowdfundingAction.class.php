...
class CrowdfundingAction extends BaseAction
{
    ...
    public function index(){
        ...
        if($catid==-1){
            $crowdfundings=D("Crowdfunding")->field("title,digest,hit,webpic,mer_id,crowd_id,progress,invator,invatorpic,crowdplace")->where(" status = 1 ")->page(1,8)->order("crowd_id DESC")->select();
        }else{
            $crowdfundings=D("Crowdfunding")->field("title,digest,hit,webpic,mer_id,crowd_id,progress,invator,invatorpic,crowdplace")->where("crowdcat = ".$catid)->page(1,8)->order("crowd_id DESC")->select();
        }
    ...
    public function detail(){
        ...
        if($crowdfunding['leftday']<=0){
            $crowdfunding["leftday"]="已超时结束";
            $crowdfunding["overtime"]=1;
        }
      ...
      //  echo  json_encode($crowdfunding);die;
        $buyrecords=D('Crowdrecord')->where(array('crowd_id'=>$crowdid))->limit(7)->order("id DESC")->select();
        if(!empty($buyrecords)){
    ...
    public function  supporters(){
        ...
        $supporters=D("Crowdorder")->field("uid,payment_money,paytime")->where(array("crowd_id"=>$crowdid,"paid"=>1))->order("corder_id DESC")->select();
        $userdata=D("User");
        $avatar=new avatar_image();
        foreach ($supporters as $key=>$value){
            $user=$userdata->where(array("uid"=>$value["uid"]))->find();
            if(strpos($user['avatar'],'http')===0){
                $supporters[$key]["avator"]=$user['avatar'];
            }else{

                 $supporters[$key]["avator"]=$avatar->get_image_by_path($user['avatar'])['image'];
            }
