...
class PayAction extends BaseAction{
	public function check(){
		...
		if(!in_array($_GET['type'],array('group','meal','weidian','takeout', 'food','recharge','appoint','wxapp','cardrecharge',"jicaicard"))){
			$this->error_tips('订单来源无法识别，请重试。3');
		}
		if($_GET['type'] == 'group'){
        ...
        }else if($_GET["type"]=='jicaicard'){
            $now_order = D('Jicaicard')->get_pay_order($this->user_session['uid'],intval($_GET['order_id']));
        }
		else{
	...
	public function go_pay(){
		...
		if(!in_array($_POST['order_type'],array('group','meal','weidian','takeout','food','recharge','appoint','wxapp','cardrecharge','jicaicard'))){
			$this->error_tips('订单来源无法识别，请重试。2');
		}
		switch($_POST['order_type']){
			...
			case 'wxapp':
				...
				break;
            case 'jicaicard':
                $now_order = D('Jicaicard')->get_pay_order($this->user_session['uid'],intval($_POST['order_id']));
                break;
			default:
				$this->error_tips('非法的订单');
		}
		...
		if(($_POST['order_type'] != 'recharge')&&($_POST['order_type']!='cardrecharge')&&($_POST['order_type']!='jicaicard')){
			//优惠券
		...
		}else if($order_info['order_type']=='jicaicard'){
            $save_result = D('Jicaicard')->web_befor_pay($order_info,$now_user);
        }
...
// 		}
	...
	public function weixin_back(){
		switch($_GET['order_type']){
			...
			case 'wxapp':
				...
				break;
            case 'jicaicard':
                $now_order=D('Jicaicard')->get_order_by_id($this->user_session['uid'],intval($_GET['order_id']));
                break;
			default:
				$this->error_tips('非法的订单');
		}
		...
		if($go_query_param['error'] === 0){
			switch($_GET['order_type']){
				...
				case 'wxapp':
					...
					break;
                case 'jicaicard':
                    D('Jicaicard')->after_pay($go_query_param['order_param']);
                    break;
			}
		}
		switch($_GET['order_type']){
  ...
  //       fclose($fo);
		if(empty($get_pay_param['error'])){
			...
			}else if($get_pay_param['order_param']['order_type'] == 'jicaicard'){
                $pay_info = D('Jicaicard')->after_pay($get_pay_param['order_param']);
            }
			else{
