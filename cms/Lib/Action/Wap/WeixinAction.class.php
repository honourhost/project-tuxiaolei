...
class WeixinAction extends CommonAction
{
	public function index()
	{
		...
		//	file_put_contents("justfortest2.txt",json_encode($this->config),FILE_APPEND);
		$data = $wechat->request();
//	file_put_contents("justfortest.txt",json_encode($data),FILE_APPEND);
	...
	private function scan($id, $openid = '', $issubscribe = 0)
	{
		if($id==66588){
			//先查看是否已经绑定用户，没有的话先绑定
			return array('<a href="' .$this->config['site_url'].U('Ticket/weixin',array("secret"=>$id)).'">点击领取100元奖券</a>', 'text');
		}
		...
		if ($recognition = M('Recognition')->field(true)->where(array('id' => $id))->find()) {
			...
			case 'merchant':
				...
				$return[] = array('[农场主页]' . $now_merchant['name'], $now_merchant['txt_info'], $pic, $this->config['site_url'] . '/wap.php?g=Wap&c=Index&a=index&token=' . $recognition['third_id']);
	...
	private function other_message($return, $token, $group_id = 0, $store_id = 0)
	{
		...
		// }
		//扫描结果改为农场主页
		$now_merchant = D('Merchant')->field(true)->where(array('mer_id' => $token))->find();
				$pic = '';
		...
		$return[] = array('[农场主页]' . $now_merchant['name'], $now_merchant['txt_info'], $pic, $this->config['site_url'] . '/wap.php?g=Wap&c=Index&a=index&token=' . $token);
	...
	private function special_keyword($key, $data = array())
	{
		...
		if (($key == '附近' . $this->config['group_alias_name']) || ($key == '附近' . $this->config['meal_alias_name'])) {
			...
			return array('主人【小农丁】已经接收到你的指令请发送您的地理位置(对话框右下角点击＋号，然后点击“位置”)给我哈', 'text');
		}
