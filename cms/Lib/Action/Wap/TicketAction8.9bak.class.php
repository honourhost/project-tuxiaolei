...
class TicketAction extends BaseAction{
        ...
        if(empty($result['error_code'])){
            ...
            if($now_user['is_receive']=="ticket"){
                $this->error_tips('您已经注册并参加过了无法再参加此次活动！',U('Login/index'));
                 ...
                 $data=array(
                        "now_money"=>($now_user['now_money']+10),
                        "is_receive"=>"ticket"
                        );
                $res=D('User')->where("uid=".$now_user['uid'])->setField($data);
                if($res){
                    D('User_money_list')->add_row($now_user['uid'],1,10,"领取10元现金券");
     ...
     * 6.18 活动优惠券
     */
    public function weixin618(){
       // echo  date("Y-m-d");die;
        if(date("Y-m-d")!='2017-06-18'){
     //       $this->error_tips('该活动限618当天参与！',U('Home/index'));
        }

        $_SESSION['weixin']['key']=20170618;
        $_SESSION['weixin']['referer'] = !empty($_GET['referer']) ? htmlspecialchars_decode($_GET['referer']) : U('My/index',array("secret618"=>'20170618'));
        $_SESSION['weixin']['state']   = md5(uniqid());

        $customeUrl = $this->config['site_url'].'/wap.php?c=Ticket&a=weixin_back618';
        $oauthUrl='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$this->config['wechat_appid'].'&redirect_uri='.urlencode($customeUrl).'&response_type=code&scope=snsapi_userinfo&state='.$_SESSION['weixin']['state'].'#wechat_redirect';
        redirect($oauthUrl);

}

    public function weixin_back618(){
        $referer = !empty($_SESSION['weixin']['referer']) ? $_SESSION['weixin']['referer'] : U('My/index',array("secret618"=>$_SESSION['weixin']['key']));
        ...
        if (isset($_GET['code'])){
            ...
            /*优先使用 unionid 登录*/
            if(!empty($jsonrt['unionid'])){
                $this->autologin618('union_id',$jsonrt['unionid'],$referer);
            }
            /*再次使用 openid 登录*/
            $this->autologin618('openid',$jsonrt['openid'],$referer);
    ...
    protected function autologin618($field,$value,$referer){
        $result = D('User')->autologin($field,$value);
        $couponrecord=D("Couponrecord");
        if(empty($result['error_code'])){
            $now_user = $result['user'];
            session('user',$now_user);
            //判断is_receive字段值是否为ticket
            if($couponrecord->where(array("uid"=>$now_user["uid"]))->find()){
                $this->error_tips('活动中每个用户只能领取一次优惠券哦~，赶紧去兑换频道看看吧！','http://www.xiaonongding.com/wap.php?g=Wap&c=Topic&a=index&topic=618&share_distribution_id=VmRwtgHOomY_870&from=singlemessage');
            }else{
                $data=array(
                    "now_money"=>($now_user['now_money']+10)
                );
                $res=D('User')->where("uid=".$now_user['uid'])->setField($data);
                if($res){
                    D("Couponrecord")->data(array("uid"=>$now_user["uid"],"addtime"=>time()))->add();
                    D('User_money_list')->add_row($now_user['uid'],1,10,"领取10元现金券");
                    $this->success_tips('恭喜您成功获得了奖券!',$referer);
                }else{
                    $this->error_tips('领取奖券失败，请联系管理员!',U('Login/index'));
