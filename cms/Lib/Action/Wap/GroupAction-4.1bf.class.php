...
// 		}
	...
	public function getOrderList(){
		...
		$mer_id=intval($_POST['mer_id']);

		$where="go.group_id=".$group_id." AND go.mer_id=".$mer_id." AND go.paid=1 AND (go.user_confirm=1 OR go.status=2)";
		...
		$p = new Page($count_group_order,2,'page');

		$group_order_list=D('Group_order')->alias("go")->join(C('DB_PREFIX')."user u ON go.uid=u.uid")->where($where)->limit($p->firstRow.','.$p->listRows)->select();
		$total = ceil($count_group_order/2);
