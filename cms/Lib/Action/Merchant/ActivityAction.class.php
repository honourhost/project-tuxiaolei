...
class ActivityAction extends BaseAction{
	...
	public function add(){
		if(IS_POST){
			...
			if(empty($_POST['activity_type_id'])){
				$this->error('请选择活动类别的id');
			}
			...
			if($this->config['activity_sign_term']){
				...
				$activity_term_list[0] = D('Extension_activity_type')->where(array('end_time'=>array('gt',$_SERVER['REQUEST_TIME']),'status'=>'1'))->find();		
			}
			if(empty($activity_term_list) || empty($activity_term_list[0])){
				$this->error('平台暂时没有开启的活动！');
			}
			$type_list=D("Extension_activity_type")->where("status=1")->limit(5)->order("create_time desc")->select();
	...
	public function frame_edit(){
		...
		if(IS_POST){
			...
			$type_list=D("Extension_activity_type")->where("status=1")->order("`create_time` desc")->select();
			$this->assign("type_list",$type_list);
