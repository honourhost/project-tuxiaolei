...
class CrowdorderAction extends  BaseAction
{
    ...
    public function  orderlist(){
        ...
        $condition_where="`co`.`uid`=`u`.`uid` AND `co`.`mer_id`=`m`.`mer_id` AND `co`.`cgrade_id`=`cg`.`grade_id` AND `co`.`crowd_id`=`cf`.`crowd_id` AND `co`.`paid`='1' AND `co`.`crowd_id` = ".$crowdid." AND `cg`.`is_lottery_grade` = 1 AND `co`.`mer_id`= ".$this->merchant_session["mer_id"];
    ...
    public function  lottery_orderlist(){

        $crowdid=$_GET["crowd_id"];
        $condition_table = array(C('DB_PREFIX').'crowdorder'=>'co', C('DB_PREFIX').'crowdfunding'=>'cf',C('DB_PREFIX').'crowdgrade'=>'cg', C('DB_PREFIX').'user'=>'u', C('DB_PREFIX').'merchant'=>'m');
        $condition_where="`co`.`uid`=`u`.`uid` AND `co`.`mer_id`=`m`.`mer_id` AND `co`.`cgrade_id`=`cg`.`grade_id` AND `co`.`crowd_id`=`cf`.`crowd_id` AND `co`.`paid`='1' AND `co`.`crowd_id` = ".$crowdid." AND `cg`.`is_lottery_grade` = 2 AND `co`.`lottery` = '1' AND `co`.`mer_id`= ".$this->merchant_session["mer_id"];
