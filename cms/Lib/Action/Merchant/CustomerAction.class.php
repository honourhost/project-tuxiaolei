...
// 		$fans_count = D('')->table($table)->where($condition)->count();
		...
		$sql = "SELECT u.*, m.* FROM " . C('DB_PREFIX') . "merchant_user_relation AS m LEFT JOIN " . C('DB_PREFIX') . "user AS u ON `m`.`openid`=`u`.`openid` WHERE {$where}  ORDER BY u.uid DESC LIMIT {$p->firstRow}, {$p->listRows}";
		...
		foreach($fans_list as $key=>$val){
			if(!empty($val['avatar'])){
					if(!strstr($val['avatar'],"http://")){
						...
						$fans_list[$key]['avatar']=$res['image'];
					}
				}
		}
// 		$fans_list = D('')->table($table)->where($condition)->limit($p->firstRow.','.$p->listRows)->select();
	...
	private function topost()
	{
		...
		$todata = json_decode($this->https_request('http://im-link.winworlds.cn/api/app_service.php', $data), true);
