...
class WeixinAction extends BaseAction
{
	...
	private function getToken(){
		if (!$this->weixintoken) {
			import('ORG.Net.Http');
			$this->weixinbind = D('Weixin_bind')->where(array('mer_id' => $this->merchant_session['mer_id']))->find();
			$url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=' .
					$this->weixinbind['authorizer_appid'] . '&secret=' .
					$this->weixinbind['authorizer_refresh_token'];
			$result = Http::curlGet($url);
			$result = json_decode($result, true);
			if (!empty($result['errcode']))
				exit($result['errmsg']);
			$data = $result['access_token'];
			$this->weixintoken=$data;
		}else{
			$data=$this->weixintoken;
		}
		return $data;
	}
	...
	public function index()
	{
		...
		if ($weixin_bind = D('Weixin_bind')->where(array('mer_id' => $this->merchant_session['mer_id']))->find()) {
			...
			if (empty($auth_code['errcode'])) {
				...
				$this->assign('url', '/merchant.php?g=Merchant&c=weixin&a=create_account');
			}
		}
	...
	public function create_account(){
		if(!empty($_POST)){
			if(!isset($_POST['pigcms_id'])) {
				$data['mer_id'] = $this->merchant_session['mer_id'];
				$data['qrcode_url'] = $_POST['url'];
				$data['authorizer_appid'] = $_POST['authorizer_appid'];
				$data['authorizer_refresh_token'] = $_POST['authorizer_refresh_token'];
				$data['func_info'] = $_POST['func_info'];
				$data['user_name'] = $_POST['wechat_encodingaeskey'];
				$data['service_type_info'] = $_POST['service_type_info'];
				$weixin_bind = D('Weixin_bind')->add($data);
			}else{
				...
				$data['user_name'] = $_POST['wechat_encodingaeskey'];
				$data['service_type_info'] = $_POST['service_type_info'];
				$weixin_bind = D('Weixin_bind')->save($data);
			}
			if($weixin_bind) {
				header("Location:/merchant.php?g=Merchant&c=Weixin&a=index");
			}else{
				$this->error('添加失败', "/merchant.php?g=Merchant&c=Weixin&a=index");
			}
		}else{

			if($weixin_bind = D('Weixin_bind')->where(array('mer_id' => $this->merchant_session['mer_id']))->find()){
			$this->error("已经存在绑定的微信号，禁止重新创建！");
			}
			$now_merchant=D("Merchant")->where("mer_id=".$this->merchant_session['mer_id'])->find();
			if(!empty($now_merchant)){
				$this->assign("token",$now_merchant['account'].$this->merchant_session['mer_id']);
			}else{
				$this->error("出错了！");
			}
			$this->assign("mer_id",$this->merchant_session['mer_id']);
			$this->display();
		}
	}
...
// 			if ($val['url'] && !strstr($val['url'], $this->config['site_url'])) exit(json_encode(array('errcode' => 1, 'errmsg' => 'URL地址不合法')));
	...
	public function testmenu()
	{
		...
		$data = file_get_contents('https://api.weixin.qq.com/cgi-bin/menu/get?access_token='.$token_data);
...
// 		if (IS_GET) {
			$token_data = $this->getToken();
			if (empty($token_data)) exit(json_encode($token_data));
...
// 			$authorizer_access_token = $json->access_token;
			//file_get_contents('https://api.weixin.qq.com/cgi-bin/menu/delete?access_token='.$this->getToken());
			$url='https://api.weixin.qq.com/cgi-bin/menu/create?access_token='.$token_data;
			import('ORG.Net.Http');
