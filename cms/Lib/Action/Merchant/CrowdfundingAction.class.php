...
class CrowdfundingAction extends BaseAction
{
    ...
    public function addaction()
    {
        if (IS_POST) {
            ...
            if(empty($_POST["invatorpic"])){
                $this->error('请上传众筹发起人头像');
            }
            if(empty($_POST["invator"])){
                $this->error('请填写众筹发起人');
            }
            ...
            if($crowdid=$database->data(array("title"=>$_POST["title"],"digest"=>$_POST["digest"],"funds"=>$_POST["funds"],"webpic"=>$_POST["webpic"],"content"=>$_POST["content"],"starttime"=>strtotime($_POST["starttime"]),
                "endtime"=>strtotime($_POST["endtime"]),"mer_id"=>$this->merchant_session["mer_id"],"video"=>$_POST["videourl"],"videopic"=>$_POST["videopic"],"crowdplace"=>$_POST["crowdplace"],"crowdcat"=>$_POST["crowdcat"],"invator"=>$_POST["invator"],"invatorpic"=>$_POST["invatorpic"]))->add()){
    ...
    private function  insertgrade($grade,$crowdid,$merid){
        ...
        if($grade[5]==0){
            $database->data(array("crowd_id"=>$crowdid,"title"=>$grade[0],"digest"=>$grade[1],
                "price"=>$grade[2],"limitnum"=>$grade[3],"mer_id"=>$merid,"is_lottery_grade"=>intval($grade[4]),"lottery_num"=>$grade['6']))->add();
        }else{
            $database->where("grade_id = ".$grade[5])->data(array("crowd_id"=>$crowdid,"title"=>$grade[0],"digest"=>$grade[1],
                "price"=>$grade[2],"limitnum"=>$grade[3],"mer_id"=>$merid,"is_lottery_grade"=>intval($grade[4]),"lottery_num"=>$grade['6']))->save();
        }
    ...
    public function  edit(){
        ...
        $now_group["invatorpicture"]=$group_image_class->get_image_by_path($now_group['invatorpic'],'s');
    ...
    public function  editaction(){
        if (IS_POST) {
            ...
            if($database->where(array("crowd_id"=>$_POST["crowd_id"]))->data(array("title"=>$_POST["title"],"digest"=>$_POST["digest"],"funds"=>$_POST["funds"],"webpic"=>$_POST["webpic"],"content"=>$_POST["content"],"starttime"=>strtotime($_POST["starttime"]),
                "endtime"=>strtotime($_POST["endtime"]),"mer_id"=>$datacrowd['mer_id'],"video"=>$_POST["videourl"],"videopic"=>$_POST["videopic"],"crowdplace"=>$_POST["crowdplace"],"crowdcat"=>$_POST["crowdcat"],"update"=>time(),"invator"=>$_POST["invator"],"invatorpic"=>$_POST["invatorpic"]))->save()){
                for ($i=1;$i<=$gradeindex;$i++){
                    if(!($this->isgradenotempty($_POST["grade".$i]))){
                       // $database->where(array("crowd_id"=>$crowdid))->delete();
                        $this->error("请填写完整的众筹档次信息");
