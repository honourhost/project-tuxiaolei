    ...
    public function fbintroduce() {
        ...
        if (IS_POST) {
            ...
            if ($insert_id > 0) {
             ...
             if ($id > 0) {
                unset($_POST['mer_id']);
                $exit = $merchant_inDb->where(array('module_id' => $_POST['module_id'], 'mer_id' => $this->mer_id,"id"=>array('neq',$id)))->find();
                if(empty($exit)) {
                    $insert_id = $merchant_inDb->where(array('id' => $id, 'mer_id' => $this->mer_id))->save($_POST);
                }else{
                    $this->error('保存失败！选择的模板下已经存在内容');
                }
            } else {
                $exit = $merchant_inDb->where(array('module_id' => $_POST['module_id'], 'mer_id' => $this->mer_id))->find();
                if(empty($exit)) {
                    $_POST['addtime'] = time();
                    $insert_id = $merchant_inDb->data($_POST)->add();
                }else{
                    $this->error('保存失败！一个模板下只能添加一次内容');
                }
            }
            if ($insert_id > 0) {
                $this->success('保存成功！', $notitle ? U('Frontmanag/navmanag') : U('Frontmanag/introduce'));
