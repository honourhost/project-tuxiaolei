...
class CountAction extends BaseAction{
	...
	public function bill()
	{
        ...
        $crowdtotalmoney=D("Crowdorder")->field("SUM(payment_money) as totalmoney")->where("paid =1 AND status > 1 AND mer_id = ".$mer_id." AND is_lottery_order <> 1 ")->find()["totalmoney"];  //总交易额
        $crowddevisionmoney=$crowdtotalmoney*$percent*0.01; //扣点;
		$this->assign($result);
		$this->assign('total_percent', $result['total'] * (100 - $percent) * 0.01);
		$this->assign('all_total_percent', ($result['alltotal'] + $result['alltotalfinsh']) * (100 - $percent) * 0.01+((100 - $percent) * 0.01*$crowdtotalmoney));
		$this->assign('all_totalfinish_percent', round($result['alltotalfinsh'] * (100 - $percent) * 0.01,2)+round((100 - $percent) * 0.01*$crowdtotalmoney,2));
...
//			 $alltotalfinsh=$alltotalfinsh-$r['commission'];//除去佣金的已对帐总额
		...
		$sql1="SELECT sum(gorder.balance_pay + gorder.payment_money) as price, gorder.is_pay_bill,sum(g.commission) as commission FROM ". C('DB_PREFIX') . "group_order as gorder LEFT JOIN pigcms_group as g ON gorder.group_id=g.group_id WHERE gorder.mer_id={$mer_id} AND gorder.pay_time>".strtotime("2017-07-04 00:00:00")." AND gorder.paid=1 AND gorder.status in (1,2) AND (gorder.pay_type<>'offline' OR gorder.balance_pay<>'0.00') GROUP BY gorder.is_pay_bill";
        $res1 = $mode->query($sql1);
        $alltotalfinish1=0;
        $allmoney1=0;
        foreach ($res1 as $k){
            $k['is_pay_bill'] && $alltotalfinish1 += $k['price'];//已对账的总额
        //    $alltotalfinish1=$alltotalfinish1-$k['commission'];//除去佣金的已对帐总额
            $allmoney1+=$k['price'];
        }
        $devisionmoney=$allmoney1*$percent*0.01;      //扣除扣点

    //    echo $alltotalfinsh."------".$devisioncommission."------".$devisionmoney;
       // echo  $percent; die;
    //   echo $alltotalfinsh;die;
      //  echo  $devisionmoney;die;
	//	$this->assign('all_totalfinish_percent', $alltotalfinsh * (100 - $percent) * 0.01);
        $crowdtotalmoney=D("Crowdorder")->field("SUM(payment_money) as totalmoney")->where("paid =1 AND status > 1 AND mer_id = ".$mer_id." AND is_lottery_order <> 1 ")->find()["totalmoney"];  //总交易额
        $crowddevisionmoney=$crowdtotalmoney*$percent*0.01; //扣点;
       $this->assign('all_totalfinish_percent', round($alltotalfinsh-$devisionmoney-$devisioncommission+$crowdtotalmoney*(100-$percent)*0.01,2));

		//end
     //echo  $alltotalfinish1;die;
//  $real_can_cash=round($alltotalfinsh-$devisionmoney-$devisioncommission-$allGetCash,2);
   echo  $alltotalfinsh."------".$devisioncommission."------".$devisionmoney."------".round($allGetCash,2);
 // echo $real_can_cash;
...
//			}
			...
			$sql = "SELECT sum(morder.balance_pay + morder.payment_money) as price, morder.is_pay_bill,0 as commission FROM ". C('DB_PREFIX') . "meal_order as morder  WHERE morder.mer_id={$mer_id} AND morder.paid=1 AND morder.status in (1,2) AND (morder.pay_type<>'offline' OR morder.balance_pay<>'0.00') GROUP BY morder.is_pay_bill";
			$sql .= ' UNION ALL ';
			$sql .= "SELECT sum(gorder.balance_pay + gorder.payment_money) as price, gorder.is_pay_bill,sum(g.commission) as commission FROM ". C('DB_PREFIX') . "group_order as gorder LEFT JOIN pigcms_group as g ON gorder.group_id=g.group_id WHERE gorder.mer_id={$mer_id} AND gorder.paid=1  GROUP BY gorder.is_pay_bill";
			$res = $mode->query($sql);
			$alltotalfinsh = 0;
            $devisioncommission=0;   //扣除佣金
			foreach ($res as $r) {
		...
		//		 $alltotalfinsh=$alltotalfinsh-$r['commission'];//除去佣金的已对帐总额
           ...
           //     $alltotalfinish1=$alltotalfinish1-$k['commission'];//除去佣金的已对帐总额
            ...
            $devisionmoney=$allmoney1*$percent*0.01;
         //   $devisionmoney=$alltotalfinish1*$percent*0.01;
			//$real_can_cash=($alltotalfinsh * (100 - $percent) * 0.01)-$allGetCash;
           $real_can_cash=round($alltotalfinsh-$devisionmoney-$devisioncommission-$allGetCash,2);
           //echo  $real_can_cash;die;
		 //     echo  $alltotalfinsh."------".$devisioncommission."------".$devisionmoney."------".round($allGetCash,2);die;
            $crowdtotalmoney=D("Crowdorder")->field("SUM(payment_money) as totalmoney")->where("paid =1 AND status > 1 AND mer_id = ".$mer_id." AND is_lottery_order <> 1 ")->find()["totalmoney"];  //总交易额
            $crowddevisionmoney=$crowdtotalmoney*$percent*0.01; //扣点;
			if($_POST['cash_num']>($real_can_cash+round((100-$percent)*$crowdtotalmoney*0.01,2))){
			 		$this->error("申请提现金额超过可提现金额，禁止申请！"."----".$real_can_cash);
			}
