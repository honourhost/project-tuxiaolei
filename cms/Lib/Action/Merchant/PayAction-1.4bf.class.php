...
class PayAction extends BaseAction{
		...
		if($now_order['paid']){
			switch($_GET['order_type']){
				...
				case 'food':
					...
					break;
				case 'recharge':
					$redirctUrl = C('config.site_url').'/merchant.php?g=Merchant&c=Merchant&a=recharge&order_id='.$now_order['order_id'];
					break;
				case 'appoint':
	...
	public function alipay_return(){
		...
		$pay_class = new Alipay('','',$pay_type,$pay_method['alipay']['config'],$this->merchant_session,0);
		$go_query_param = $pay_class->query_order();
		if($go_query_param['error'] === 0){
			switch($order_type){
				case 'group':
