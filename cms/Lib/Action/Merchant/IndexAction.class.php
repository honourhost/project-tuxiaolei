...
class IndexAction extends BaseAction{
	...
	public function checkExitVerify(){
		...
		if(!empty($company)){
			...
			$company['legal_represent_cardimage_other'] = $merchant_image_class->get_image_by_path($company['legal_represent_cardimage_other'],$this->config['site_url'],'s');
			if($company['status']==2){
				$flag=1;
			...
			if(!empty($person)){
				...
				$person['person_cardimage_other'] = $merchant_image_class->get_image_by_path($person['person_cardimage_other'],$this->config['site_url'],'s');
				if($person['status']==2){
					$flag=1;
					$this->assign("flag",$flag);
				}
	...
	public function companyVerify(){
		if(IS_POST){
			...
			//新增相关信息的照片上传存储
			if(!empty($_FILES)){
				if(empty($_FILES['business_liscence_image'])&&empty($_FILES['legal_represent_cardimage'])&&($_FILES['legal_represent_cardimage_other'])){
						$this->error("证件照上传出现错误！");
				}
				if($_FILES['business_liscence_image']['error'] != 4&&$_FILES['legal_represent_cardimage']['error']!=4&&$_FILES['legal_represent_cardimage_other']['error']!=4){
					...
					if ($image['error']) {
						...
						$data['legal_represent_cardimage_other']=$_POST['legal_represent_cardimage_other'];
					}
				}else{
	...
	public function personVerify(){
		if(IS_POST){
			...
			//新增相关信息的照片上传存储
			if(!empty($_FILES)){
				if(empty($_FILES['person_cardimage'])&&empty($_FILES['person_cardimage_other'])){
					$this->error("证件照上传出现错误！");
				}
				if($_FILES['person_cardimage']['error'] != 4&&$_FILES['person_cardimage_other']['error']!=4){
					$param = array('size' => $this->config['meal_pic_size']);
					$param['thumb'] = true;
					$param['imageClassPath'] = 'ORG.Util.Image';
					$param['thumbPrefix'] = 'm_,s_';
					...
					if ($image['error']) {
						...
						$data['person_cardimage_other']=$_POST['person_cardimage_other'];
					}
				}else{
					$this->error("证件上传过程出现错误，请重试！");
				}
		...
		if(!empty($_POST)){
			...
			if($_FILES['business_liscence_image']['error'] != 4 || $_FILES['legal_represent_cardimage']['error']!=4 || $_FILES['legal_represent_cardimage_other']['error']!=4){
				...
				if ($image['error']) {
					...
					if($_FILES['legal_represent_cardimage_other']['error'] != 4){
						$data['legal_represent_cardimage_other']=$_POST['legal_represent_cardimage_other'];
					}
				}
			}
			...
			//相关信息的照片修改存储
				if($_FILES['person_cardimage']['error'] != 4 || $_FILES['person_cardimage_other']['error']!=4){
					...
					if ($image['error']) {
						...
						if($_FILES['person_cardimage_other']['error'] != 4){
							$data['person_cardimage_other']=$_POST['person_cardimage_other'];
						}
					}
				}
