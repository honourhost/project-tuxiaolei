...
class GroupAction extends BaseAction {
    ...
    public function add() {
        if (IS_POST) {
            ...
            //如果是拼团商品判断是否添加了参加人数
            if(intval($_POST['is_group_buy'])==1){
                if (empty($_POST['need_num'])) {
                    $this->error('请填写所需参团人数！');
                }
                if(empty($_POST['related_id'])){
                    $this->error("请填写关联的特卖的id！");
                }
            }else{
                //非拼团则删除参团人数字段
                unset($_POST['need_num']);
            ...
            if (empty($_POST['picapp'])) {
                $this->error('请至少上传一张APP照片');
            }
            if (empty($_POST['picthumb'])) {
                $this->error('请至少上传一张APP缩略照片');
            }
            ...
            $_POST['picapp'] = implode(';', $_POST['picapp']);
            $_POST['picthumb'] = implode(';', $_POST['picthumb']);
            if ($_POST['cue_field']) {
                $cue_field = array();
            ...
            $marketers=D("Marketers")->select();
            if(is_array($marketers)&&count($marketers)>0){
    ...
    public function frame_edit() {
     ...
     //   echo json_encode($now_group);die;
        ...
        if (IS_POST) {
            ...
            $tmp_pic_arr_app=explode(';',$now_group['picapp']);
            foreach ($tmp_pic_arr_app as $key => $value) {
                $now_group['pic_arr_app'][$key]['title'] = $value;
                $now_group['pic_arr_app'][$key]['url'] = $group_image_class->get_image_by_path($value, 's');
            }
            $tmp_pic_arr_appthumb=explode(';',$now_group['picthumb']);
            foreach ($tmp_pic_arr_appthumb as $key => $value) {
                $now_group['pic_arr_appthumb'][$key]['title'] = $value;
                $now_group['pic_arr_appthumb'][$key]['url'] = $group_image_class->get_image_by_path($value, 's');
            }
            if ($now_group['cue']) {
                $now_group['cue_arr'] = unserialize($now_group['cue']);
            }
            $this->assign('now_group', $now_group);
            //增加是否可以编辑分类的判断，订单中存在已支付未确认的订单则不可以修改特卖详情
            $order=M("Group_order");
            $where=array(
                "group_id"=>$now_group['group_id'],
                "paid"=>1,
            ...
            $mar=D("Marketers")->where(array("id"=>$now_group["marketer_id"]))->find();
            if(!empty($mar)){
                $this->assign("mar",$mar);
            }
    ...
    public function edit(){
        ...
        if ($now_group['group_lock']==1){
            $this->error('该' . $this->config['group_alias_name'] . '已经被锁定，禁止重新编辑！');
        }
        if (IS_POST) {
            if (empty($_POST['name'])) {
                $this->error('请填写商品标题');
    ...
    public function  ajax_get_tags(){
        $alltags=D("Tags")->where(array("status"=>1))->select();
       echo  json_encode($alltags);die;
    ...
    public function order_list() {
        ...
        //订单列表
        $condition_where = "`o`.`uid`=`u`.`uid` AND `o`.`group_id`=`g`.`group_id` AND `o`.`group_id`='$group_id' AND `o`.`mer_id`='$mer_id'";//AND `o`.`paid`='1' 查询所有订单
