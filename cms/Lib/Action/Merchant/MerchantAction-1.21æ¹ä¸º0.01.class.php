...
class MerchantAction extends BaseAction{
    ...
    public function recharge(){
        if(!empty($_POST)){
            ...
            if(empty($money) || $money > 10000 || $money!=1000){
                $this->error('请输入有效的金额，目前为'.$money.'！最高不能超过1万元。');
            }
            ...
            //查询看是否存在已经缴纳押金
            $deposit=D('Merchant_pay_deposite')->where(array("mer_id"=>$this->merchant_session["mer_id"]))->order("create_time desc")->find();
            if(!empty($deposit)){
        ...
        if(!empty($_POST)){
            ...
            if(!empty($deposit)){
                if($deposite['status']==2){
                    ...
                    $this->error('提交的缴纳押金还没有审核或者已经审核通过');
                }
            }else{
                $this->error('不存在已经缴纳的押金');
            }
        }
    }
    //测试
