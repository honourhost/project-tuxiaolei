...
//导入excel会员卡
    ...
    public function uploadvideo($name = 'file', $isAjax = 0)
    {


        $types = array('jpg', 'jpeg', 'gif', 'png', 'mp4', 'mp3');
        // 图片url 域名
        //原文件名
        ...
        if (!isset($_FILES[$name]['tmp_name'])) {
            $this->alert('图片上传失败，图片大小超过限制');
        }


        $aryStr = explode(".", $_FILES[$name]['name']);
        ...
        $newFileName = rand(10) . '.' . $fileType;

        if (!move_uploaded_file($_FILES[$name]['tmp_name'], $upload_dir . $newFileName)) {
            $this->alert('移动失败');
        };

        header('Content-type: text/html; charset=UTF-8');
        echo json_encode(array('error' => 0, 'url' => $upload_dir . $newFileName));
        exit;
...
// 		}
        ...
        $upload->maxSize = 7 * 1024 * 1024 * 1000;
        $upload->allowExts = array('jpg', 'jpeg', 'png', 'gif', 'mp3', 'mp4');
        $upload->allowTypes = array('image/png', 'image/jpg', 'image/jpeg', 'image/gif', 'video/mp4', 'audio/mp3', 'application/octet-stream');
        $path = $path ? $path : 'site';
        $img_mer_id = sprintf("%09d", $this->merchant_session['mer_id']);
        $rand_num = substr($img_mer_id, 0, 3) . '/' . substr($img_mer_id, 3, 3) . '/' . substr($img_mer_id, 6, 3);
        $upload_dir = "./upload/{$path}/{$rand_num}/";
        ...
        if (in_array($_FILES['imgFile']['type'], array('audio/mp3'))) {

            try {
                $des = "/upload/{$path}/{$rand_num}/" . time() . '.mp4';
                move_uploaded_file($_FILES['imgFile']['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . $des);
                die('{"error":0,"url":"' . 'http://' . $_SERVER['HTTP_HOST'] . $des . '"}');
            } catch (Exception $exception) {
                $this->alert($exception);
            }

        } else if (in_array($_FILES['imgFile']['type'], array('video/mp4'))) {
        ...
        if (!$upload->upload()) {// 上传错误提示错误信息
            ...
            //  file_put_contents("tttt.txt",  json_encode($info),FILE_APPEND);
            $this->siteUrl = $this->siteUrl ? $this->siteUrl : $this->config['site_url'];
        ...
        if (!$upload->upload()) {// 上传错误提示错误信息
            ...
            file_put_contents("tttt.txt", json_encode($msg), FILE_APPEND);
        }
        return array('error' => $error, 'msg' => $msg);
    }
