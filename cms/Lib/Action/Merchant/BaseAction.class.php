...
class BaseAction extends Action
{
    ...
    protected function _initialize()
    {
        ...
        if (empty($this->merchant_session) && (MODULE_NAME != "Store")&&(MODULE_NAME != "Pay")) {
		...
		//echo json_encode($this->merchant_session);
		//die;
        $this->config = D("Config")->get_config();
        $this->assign("config", $this->config);
        C("config", $this->config);
        ...
        foreach ($merchant_menu_list as $value ) {
            ...
            if (($value["module"] == MODULE_NAME) && ($value["action"] == ACTION_NAME)) {
                if(empty($this->merchant_session["menus"])){
                    ...
                    if(empty($company)&&empty($person)){
                        $this->error("您还没有完成实名认证，请先完成认证！", U("Index/ruzhu"));
                    }else{
                        $this->error("您已经提交了实名认证信息，请耐心等待审核完成！",U("Index/ruzhu"));
                    }
                }
