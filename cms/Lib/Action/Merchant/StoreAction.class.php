...
class StoreAction extends BaseAction{
	...
	public function login(){
		if(IS_POST){
			...
			//如果已经登录商家账号则如果当前有店员登录只能是该商家下的店员
			$mer_id=$this->merchant_session["mer_id"];
			if(!empty($mer_id)){
			if(($this->merchant_session["mer_id"])!=$now_staff['token']){
				exit(json_encode(array('error'=>'4','msg'=>'非本店店员禁止登录！','dom_id'=>'warning')));
			}
			}
	...
	public function group_list(){
		...
		//先看是否是拼团订单选项
		$condition_where = "`gb`.`status`=1 AND `o`.`store_id`='$store_id' AND `o`.`paid`=1 AND `o`.`status`<=3";

		$page_count=D('GroupBuy')->alias("gb")->distinct(true)->field('o.sun_id,o.*')->join("LEFT JOIN ".C("DB_PREFIX")."group_order o ON `o`.`sun_id`=`gb`.`sun_id` AND `o`.`group_id`=`gb`.`group_id`")->where($condition_where)->count();

		$p = new Page($page_count,10,'page');

		$order_list = D('GroupBuy')->alias("gb")->distinct(true)->field('o.sun_id,o.*')->join("LEFT JOIN ".C("DB_PREFIX")."group_order o ON `o`.`sun_id`=`gb`.`sun_id` AND `o`.`group_id`=`gb`.`group_id`")->where($condition_where)->limit($p->firstRow,$p->listRows)->select();
		
		$this->assign('order_list',$order_list);

		$this->assign('pagebar',$p->show());

		$this->display("group_list_new");
		die;
		}
		...
		if($special){
			...
			$condition_where = "`o`.`uid`=`u`.`uid` AND `o`.`group_id`=`g`.`group_id` AND `o`.`store_id`='$store_id' AND `o`.`paid`=1 AND `o`.`status`=0 AND (`o`.pay_time - `o`.add_time) >".$one_day;
		...
		if($paid==1){
			...
			$condition_where = "`o`.`uid`=`u`.`uid` AND `o`.`group_id`=`g`.`group_id` AND `o`.`store_id`='$store_id' AND `o`.`paid`=1 AND `o`.`status`=0 AND `g`.`is_group_buy` = 0 ";

		}else{
			if($hassend){
				$condition_where = "`o`.`uid`=`u`.`uid` AND `o`.`group_id`=`g`.`group_id` AND `o`.`store_id`='$store_id' AND `o`.`status`=1 AND `g`.`is_group_buy` = 0 ";
			}else{
				$condition_where = "`o`.`uid`=`u`.`uid` AND `o`.`group_id`=`g`.`group_id` AND `o`.`store_id`='$store_id' AND `g`.`is_group_buy` = 0 ";
			}
		}
	...
	public function group_buy_list(){
        ...
        $store_id = $this->store['store_id'];
       // $condition_where = "`gb`.`status`=1 AND `o`.`store_id`='$store_id' AND `o`.`paid`=1 AND `o`.`status`<=3";
        $condition_where = "`gb`.`status`=1 AND `o`.`store_id`='$store_id' AND `o`.`is_lottery_group` = 0 ";

        if(isset($_GET['uppaid'])){
            $condition_where.="AND `o`.`paid` =0";

        }elseif (isset($_GET['unsend'])){
            $condition_where.="AND `o`.`paid` =1 AND `o`.`status`=0";
        }elseif (!empty($_GET['hassend'])){
            $condition_where.="AND `o`.`paid` =1 AND `o`.`status`=1";
        }else{
            $condition_where.="AND `o`.`paid` =1 AND `o`.`status`<=3";
        }
          // exit($condition_where);
        $page_count=D('GroupBuy')->alias("gb")->distinct(true)->field('o.sun_id,o.*')->join("LEFT JOIN ".C("DB_PREFIX")."group_order o ON `o`.`sun_id`=`gb`.`sun_id` AND `o`.`group_id`=`gb`.`group_id`")->where($condition_where)->count();
          import('@.ORG.merchant_page');
        $p = new Page($page_count,10,'page');

        $order_list = D('GroupBuy')->alias("gb")->distinct(true)->field('o.sun_id,o.*')->join("LEFT JOIN ".C("DB_PREFIX")."group_order o ON `o`.`sun_id`=`gb`.`sun_id` AND `o`.`group_id`=`gb`.`group_id`")->where($condition_where)->order('`o`.`order_id` DESC')->limit($p->firstRow,$p->listRows)->select();
    ...
    public function  lottery_group_list(){
        $this->check_group();
        $store_id = $this->store['store_id'];
        // $condition_where = "`gb`.`status`=1 AND `o`.`store_id`='$store_id' AND `o`.`paid`=1 AND `o`.`status`<=3";
        $condition_where = "`gb`.`status`=1 AND `o`.`store_id`='$store_id' AND `o`.`is_lottery_group` = 1 AND `o`.`is_lottery_order` = 1 ";
      ...
      //  exit($condition_where);
        ...
        $this->display("lottery_group_list");

    }
	public function pingroup_find(){
		if(IS_POST){
			$mer_id = $this->store['mer_id'];
			$condition_where = "`o`.`uid`=`u`.`uid` AND `o`.`group_id`=`g`.`group_id` AND `o`.`mer_id`='$mer_id' AND `g`.`is_group_buy` =1 AND `g`.`is_lottery_group_buy` <> 1 ";
			$find_value = $_POST['find_value'];
			$store_id = $this->store['store_id'];
			...
			echo json_encode($return);
			  ...
			  $this->assign('merid',$this->store['mer_id']);
    ...
    public function lotterygroup_find(){
        if(IS_POST){
            $mer_id = $this->store['mer_id'];
            $condition_where = "`o`.`uid`=`u`.`uid` AND `o`.`group_id`=`g`.`group_id` AND `o`.`mer_id`='$mer_id' AND `g`.`is_group_buy` = 1 AND `g`.`is_lottery_group_buy` =1 ";
            $find_value = $_POST['find_value'];
    ...
    public function group_find(){
        if(IS_POST){
            $mer_id = $this->store['mer_id'];
            $condition_where = "`o`.`uid`=`u`.`uid` AND `o`.`group_id`=`g`.`group_id` AND `o`.`mer_id`='$mer_id' AND `g`.`is_group_buy` <> 1 ";
            $find_value = $_POST['find_value'];
            $store_id = $this->store['store_id'];
            if($_POST['find_type'] == 1 && strlen($find_value) == 14){
                $condition_where .= " AND `o`.`group_pass`='$find_value'";
	...
	public function group_edit(){
		...
		if(!empty($now_order['sun_id'])){
			$groupbuy=M("GroupBuy")->where("sun_id='".$now_order['sun_id']."'")->find();
			if(!empty($groupbuy)){
				$now_order['pinuid']=$groupbuy["user_id"];
			}
		}
...
// 						}
		...
		private function meal_notice($order)
	{
		...
		if ($this->config['sms_finish_order'] == 1 || $this->config['sms_finish_order'] == 3) {
			...
			$sms_data['content'] = '您在 ' . $this->store['name'] . '店中下的订单(订单号：' . $order['order_id'] . '),已经完成了消费，如有任何异议，请您及时联系本店，欢迎再次光临！';
