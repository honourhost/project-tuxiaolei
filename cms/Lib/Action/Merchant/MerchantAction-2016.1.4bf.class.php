...
class MerchantAction extends BaseAction{
    ...
    public function recharge(){
        if(!empty($_POST)){
            ...
            if($order_id = D('Merchant_recharge_order')->data($data_merchant_recharge_order)->add()){
                redirect(U('Merchant/Pay/check',array('order_id'=>$order_id,'type'=>'recharge')));
            }else{
    ...
    //缴纳完成回调页
    public function depositeReturn($deposite){
        if(!empty($deposite)){
        $this->assign("deposite",$deposit);
        $this->display();
        }else{
       $order_id=$_GET['order_id'];
        if(!empty($order_id)){
            $deposit=D('Merchant_pay_deposite')->where("order_id=".$order_id)->find();
            $this->assign("deposite",$deposit);
            $this->display();
        }else{
            $this->display();
