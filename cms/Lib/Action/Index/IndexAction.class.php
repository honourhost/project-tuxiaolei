...
//            die();
        ...
        $this->assign("toptui",$this->merchant_list2());
      //  if($_SESSION["user"]["uid"]==753){
       //     echo  json_encode($this->merchant_list2());die;
      //  }

        //众筹推荐
        $crowdfundings=D("Crowdfunding")->field("title,digest,hit,webpic,mer_id,crowd_id,progress")->where(" status = 1 ")->page(1,6)->order("crowd_id DESC")->select();
        $group_image_class = new group_image();
        ...
        $this->assign("crowdlist",$crowdfundings);
		$this->display();
    }
     ...
     * 后台主动推荐的活动
     */
    public function merchant_list2(){


        $database_adver = D('Adver');
        $condition_adver['cat_id'] = 23;
        $merchant_list = $database_adver->field(true)->where($condition_adver)->order('`id` DESC')->select();
        foreach ($merchant_list as $key=>$value){
            $merchant_list[$key]["merinfo"]=$this->getmerinfo2($value["desc"]);
        }

        return $merchant_list;
    }

    protected  function  getmerinfo2($merid){
        $list=M("Group")->where(array("group_id"=>$merid))->find();
        $merchant_image_class=new group_image();
        $merchant=D("Merchant")->where(array("mer_id"=>$list['mer_id']))->find();
        if(!empty($merchant["person_image"])){
            $merson_image=new merchant_image();
            $merchant["person_image"]=$merson_image->get_image_by_path($merchant["person_image"]);
        }
        $list['merchantperson']=$merchant;
        if (!empty($list['pic'])) {
            //echo "fffff";die;
            $list["pic"] = $merchant_image_class->get_image_by_path($list['pic']);
        }
        return $list;
    }
  ...
  return $list;
	  ...
	  public function  statistic(){
        ...
        $pigcms_assign["group_money_count"]=D("Group_order")->sum("payment_money");
          $fast1=D("Fast_order")->field("sum(payment_money) as totalm")->select();
          $pigcms_assign['fast_order_sum']=$fast1[0]['totalm'];
          $pigcms_assign['fast_order_sum_mon']=D("FastOrder")->get_all_oreder_sum("month");
          $fast2=D("Fast_order")->field("count(payment_money) as totalm")->select();
          $pigcms_assign['fast_order_count']=$fast2[0]['totalm'];
          $pigcms_assign['fast_order_count_mon']=D("FastOrder")->get_all_oreder_count("month");
          $pigcms_assign['fast_order_count_today']=D("FastOrder")->get_all_oreder_count("day");
          $pigcms_assign['fast_order_sum_today']=D("FastOrder")->get_all_oreder_sum("day");
    ...
    public  function pwdtest(){
        ...
        if($_SESSION["ppwwddtest"]=="xiaonongding"){
            redirect(U("Index/Index/statistest"));
        }else{
            $this->display("pwdtest");
        }


    }

    public function  statistest(){

        if(IS_POST){
            $pwd=$_POST["password"];
            if($pwd!="xiaonongding"){
                // $this->error_tips("请输入正确的",U("Index/Index/pwd"));
                redirect(U("Index/Index/pwdtest"));
            }else{
                session("ppwwddtest",$pwd);
            }
        }

        if(empty($_SESSION["ppwwddtest"])||$_SESSION["ppwwddtest"]!="xiaonongding"){
            //$this->error_tips("请输入密码",U("Index/Index/pwd"));
            redirect(U("Index/Index/pwdtest"));
        }
