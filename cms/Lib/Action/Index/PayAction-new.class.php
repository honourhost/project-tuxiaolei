...
class PayAction extends BaseAction{
	public function check(){
		...
		// 	}
		// }
		if($this->config['pay_limit']){
			if($this->config['pay_limit_group']){
				$result=D("Group_order")->field("group_id")->where("order_id=".$order_info['order_id'])->find();
				$arr=explode(",",$this->config['pay_limit_group']);
				//如果在过滤余额支付的商品中，则将其余额虚拟为0，禁止使用余额支付
				if(in_array($result['group_id'], $arr)){
					$now_user['now_money']=0;
				}else{
					//如果不在过滤的商品中，则使用余额，并使用限制余额支付的数额
				if($now_user['now_money']>($this->config['pay_limit_num'])){
				$now_user['now_money']=floatval($this->config['pay_limit_num']);
				}
				}
			}
		}
