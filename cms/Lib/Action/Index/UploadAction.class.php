...
// 			}
    ...
    public function editor_ajax_uploadfile(){
        if(!in_array($_GET['upload_dir'],array('group/content','merchant/news','activity/content','system/image','activity/index_pic','appoint/content'))){
            $this->editor_alert('非法的目录！');
        }

        if($_FILES['imgFile']['error'] != 4){
            $upload_dir = './upload/crowdvideo/';
 			if(!is_dir($upload_dir)){
 				mkdir($upload_dir,0777,true);
 			}

 			import('ORG.Net.UploadFilenew');
 			$upload = new UploadFilenew();
 		    $upload->maxSize = 20*1024*1024;
 			$upload->allowExts = array('mp4','wmv');
 			$upload->allowTypes = array('video/mp4','video/x-ms-wmv');
 			$upload->savePath = $upload_dir;
 			$upload->thumb=false;
 			$upload->saveRule = 'uniqid';
 			if($upload->upload()){
