...
class ActivityAction extends BaseAction{
	public function index() {
		...
		if($_SESSION['areaarr']!="all"){
			...
			if($count!=0){
				$group_list = D('')->field($field_condition)->table($condition_table)->where($condition_where)->order("g.index_tui desc")->select();
			}else{
				$now_time = $_SERVER['REQUEST_TIME'];
		...
		if(!empty($group_list)){
			...
			foreach($group_list as $k=>$v){
				...
				$group_list[$k]['mer_person']=C('config.site_url').'/upload/merchant/'.$mer_image;
			}

			$this->assign("grouplist",$group_list);
            $this->assign("toptui",$this->merchant_list());
     //echo  json_encode($this->merchant_list());die;
			$this->display();
		}
	}
		...
		switch($now_activity['type']){
			case '1':
				if($now_activity['is_finish']){
					...
					if(strstr($lottery_user['avatar'],",")) {
					$avatar_image_class = new avatar_image();
		
					$image= $avatar_image_class->get_image_by_path($lottery_user['avatar'], C('config.site_url'));
					$lottery_user['avatar']=$image['s_image'];
					}
	...
	private function convertPartList($tmp_part_list){
		...
		foreach($tmp_part_list as $value){
			...
			if(strstr($value['avatar'],",")) {
				    $avatar_image_class = new avatar_image();
					$image = $avatar_image_class->get_image_by_path($value['avatar'], C('config.site_url'));
					$value['avatar']=$image['s_image'];
				}
	...
	public function submit(){
		...
		if($now_activity['money'] != 0){
			...
			$save_result = D('User')->user_money($now_user['uid'],$use_money,'参加一元抢购活动');
	...
	protected function type_list($area_url=''){
		if($area_url){
			return array(
				...
				array('name'=>'一元抢购','type'=>'1','url'=> $this->config['site_url'].'/activity/1/'.$area_url),
			...
			return array(
				...
				array('name'=>'一元抢购','type'=>'1','url'=> $this->config['site_url'].'/activity/1/all'),
