...
class MerchantAction extends BaseAction {
    ...
    public function index() {
        ...
        //视频
        $videos=D("Merchant_video")->where("mer_id=".$this->merid)->limit(3)->order("sort asc")->select();
        $this->assign("videos",$videos);
        //三个单独的分类及其下的商品
        $threecategory=D("Group")->field("*")->where(array("mer_id"=>$this->merid,"status"=>1))->order("`sort` asc")->select();
        //去除选数组中含有两个字段的重复值的方法
        $key1="cat_fid";
        $key2="cat_id";
        ...
        $this->assign("threelast",array_filter($threelast));
        //农小店分类选取
        $meal_catogory=D("Meal_sort")->where("store_id=".$this->mainstore['store_id'])->order("sort asc")->select();
        //选取对应分类下的食品
        //现在是按照索引对应，即0-0,1-1
        $catogory_meals=null;
        foreach($meal_catogory as $key=>$val){
            $meals=D("Meal")->where(array("sort_id"=>$val['sort_id'],"store_id"=>$this->mainstore['store_id']))->limit(6)->select();
            $catogory_meals[]=$meals;
        }
        //选取模板分类及其对应内容
