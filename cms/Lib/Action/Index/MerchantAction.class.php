...
class MerchantAction extends BaseAction {
    ...
    //    echo json_encode($imgs);die;
     ...
     // echo D()->getLastSql(); die;
      //      echo  json_encode($threecategory);die;
        }
        $this->assign("allproduct",$threecategory);
        $newthreecategory=$threecategory;
        //去除选数组中含有两个字段的重复值的方法
        $key1="cat_fid";
        $key2="cat_id";
        $last=$this->deleteRepeat($newthreecategory,$key1,$key2);
      ...
      //  echo  json_encode($moduledetails);die;
        //选出当前农场的信息
