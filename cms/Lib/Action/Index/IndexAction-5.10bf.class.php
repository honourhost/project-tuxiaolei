...
class IndexAction extends BaseAction {
  ...
  //   	$this->assign('search_hot_list',$search_hot_list);
		...
		//taorong
		// 首页tuijian团购
		$index_tui_group_list =D('Group')->get_group_list('index_tui',9);
		...
		foreach($res as $key=>$val){
			$artlist[]=$Image_text->alias("t")->field("t.pigcms_id,t.title,t.digest,t.cover_pic,m.area_id")->join(C("DB_PREFIX")."merchant m ON t.mer_id=m.mer_id")->where("t.cat_id=".$val['id'])->order("count DESC,dateline DESC")->limit(6)->select();
		}
