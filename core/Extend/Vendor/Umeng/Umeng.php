...
class Umeng {
	...
	function sendAndroidUnicast($data,$title,$device_tokens) {
		try {
            ...
            $unicast->setPredefinedKeyValue("after_open",       "go_cumtom");
//			$unicast->setPredefinedKeyValue("after_open",       "go_app");
            $data=json_encode(array("name"=>"jiangke","value"=>"27"));
			$unicast->setPredefinedKeyValue("custom",  $data);
			// Set 'production_mode' to 'false' if it's a test device.
			// For how to register a test device, please see the developer doc.
			...
			$unicast->send();
//			 print("Sent SUCCESS\r\n");
            file_put_contents("upush.txt","---send success---",FILE_APPEND);
		} catch (Exception $e) {
            file_put_contents("upush.txt","----send FAILED ----",FILE_APPEND);
			 print("Caught exception: " . $e->getMessage());
		}
	}
