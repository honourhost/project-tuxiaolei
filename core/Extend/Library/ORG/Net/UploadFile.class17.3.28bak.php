...
class UploadFile {//类定义开始
    ...
    private function save($file) {
		// file_put_contents("tttt.txt",  json_encode($file),FILE_APPEND);
        $filename = $file['savepath'].$file['savename'];
	 file_put_contents("tttt.txt",  $filename,FILE_APPEND);
        ...
        if( in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png','swf'))) {
            ...
            if(false === $info || ('gif' == strtolower($file['extension']) && empty($info['bits']))){
				 file_put_contents("tttt.txt",  'feifatupianwenjian',FILE_APPEND);
                $this->error = '非法图像文件';
                return false;                
            }
        }
        //中转文件
        $new=dirname($filename)."/hehe.".$file['extension'];
        ...
        //先保存到临时文件中
        if(!move_uploaded_file($file['tmp_name'], $this->autoCharset($new,'utf-8','gbk'))) {//$file['tmp_name']
        ...
        Image::changeImage($new,$filename);
        }catch(Exception $e){
            $this->error=$e->getMessage();
			       file_put_contents("tttt.txt",$this->error."2333",FILE_APPEND);
            return false;
        }   
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    ...
                    Image::thumb($filename,$thumbPath.$thumbname.'.'.$thumbExt,$thumbExt,$thumbWidth[$i],$thumbHeight[$i],true);                    
                }
